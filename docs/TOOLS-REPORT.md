...
| `tools/local-e2e/`                               | **Dossier unique** des scripts Playwright locaux : `human-qa-test.mjs`, Axe, réseau, audit/prod, screenshots + helpers. Voir `tools/local-e2e/README.md`. | Oui. Plus de `*-test.mjs` à la racine.                                                                                        |
...
| `tools/check-external-links.js`                  | Garde anti-regression : echoue si un lien de navigation externe non allowliste apparait dans `client/src`. Baseline `tools/external-links-allowlist.json` (`--update`).                    | Oui. Controle les ancres ; `tools/local-e2e/local-network-test.mjs` contrôle les requêtes.                                    |
...
- `tools/local-e2e/local-network-test.mjs` bloque GTM/GA/Stripe, le port 3000 et les problèmes
  console audités dans les trois moteurs ;
- `tools/local-e2e/axe-test.mjs --strict` garantit que toutes les pages demandées ont été
  réellement scannées.
