...
À l’intérieur de la boucle, définis l’événement `oninput` de chaque élément `input` sur la fonction `handleInput`.

Avec ça, l’atelier du gestionnaire de formes est terminé !

# --hints--

Tu dois définir `input.oninput` sur la fonction `handleInput`.
