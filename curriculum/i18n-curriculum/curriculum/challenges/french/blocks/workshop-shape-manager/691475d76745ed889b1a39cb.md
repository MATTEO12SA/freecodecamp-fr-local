...
Tu dois aussi travailler sur une fonction qui effacera les champs de saisie.

Crée une fonction `clearInputFields`. À l’intérieur, utilise `querySelectorAll` pour récupérer tous les champs de saisie et parcours-les avec `forEach()`. À l’intérieur du `forEach()`, définis la valeur de chaque `input` sur une chaîne vide.

# --hints--

Tu dois créer une fonction `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

Ta fonction `clearInputFields` doit définir chaque champ de saisie sur une chaîne vide.
