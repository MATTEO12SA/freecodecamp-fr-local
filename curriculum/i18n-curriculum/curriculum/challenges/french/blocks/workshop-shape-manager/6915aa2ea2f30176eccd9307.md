...
Utilise l’événement `oninput` sur `shapeTypeSelect` et définis-le sur `handleShapeSelect`. Cela fera s’exécuter la fonction à chaque changement de sélection de forme.

Après cela, tu devrais pouvoir voir les champs de saisie de chaque forme. Mais les calculs ne fonctionneront pas encore, et c’est ce que tu géreras ensuite.

# --hints--

Tu dois définir `shapeTypeSelect.oninput` sur `handleShapeSelect`.
