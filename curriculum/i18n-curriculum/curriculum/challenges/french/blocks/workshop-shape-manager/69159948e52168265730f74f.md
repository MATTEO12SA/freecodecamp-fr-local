...
Tu as besoin d’un moyen d’accéder en toute sécurité à l’élément select et de te protéger contre les valeurs `null`. Pour cela, crée une constante `val` avec la valeur `e.currentTarget` et type-la `HTMLSelectElement` avec le mot-clé `as`.

Sous la constante, utilise une instruction `if` pour vérifier si `val` est falsy en la niant. Dans le bloc `if`, renvoie `target value not found`.

# --hints--

Tu dois avoir une constante appelée `val`.

```js
assert.match(code, /const\s+val/)
```

Ta constante `val` doit avoir la valeur `e.currentTarget` typée `HTMLSelectElement` avec le mot-clé `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Tu dois avoir une instruction `if` qui vérifie si `val` est null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Ton instruction `if` doit renvoyer `target value not found`.
