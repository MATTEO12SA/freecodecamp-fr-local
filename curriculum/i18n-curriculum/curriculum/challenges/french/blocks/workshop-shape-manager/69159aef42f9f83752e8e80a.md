...
Maintenant, appelle la fonction `chooseShape()` avec `val.value` pour afficher les bons champs de saisie, puis appelle `updateResult()` pour recalculer et afficher l’aire.

# --hints--

Tu dois appeler la fonction `chooseShape` avec `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Tu dois appeler la fonction `updateResult()`.
