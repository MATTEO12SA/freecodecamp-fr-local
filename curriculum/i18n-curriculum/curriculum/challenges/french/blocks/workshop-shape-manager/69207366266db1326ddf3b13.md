...
Ajoute l’événement `DOMContentLoaded` au document racine et passe `initializeApp` comme fonction callback à exécuter. Cela garantit que toute l’initialisation n’a lieu qu’après le chargement complet du DOM.

# --hints--

Tu dois ajouter l’événement `DOMContentLoaded` au `document` racine.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Tu dois référencer `initializeApp` comme fonction à exécuter dans ton écouteur d’événement.
