...
Pour enfin afficher les calculs, parcours l’objet `propertyInputs` avec une boucle `for...of` et `Object.entries()`. Tu peux ignorer la clé en utilisant une virgule avant `input` dans la déclaration de la boucle.

# --hints--

Tu dois créer une boucle `for...of` qui déstructure chaque entrée de `Object.entries(propertyInputs)` en sautant la clé et en ne gardant que `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Ta boucle `for...of` doit itérer sur `Object.entries(propertyInputs)`.
