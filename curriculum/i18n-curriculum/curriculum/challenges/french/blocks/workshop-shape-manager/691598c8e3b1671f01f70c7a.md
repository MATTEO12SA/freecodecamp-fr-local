...
À l’intérieur de la fonction `handleShapeSelect`, appelle la méthode `.preventDefault()` sur `e`, et appelle la fonction `clearInputFields()` pour effacer les champs de saisie.

# --hints--

Tu dois appeler `.preventDefault()` sur `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Tu dois appeler la fonction `clearInputFields`.
