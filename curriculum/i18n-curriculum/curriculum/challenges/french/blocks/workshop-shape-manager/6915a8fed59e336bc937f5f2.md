...
Le dernier ensemble d’éléments à sélectionner sont les éléments de résultat. Utilise ta fonction `getElement` pour récupérer les éléments avec les ids `result-text` et `result-card`. Les deux doivent utiliser `HTMLElement` comme type.

Souviens-toi que tu as déjà déclaré les variables `resultText` et `resultCard` pour eux, donc tu n’as qu’à les assigner.

# --hints--

Tu dois assigner une valeur à la variable `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` doit sélectionner l’élément `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Tu dois utiliser `HTMLElement` comme type du `getElement` que tu utilises pour sélectionner l’élément `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Tu dois assigner une valeur à la variable `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` doit sélectionner l’élément `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Tu dois utiliser `HTMLElement` comme type du `getElement` que tu utilises pour sélectionner l’élément `result-card`.
