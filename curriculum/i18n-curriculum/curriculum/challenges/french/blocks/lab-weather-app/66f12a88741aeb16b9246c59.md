...
title: Crée une application météo
challengeType: 25
...
Dans ce lab, tu vas construire une application qui fournit des informations météo de différentes villes.

Tu utiliseras une API météo. Les données de sortie ont le format suivant :
...
1. Tu dois avoir un élément `button` avec un `id` de `get-weather-btn`.

1. Tu dois avoir un élément `select` avec sept éléments `option` imbriqués dedans. La première option doit avoir une chaîne vide comme texte et attribut `value`. Les autres doivent avoir ce qui suit pour leur texte et valeurs (la valeur étant en minuscules) :
...
1. Si aucune ville n'est sélectionnée, appuyer sur le bouton ne doit rien faire.

1. Si une ville est sélectionnée, des éléments pour afficher la météo doivent apparaître :

   - Tu dois avoir un élément `img` avec l'id `weather-icon` pour afficher l'icône météo.

   - Tu dois avoir un élément avec l'id `main-temperature` pour afficher la température principale.

   - Tu dois avoir un élément avec l'id `feels-like` pour afficher la température ressentie.

   - Tu dois avoir un élément avec l'id `humidity` pour afficher le taux d'humidité dans l'air.

   - Tu dois avoir un élément avec l'id `wind` pour afficher la vitesse du vent.

   - Tu dois avoir un élément avec l'id `wind-gust` pour afficher les rafales de vent.

   - Tu dois avoir un élément avec l'id `weather-main` pour afficher le type de météo principal.

   - Tu dois avoir un élément avec l'id `location` pour afficher la localisation actuelle.

1. Tu dois avoir une fonction asynchrone nommée `getWeather` qui récupère les informations météo depuis l'API `https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` et les renvoie. Note que cette API renvoie des données en système métrique, c'est-à-dire m/s pour la vitesse du vent, et Celsius pour la température.

1. La fonction asynchrone `getWeather` doit accepter une ville comme argument.

1. Tu dois gérer toutes les erreurs qui surviennent dans la fonction `getWeather` et les afficher dans la console.

1. Tu dois avoir une fonction asynchrone `showWeather` qui accepte une ville comme paramètre.

1. La fonction `showWeather` doit appeler la fonction `getWeather` pour récupérer les données météo de la ville sélectionnée dans le menu déroulant.

1. Si la fonction `getWeather` a eu une erreur, l'application ne doit afficher qu'une alerte qui dit `Something went wrong, please try again later`.

1. Si les données de `getWeather` sont utilisables, la fonction `showWeather` doit afficher les données météo dans les éléments correspondants. Si une certaine valeur de l'API est `undefined`, tu dois écrire `N/A` dans l'élément correspondant.

NOTE : Les tests prendront du temps à se terminer. Tant que tu vois `// running tests` dans la console, ils sont en cours d'exécution.
...
Tu dois avoir un élément `button` avec un `id` de `get-weather-btn`.

```js
assert.exists(document.querySelector('button#get-weather-btn'));
```

Tu dois avoir un élément `select`.

```js
assert.exists(document.querySelector('select'));
```

Dans l'élément `select`, le premier enfant doit être un élément `option` avec un attribut `value` vide.
...
Dans l'élément `select`, il doit y avoir 6 éléments `option`, un pour chacune des villes suivantes : Paris, London, Tokyo, Los Angeles, Chicago, New York.
...
Tu dois avoir une fonction `showWeather`.

```js
assert.isFunction(showWeather);
```

Tu dois avoir une fonction `getWeather`.

```js
assert.isFunction(getWeather);
```

La fonction `getWeather` doit accepter une ville comme seul argument et renvoyer le JSON de l'API Weather.
...
La fonction `showWeather` doit appeler la fonction `getWeather` pour obtenir les données météo.
...
La fonction `showWeather` doit gérer le cas où `getWeather` renvoie `undefined`.
...
Quand New York est sélectionné, la fonction `showWeather` doit afficher les données de l'API dans les éléments HTML respectifs. Si la valeur de l'API est `undefined`, tu dois afficher `N/A`.
...
Quand Chicago est sélectionné, la fonction `showWeather` doit afficher les données de l'API dans les éléments HTML respectifs. Si la valeur de l'API est `undefined`, tu dois afficher `N/A`.
...
Quand London est sélectionné, la fonction `showWeather` doit afficher les données de l'API dans les éléments HTML respectifs. Si la valeur de l'API est `undefined`, tu dois afficher `N/A`.
...
Quand Tokyo est sélectionné, la fonction `showWeather` doit afficher les données de l'API dans les éléments HTML respectifs. Si la valeur de l'API est `undefined`, tu dois afficher `N/A`.
...
Quand Los Angeles est sélectionné, la fonction `showWeather` doit afficher les données de l'API dans les éléments HTML respectifs. Si la valeur de l'API est `undefined`, tu dois afficher `N/A`.
...
S'il y a une erreur, ta fonction `getWeather` doit afficher l'erreur dans la console.
...
Quand Paris est sélectionné, l'application doit afficher une alerte avec `Something went wrong, please try again later`.
