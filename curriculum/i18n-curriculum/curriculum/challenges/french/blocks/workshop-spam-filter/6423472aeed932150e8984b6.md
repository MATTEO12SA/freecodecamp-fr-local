...
En utilisant la même syntaxe, mets à jour `c` pour qu'il corresponde à `c`, `{`, `[` et `(`.

# --hints--

Ton `stockRegex` devrait utiliser une classe de caractères pour correspondre à `c`, `{`, `[` et `(`.
...
Ton `stockRegex` devrait correspondre à `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

Ton `stockRegex` devrait correspondre à `sto{k alert`.
