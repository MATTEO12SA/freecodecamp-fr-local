...
Ensuite, dans l'élément `input` checkbox, ajoute un attribut `checked` et définis-le à `isChecked`. Bien qu'il ne soit pas strictement nécessaire de définir l'attribut `checked`, c'est une bonne pratique pour s'assurer que chaque case à cocher reflète l'état actuel du tableau `selectedItems`.

# --hints--

Tu dois ajouter un attribut `checked` à l'élément `input` checkbox.
...
Tu dois définir l'attribut `checked` à `isChecked`.
