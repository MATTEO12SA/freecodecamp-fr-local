...
Enfin, déplace ton instruction `console.log()` à l'intérieur de la fonction callback de `useMemo()`, juste au-dessus de l'instruction `return`.

# --hints--

Tu dois déplacer ton log `Filtering items...` avant l'instruction `return` à l'intérieur de la fonction callback de `useMemo()`.
