...
Dans l'élément `li`, au-dessus du texte de `item`, ajoute un élément `input` avec l'attribut `type` défini à `checkbox`. Définis aussi son attribut `onChange` à une fonction anonyme qui appelle `toggleItem()` avec `item` en argument.

Enfin, enveloppe l'`input` et le texte de `item` dans un élément `label` pour une meilleure accessibilité.

# --hints--

Tu dois ajouter un autre élément `input` avec l'attribut `type` défini à `checkbox`.
...
Ton nouvel `input` doit avoir un attribut `onChange` défini à une fonction anonyme qui appelle `toggleItem()` avec `item` en argument.
...
Ton nouvel `input` et le texte de `item` doivent être enveloppés dans un élément `label`.
