...
Comme tu te souviens des leçons précédentes, tu peux utiliser la méthode `map` pour parcourir les différents objets et afficher les composants dynamiquement comme ceci :
...
Utilise la méthode `map()` pour afficher un seul `Card` depuis le tableau. Passe `name`, `title` et `bio` comme props au composant `Card`.

# --hints--

Tu dois utiliser la méthode `map()` pour parcourir le tableau `profiles`.

```js
assert.match(code, /profiles\.map\s*\(/);
```

Tu dois passer les props `name`, `title` et `bio` depuis l'objet `profile`.
