...
Définis la valeur de la propriété `msg` à la chaîne suivante :
...
Ton objet d'animation au sommet de la pile devrait avoir la propriété `msg`.
...
Tu dois définir la valeur de la propriété `msg` à `"decimalToBinary(1) returns '1' (base case) and gives that value to the stack below. Then it pops off the stack."`
