...
Dans la fonction `fetchMoreAuthors`, définis les variables `startingIndex` et `endingIndex` à `+= 8` chacune.
...
Tu dois définir la variable `startingIndex` à `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Tu dois définir la variable `endingIndex` à `+=8`.
