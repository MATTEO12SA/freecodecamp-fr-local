...
Si cette condition est remplie, désactive le bouton en définissant sa propriété `disabled` à `true`. Aussi, définis le `textContent` du bouton à `"No more data to load"`.
...
Tu dois définir la propriété `disabled` de `loadMoreBtn` à `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Tu dois définir le `textContent` de `loadMoreBtn` à `"No more data to load"`.
