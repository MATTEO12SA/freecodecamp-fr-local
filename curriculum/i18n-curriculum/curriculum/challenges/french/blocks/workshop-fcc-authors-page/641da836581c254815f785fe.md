...
La prochaine chose que tu afficheras sont les détails biographiques sur l'auteur. Tu peux le faire en utilisant la propriété `bio`.

Ajoute un élément paragraphe avec un attribut `class` de `"bio"`, puis interpole `bio` à l'intérieur de l'élément paragraphe.
...
Tu dois interpoler `bio` dans ton élément `p`.
