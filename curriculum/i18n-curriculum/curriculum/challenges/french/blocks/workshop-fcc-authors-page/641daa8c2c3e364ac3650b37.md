...
Ensuite, il n'y a pas beaucoup de séparation entre le nom et l'image de chaque auteur, et le reste des détails sur la carte. Un séparateur donnera aux cartes d'auteurs une hiérarchie visuelle claire.

Ajoute un élément `div` au-dessus de la bio de l'auteur et donne-lui un attribut `class` de `"purple-divider"`.
