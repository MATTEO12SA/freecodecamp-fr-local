...
C'est le moment de travailler sur les autres boutons. Les éléments `button` restants ont été ajoutés pour toi dans le fichier HTML.

Pour chaque nouveau bouton (`#confused-btn`, `#sad-btn` et `#loving-btn`), sélectionne-le avec `querySelector` et son ID, puis ajoute un écouteur d'événement click qui appelle `updateCount` avec ce bouton.

Utilise les noms de variables `confusedBtn`, `sadBtn`, `lovingBtn`.

# --hints--

Tu devrais créer une variable nommée `confusedBtn`.

```js
assert.exists(confusedBtn);
```

Tu devrais stocker l'élément `#confused-btn` dans la variable `confusedBtn`.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

Tu devrais créer une variable nommée `sadBtn`.

```js
assert.exists(sadBtn);
```

Tu devrais stocker l'élément `#sad-btn` dans la variable `sadBtn`.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

Tu devrais créer une variable nommée `lovingBtn`.

```js
assert.exists(lovingBtn);
```

Tu devrais stocker l'élément `#loving-btn` dans la variable `lovingBtn`.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

L'élément `#confused-btn` devrait avoir un écouteur d'événement click qui appelle `updateCount`.
...
L'élément `#sad-btn` devrait avoir un écouteur d'événement click qui appelle `updateCount`.
...
L'élément `#loving-btn` devrait avoir un écouteur d'événement click qui appelle `updateCount`.
