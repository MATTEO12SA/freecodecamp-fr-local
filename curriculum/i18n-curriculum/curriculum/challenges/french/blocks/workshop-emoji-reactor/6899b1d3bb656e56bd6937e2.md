...
Sous l'élément `p`, crée un élément `div` avec la classe `btn-container`.

À l'intérieur de l'élément `div`, crée un élément `button`.

Ce `button` devrait avoir un `id` `happy-btn`, une classe `emoji-btn` et un `aria-label` `Happy face emoji`.

# --hints--

Crée un nouvel élément `div` sous l'élément `p`.

```js
assert.exists(document.querySelector("p + div"));
```

Le nouvel élément `div` devrait avoir la classe `btn-container`.

```js
assert.equal(document.querySelector("p + div")?.className, "btn-container");
```

Il devrait y avoir un élément `button` à l'intérieur de l'élément `div`.

```js
assert.exists(document.querySelector("div > button"));
```

L'élément `button` devrait avoir un `id` `happy-btn`.

```js
assert.equal(document.querySelector("div > button")?.id, "happy-btn");
```

L'élément `button` devrait avoir une `class` `emoji-btn`.

```js
assert.equal(document.querySelector("div > button")?.className, "emoji-btn");
```

L'élément `button` devrait avoir un `aria-label` `Happy face emoji`.
