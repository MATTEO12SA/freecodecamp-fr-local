...
Dans les leçons précédentes, tu as appris que `querySelectorAll` renvoie un `NodeList`, une structure semblable à un tableau, et qu'une des méthodes d'un `NodeList` est `forEach()`, qui fonctionne comme le `forEach` des tableaux.

Parcours `btns` avec un `forEach()` qui ajoute le même écouteur d'événement à chaque bouton.

Utilise une fonction callback qui prend un paramètre `btn`, et à l'intérieur du callback, ajoute un écouteur d'événement click qui appelle `updateCount(btn)`.

Avec cela, tu as terminé le workshop.

# --hints--

Tu devrais utiliser `btns.forEach()` pour parcourir les boutons.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

Tous les boutons devraient avoir des écouteurs d'événements qui appellent `updateCount` au clic.
