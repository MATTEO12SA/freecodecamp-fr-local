...
C'est le moment de lier le fichier JavaScript.

Ajoute un élément `script` juste avant la balise de fermeture de l'élément `body`, avec le bon attribut et la bonne valeur pour lier le fichier `script.js`.

# --hints--

Tu devrais avoir un élément `script`.

```js
assert.match(code, /<script[^>]*>\s*<\/script>/);
```

L'élément `script` devrait avoir un `src` `./script.js`.
