...
Maintenant, tu dois obtenir le premier nombre dans le texte `0/10`, d'une façon qui fonctionne aussi pour `5/10` ou `10/10`.

Analyse le contenu texte de l'élément `.count` pour extraire le nombre actuel.

Convertis le compteur d'une chaîne en nombre.

Remplace le précédent `console.log(countEl.textContent)` par ton nouvel affichage.

Affiche le compteur actuel dans ce format : `console.log("Current count:", currCount)`.

Indice : souviens-toi que tu peux utiliser la méthode `split` pour diviser une chaîne en un tableau de sous-chaînes.

# --hints--

Tu devrais afficher le compteur actuel comme un nombre.
...
Tu devrais afficher le compteur actuel comme un nombre d'une façon qui fonctionne pour les différentes valeurs possibles.
