...
Tu as probablement remarqué que ton code devient assez répétitif ! Il y a une meilleure façon de gérer plusieurs éléments similaires.

Au lieu de sélectionner chaque bouton individuellement, utilise `querySelectorAll(".emoji-btn")` pour sélectionner d'un coup tous les boutons avec la classe `emoji-btn`. Stocke le résultat dans une variable appelée `btns`.
...
Tu devrais utiliser `querySelectorAll` pour sélectionner tous les boutons emoji.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Tu devrais stocker le résultat dans une variable appelée `btns`.
...
La variable `btns` devrait contenir les 4 boutons emoji.
