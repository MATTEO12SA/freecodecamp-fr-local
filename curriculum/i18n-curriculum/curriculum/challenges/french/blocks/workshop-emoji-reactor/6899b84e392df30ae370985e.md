...
Maintenant, dans le fichier JavaScript, crée une variable `happyBtn`.

Utilise `querySelector` pour stocker une référence à l'élément `#happy-btn` dans la variable `happyBtn`.

# --hints--

Tu devrais avoir une variable nommée `happyBtn`.

```js
assert.exists(happyBtn);
```

La variable `happyBtn` devrait contenir une référence à l'élément `#happy-btn`.
