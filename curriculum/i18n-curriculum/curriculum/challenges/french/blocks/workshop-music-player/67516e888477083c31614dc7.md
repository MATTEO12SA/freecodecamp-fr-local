...
Si tu regardes de près, tu verras que la chanson en cours de lecture n'est pas mise en évidence dans la playlist, donc tu ne sais pas vraiment quelle chanson est lue. Tu peux corriger cela en créant une fonction pour mettre en évidence toute chanson en cours de lecture.

Crée une fonction nommée `highlightCurrentSong`. À l'intérieur de la fonction, vérifie qu'un élément `.playlist-song` avec `aria-current="true"` existe. Si c'est le cas, utilise la méthode `removeAttribute` pour retirer l'attribut `aria-current` de cet élément.

# --hints--

Tu devrais avoir une fonction nommée `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

Ta fonction `highlightCurrentSong` devrait utiliser la méthode `removeAttribute` pour retirer l'attribut `aria-current` de l'élément `.playlist-song` avec `aria-current="true"` si cet élément existe.
