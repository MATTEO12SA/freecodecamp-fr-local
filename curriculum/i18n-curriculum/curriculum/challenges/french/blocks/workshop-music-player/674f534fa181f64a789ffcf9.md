...
Si `userData.currentSong` n'est pas `null`, tu dois trouver la chanson suivante à lire. Souviens-toi que tu peux utiliser `getNextSong` pour cela.

Complète ta fonction `playNextSong` pour qu'elle lise la chanson suivante de la playlist. Si la chanson actuelle est la dernière, définis `userData.currentSong` à `null`, `userData.songCurrentTime` à zéro et appelle la fonction `pauseSong`.

# --hints--

Quand le `currentSong` de `userData` est truthy et n'est pas la dernière chanson de la playlist, la fonction `playNextSong` devrait lire la chanson suivante de la chanson actuelle.
...
Quand le `currentSong` de `userData` est la dernière chanson de la playlist, la fonction `playNextSong` devrait définir `userData.currentSong` à `null`.
...
Quand le `currentSong` de `userData` est la dernière chanson de la playlist, la fonction `playNextSong` devrait définir `userData.songCurrentTime` à `0`.
...
Quand le `currentSong` de `userData` est la dernière chanson de la playlist, la fonction `playNextSong` devrait appeler la fonction `pauseSong`.
...
Quand le `currentSong` de `userData` est égal à `null`, ta fonction `playNextSong` devrait toujours appeler la fonction `playSong` avec l'`id` de la première chanson du tableau `userData.songs` comme argument.
