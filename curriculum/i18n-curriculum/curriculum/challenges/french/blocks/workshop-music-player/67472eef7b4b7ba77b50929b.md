...
Dans ton objet `userData`, crée une propriété `songs` et définis sa valeur au tableau `allSongs`.

Ensuite, pour gérer les informations de la chanson actuelle et suivre son temps de lecture, crée les propriétés `currentSong` et `songCurrentTime`. Définis les valeurs respectivement à `null` et `0`.

# --hints--

Ton objet `userData` devrait avoir une clé `songs` définie au tableau `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Ton objet `userData` devrait avoir une clé `currentSong` définie à `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Ton objet `userData` devrait avoir une clé `songCurrentTime` définie à `0`.
