...
Si tu appuies sur le bouton précédent quand aucune chanson n'est en lecture, la première chanson sera lue. Pour corriger cela, ajoute une instruction `if` au début de la fonction `playPreviousSong`. Vérifie si la chanson actuelle est `null` et arrête l'exécution de la fonction avec une instruction `return`.

# --hints--

Tu devrais ajouter une instruction `if` qui vérifie si `userData.currentSong === null` et arrête l'exécution de la fonction avec une instruction `return` à l'intérieur de ta fonction `playPreviousSong`.
