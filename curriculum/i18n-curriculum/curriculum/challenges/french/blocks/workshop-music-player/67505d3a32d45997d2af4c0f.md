...
Crée une fonction nommée `playPreviousSong`. À l'intérieur de ta nouvelle fonction, utilise la valeur renvoyée par la fonction `getPreviousSong` pour trouver la chanson à lire. Quand `getPreviousSong()` est `undefined`, lis la première chanson de ta playlist. Sinon, lis la chanson précédente de celle en cours de lecture.

# --hints--

Tu devrais avoir une fonction nommée `playPreviousSong`.

```js
assert.isFunction(playPreviousSong);
```

La fonction `playPreviousSong` devrait lire la chanson précédente de celle en cours de lecture dans la playlist.
...
Quand `getPreviousSong()` est `undefined`, la fonction `playPreviousSong` devrait lire la première chanson de la playlist.
