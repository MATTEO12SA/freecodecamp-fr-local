...
À l'intérieur du tableau `allSongs`, crée un objet avec les propriétés et valeurs suivantes :
...
Ton tableau `allSongs` devrait avoir une valeur.

```js
assert.equal(allSongs.length, 1);
```

Ton tableau `allSongs` devrait avoir un objet comme première valeur.

```js
assert.isObject(allSongs[0]);
```

Ton tableau `allSongs` devrait avoir un objet avec une propriété `id` définie au nombre `0`.

```js
assert.strictEqual(allSongs[0].id, 0);
```

Ton tableau `allSongs` devrait avoir un objet avec une propriété `title` définie à la chaîne `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Ton tableau `allSongs` devrait avoir un objet avec une propriété `artist` définie à la chaîne `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Ton tableau `allSongs` devrait avoir un objet avec une propriété `duration` définie à la chaîne `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Ton tableau `allSongs` devrait avoir un objet avec une propriété `src` définie à la chaîne `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
