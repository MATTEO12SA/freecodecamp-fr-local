...
Ensuite, tu dois afficher le titre et l'artiste de la chanson actuelle dans l'affichage du lecteur.

Comme première étape, accède aux éléments `#player-song-title` et `#player-song-artist`, responsables de l'affichage du titre et de l'artiste, et stocke-les respectivement dans les variables `playingSong` et `songArtist`.

# --hints--

Tu devrais avoir une variable nommée `playingSong`.

```js
assert.isDefined(playingSong);
```

Ta variable `playingSong` devrait avoir la valeur de l'élément `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Tu devrais avoir une variable nommée `songArtist`.
...
Ta variable `songArtist` devrait avoir la valeur de l'élément `#player-song-artist`.
