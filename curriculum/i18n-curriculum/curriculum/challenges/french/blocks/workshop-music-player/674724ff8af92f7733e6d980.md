...
Ensuite, crée un tableau vide nommé `allSongs` pour stocker toutes les chansons.

# --hints--

Tu devrais avoir une variable appelée `allSongs`.

```js
assert.isDefined(allSongs)
```

Tu devrais assigner un tableau à ta variable `allSongs`.

```js
assert.isArray(allSongs)
```

Ton tableau devrait être vide.
