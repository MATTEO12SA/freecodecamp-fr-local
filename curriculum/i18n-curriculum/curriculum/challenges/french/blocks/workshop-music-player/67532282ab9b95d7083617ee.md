...
À l'intérieur de la fonction `playNextSong`, appelle les fonctions `setPlayerDisplay`, `highlightCurrentSong` et `setPlayButtonAccessibleText` pour mettre correctement à jour le lecteur quand la playlist se termine.

# --hints--

Tu devrais appeler `setPlayerDisplay` à l'intérieur de l'instruction `else` de ta fonction `playNextSong`.
...
Tu devrais appeler `highlightCurrentSong` à l'intérieur de l'instruction `else` de ta fonction `playNextSong`.
...
Tu devrais appeler `setPlayButtonAccessibleText` à l'intérieur de l'instruction `else` de ta fonction `playNextSong`.
