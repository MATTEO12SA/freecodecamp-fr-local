...
Il est maintenant temps de tester le bouton de pause.

Ajoute un écouteur d'événement `"click"` à l'élément `pauseButton`, puis passe `pauseSong` comme deuxième argument de l'écouteur d'événement. C'est la fonction que l'écouteur d'événement exécutera.

Teste ton application en cliquant d'abord sur le bouton de lecture, puis sur le bouton de pause. Tu devrais voir que tout fonctionne comme prévu.

# --hints--

Tu devrais appeler la méthode `addEventListener()` sur ta variable `pauseButton`.
...
Tu devrais passer `pauseSong` comme deuxième argument de ta méthode `addEventListener`.
