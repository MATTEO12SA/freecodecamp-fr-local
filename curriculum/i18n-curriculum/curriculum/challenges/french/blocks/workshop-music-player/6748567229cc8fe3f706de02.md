...
Maintenant, tu dois travailler sur la mise en pause de la chanson en cours de lecture.

Définis une fonction `pauseSong` qui ne prend aucun paramètre. À l'intérieur de ta nouvelle fonction, pour stocker le temps actuel de la chanson quand elle est mise en pause, définis le `songCurrentTime` de l'objet `userData` au `currentTime` de la variable `audio`.


# --hints--

Tu devrais avoir une fonction `pauseSong`.

```js
assert.isFunction(pauseSong);
```

Ta fonction `pauseSong` devrait définir le `songCurrentTime` de l'objet `userData` au `currentTime` de la variable `audio`.
