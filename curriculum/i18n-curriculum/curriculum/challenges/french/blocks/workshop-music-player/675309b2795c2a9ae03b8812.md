...
Remarque que la pochette d'album dans le HTML et les chansons du tableau `userData.songs` ont changé. Nous avons remplacé les chansons d'origine par des plus courtes que tu peux utiliser pour tester ton application.

Toutes les fonctionnalités principales sont maintenant en place. Le seul problème maintenant est que la chanson suivante ne se lit pas automatiquement quand la chanson en cours se termine.

Pour corriger cela, tu peux configurer un écouteur d'événement qui détectera quand la chanson en cours se termine. L'écouteur d'événement `"ended"` est approprié pour cela. Il est déclenché quand la lecture d'un média atteint la fin.

Ajoute un écouteur d'événement à l'élément `audio` qui écoute l'événement `"ended"` et passe une référence à la fonction `playNextSong`. Avec cela, le projet du lecteur de musique est terminé !

# --hints--

Tu devrais chaîner la méthode `addEventListener()` à ta variable `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

L'écouteur d'événement que tu as utilisé sur ta variable `audio` devrait écouter un événement `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Tu devrais passer une référence à `playNextSong` à ton écouteur d'événement `"ended"`.
