...
Dans les étapes précédentes, tu as construit la fonctionnalité pour lire une chanson. Maintenant, tu dois ajouter la fonctionnalité au bouton de lecture.

Utilise la méthode `addEventListener()` sur `playButton` et passe un événement `"click"` comme premier argument. Pour l'instant, utilise comme deuxième argument un rappel qui appelle `playSong(0)`.

Ensuite, teste en cliquant sur le bouton de lecture.


# --hints--

Tu devrais ajouter un écouteur d'événement pour l'événement `click` au `playButton` et passer une fonction de rappel qui appelle `playSong(0)` comme deuxième argument de la méthode `addEventListener`.
