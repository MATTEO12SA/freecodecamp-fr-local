...
Ton lecteur de musique devrait garder la trace des chansons, de la chanson en cours de lecture et du temps de la chanson actuelle. Pour cela, tu auras besoin de créer un objet pour stocker ces informations.

Commence par déclarer une nouvelle variable appelée `userData` et assigne-lui un objet vide.

# --hints--

Tu devrais avoir une variable nommée `userData`.

```js
assert.isDefined(userData);
```

Tu devrais assigner un objet vide à ta variable `userData`.
