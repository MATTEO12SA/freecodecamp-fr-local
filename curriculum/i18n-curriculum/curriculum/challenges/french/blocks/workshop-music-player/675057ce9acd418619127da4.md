...
Maintenant, crée une fonction nommée `getPreviousSong`. Fais-lui renvoyer la chanson qui se trouve à `getCurrentSongIndex()` moins un dans `userData.songs`, ou `undefined` si la chanson actuelle est la première de la playlist.

# --hints--

Tu devrais avoir une fonction nommée `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

Ta fonction `getPreviousSong` devrait appeler `getCurrentSongIndex`.
...
Ta fonction `getPreviousSong` devrait renvoyer la chanson précédente de la chanson actuelle.
...
Ta fonction `getPreviousSong` devrait renvoyer `undefined` quand la chanson actuelle est la première de la playlist.
