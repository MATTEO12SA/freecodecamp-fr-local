...
La méthode `find()` récupère le premier élément d'un tableau qui remplit les conditions spécifiées dans la fonction de rappel fournie. Si aucun élément ne satisfait la condition, la méthode renvoie `undefined`.

Dans l'exemple ci-dessous, la méthode `find()` est utilisée pour trouver le premier nombre supérieur à 25 :
...
À l'intérieur de ta fonction `playSong`, utilise la méthode `find()` sur le tableau `userData.songs` pour chercher une chanson dont l'`id` est strictement égal à l'`id` passé à la fonction `playSong`. Assigne l'appel `find` à une variable nommée `song`.

# --hints--

Ta fonction `playSong` devrait appeler la méthode `find` sur `userData.songs`.
...
Ton appel `find` devrait renvoyer une chanson dont l'`id` est strictement égal à l'`id` passé à la fonction `playSong`.
...
Tu devrais assigner ton appel `find` à une variable nommée `song`.
