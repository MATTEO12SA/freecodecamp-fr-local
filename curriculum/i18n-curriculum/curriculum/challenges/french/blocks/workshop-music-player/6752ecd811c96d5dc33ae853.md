...
Pour garantir que l'affichage du lecteur se mette à jour chaque fois qu'une nouvelle chanson commence à être lue, appelle la fonction `setPlayerDisplay()` à l'intérieur de la fonction `playSong()`.

Tu devrais maintenant voir le titre de la chanson et l'artiste apparaître dans l'affichage.

# --hints--

Tu devrais appeler `setPlayerDisplay` à l'intérieur de ta fonction `playSong`.
