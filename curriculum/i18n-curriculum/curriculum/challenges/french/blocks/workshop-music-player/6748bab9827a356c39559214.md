...
Utilise `classList` et la méthode `remove()` pour retirer la classe `playing` de `playButton`, puisque la chanson sera en pause à ce moment.

Pour enfin mettre la chanson en pause, utilise la méthode `pause()` sur la variable `audio`. `pause()` est une méthode de l'API Web Audio pour mettre en pause des fichiers musicaux.

# --hints--

Ta fonction `pauseSong` devrait retirer la classe `playing` de `playButton`.
...
Ta fonction `pauseSong` devrait appeler la méthode `pause()` sur `audio`.
