...
Accède aux éléments `#next` et `#previous` du fichier HTML.

Assigne-les respectivement aux variables nommées `nextButton` et `previousButton`.

# --hints--

Tu devrais avoir une variable appelée `nextButton`.

```js
assert.isDefined(nextButton);
```

Tu devrais assigner l'élément `#next` à la variable `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

Tu devrais avoir une variable appelée `previousButton`.

```js
assert.isDefined(previousButton);
```

Tu devrais assigner l'élément `#previous` à la variable `previousButton`.
