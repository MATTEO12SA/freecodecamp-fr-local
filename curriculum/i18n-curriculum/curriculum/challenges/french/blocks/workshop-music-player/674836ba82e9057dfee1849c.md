...
Ensuite, utilise la propriété `classList` et la méthode `add()` pour ajouter la classe `playing` à l'élément `playButton`. Cela cherchera la classe `playing` dans le fichier CSS et l'ajoutera à l'élément `playButton`.

Puis, définis `userData.currentSong` à `song`, et pour enfin lire la chanson, utilise la méthode `play()` sur la variable `audio`. `play()` est une méthode de l'API Web Audio pour lire un fichier mp3.

# --hints--

Tu devrais ajouter la classe `playing` à l'élément `playButton` à l'intérieur de ta fonction `playSong`.
...
Tu devrais définir `userData.currentSong` à `song` à l'intérieur de ta fonction `playSong`.
...
Tu devrais appeler la méthode `play()` sur la variable `audio` à l'intérieur de ta fonction `playSong`.
