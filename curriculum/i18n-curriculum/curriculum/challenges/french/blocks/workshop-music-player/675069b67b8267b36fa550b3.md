...
Ajoute un écouteur d'événement `"click"` à l'élément `previousButton`, puis passe `playPreviousSong` comme deuxième argument.

# --hints--

Tu devrais appeler la méthode `addEventListener()` sur ta variable `previousButton`.
...
Tu devrais passer `playPreviousSong` comme deuxième argument de ta méthode `addEventListener`.
