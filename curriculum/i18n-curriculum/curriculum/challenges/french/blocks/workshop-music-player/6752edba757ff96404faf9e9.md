...
Pour rendre l'application plus accessible, il est important que le bouton de lecture décrive la chanson actuelle ou la première chanson de la playlist.

Crée une fonction nommée `setPlayButtonAccessibleText`. À l'intérieur de la fonction, définis l'attribut `aria-label` du bouton de lecture à `Play` suivi d'un espace et du titre de la chanson actuelle. Si `userData.currentSong` est `null`, définis-le à `Play`.

# --hints--

Tu devrais avoir une fonction nommée `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Ta fonction `setPlayButtonAccessibleText` devrait définir l'attribut `aria-label` du bouton de lecture à `Play` suivi d'un espace et du titre de la chanson actuelle.
...
```
 ...
 Quand `userData.currentSong` est `null`, `setPlayButtonAccessibleText` devrait définir l'attribut `aria-label` du bouton de lecture à `Play`.
