...
Dans ce workshop, tu vas créer une application de lecteur de musique capable de lire, mettre en pause et passer des chansons d'une playlist.

Le HTML et le CSS de ce projet t'ont déjà été fournis, pour que tu puisses te concentrer sur le JavaScript.

Commence par accéder aux éléments `#playlist-songs`, `#play` et `#pause`, et assigne-les respectivement aux variables `playlistSongs`, `playButton` et `pauseButton`.


# --hints--

Tu devrais avoir une variable appelée `playlistSongs`.

```js
assert.isDefined(playlistSongs);
```

Tu devrais assigner l'élément `#playlist-songs` à la variable `playlistSongs`.

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Tu devrais avoir une variable appelée `playButton`.

```js
assert.isDefined(playButton);
```

Tu devrais assigner l'élément `#play` à la variable `playButton`.

```js
assert.equal(playButton, document.getElementById('play'));
```

Tu devrais avoir une variable appelée `pauseButton`.

```js
assert.isDefined(pauseButton);
```

Tu devrais assigner l'élément `#pause` à la variable `pauseButton`.
