...
Avant de commencer à travailler sur la lecture de la chanson suivante et précédente, tu dois obtenir l'indice de chaque chanson dans la propriété `songs` de `userData`. Pour cela, tu peux utiliser la méthode de tableau `indexOf()`, qui renvoie le premier indice auquel un élément donné peut être trouvé dans le tableau, ou `-1` si l'élément n'est pas présent.
...
Crée une fonction nommée `getCurrentSongIndex` qui renvoie l'indice auquel `userData.currentSong` peut être trouvé dans `userData.songs` et `-1` quand la chanson actuelle est `null`.

# --hints--

Tu devrais avoir une fonction nommée `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Ta fonction `getCurrentSongIndex` devrait renvoyer l'indice auquel `userData.currentSong` peut être trouvé dans `userData.songs`.
...
Ta fonction `getCurrentSongIndex` devrait renvoyer `-1` quand `userData.currentSong` est `null`.
