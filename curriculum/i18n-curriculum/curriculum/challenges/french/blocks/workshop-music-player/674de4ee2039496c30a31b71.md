...
Ajoute un bloc `else` pour gérer le temps de lecture actuel de la chanson. Cela te permet de reprendre la chanson actuelle au point où elle a été mise en pause.

À l'intérieur du bloc `else`, définis la propriété `currentTime` de l'objet `audio` à la valeur stockée dans `userData.songCurrentTime`.

# --hints--

Ta fonction `playSong` devrait définir la propriété `currentTime` de l'objet `audio` à la valeur stockée dans `userData.songCurrentTime` quand une chanson est en cours de lecture.
...
Tu ne devrais pas modifier l'instruction `if` existante ni son contenu.
