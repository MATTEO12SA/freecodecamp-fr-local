...
Maintenant, tu dois ajouter l'attribut `aria-current` à la chanson en cours de lecture.

Vérifie si `songToHighlight` existe. Si c'est le cas, utilise la méthode `setAttribute` sur `songToHighlight` et passe `"aria-current"` et `"true"` comme premier et deuxième arguments.

# --hints--

`highlightCurrentSong` devrait ajouter l'attribut `aria-current` à la chanson en cours de lecture et retirer l'attribut de la chanson précédemment lue.
