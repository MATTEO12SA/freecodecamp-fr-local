...
Crée une fonction nommée `setPlayerDisplay`. À l'intérieur de la fonction, définis le `textContent` de `playingSong` au titre de la chanson actuelle, s'il est défini. Sinon, définis-le à une chaîne vide.

En dessous, définis le `textContent` de `songArtist` à l'artiste de la chanson actuelle, s'il est défini. Sinon, définis-le à une chaîne vide.


# --hints--

Tu devrais avoir une fonction nommée `setPlayerDisplay`.

```js
assert.isFunction(setPlayerDisplay);
```

Ta fonction `setPlayerDisplay` devrait définir le `textContent` de `playingSong` au titre de la chanson actuelle, s'il est défini, ou à une chaîne vide.
...
Ta fonction `setPlayerDisplay` devrait définir le `textContent` de `songArtist` à l'artiste de la chanson actuelle, s'il est défini, ou à une chaîne vide.
