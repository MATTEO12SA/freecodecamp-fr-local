...
À chaque clic sur le bouton de lecture, la première chanson de ta playlist sera toujours lue. Pour corriger cela, à l'intérieur de la fonction fléchée de l'écouteur d'événement, remplace `playSong(0)` par une instruction `if` qui vérifie si `userData.currentSong` est `null`.

À l'intérieur du bloc `if`, appelle la fonction `playSong` avec l'`id` de la première chanson du tableau `userData.songs`.

# --hints--

Tu devrais modifier le rappel de l'écouteur d'événement pour appeler la fonction `playSong` avec l'`id` de la première chanson du tableau `userData.songs` quand `userData.currentSong` est `null`.
