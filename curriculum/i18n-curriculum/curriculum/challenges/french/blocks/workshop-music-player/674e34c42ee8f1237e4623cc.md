...
Ajoute un bloc `else`. À l'intérieur du bloc `else`, appelle la fonction `playSong` avec l'`id` de la chanson en cours de lecture comme argument.

Cela garantit que la chanson en cours de lecture continuera à être lue quand on clique sur le bouton de lecture.

# --hints--

Quand on clique sur le `playButton` et que `userData.currentSong` n'est pas `null`, tu devrais appeler la fonction `playSong` avec l'`id` de la chanson en cours de lecture.
...
Tu devrais toujours appeler la fonction `playSong` avec l'`id` de la première chanson de ta playlist quand on clique sur le `playButton` et que `userData.currentSong` est `null`.
