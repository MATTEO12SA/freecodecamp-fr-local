...
Maintenant que tu sais comment trouver la chanson suivante à lire, crée une fonction nommée `playNextSong`. À l'intérieur de la fonction `playNextSong`, crée une instruction `if` pour vérifier si le `currentSong` de `userData` est strictement égal à `null`. Cela vérifiera s'il n'y a aucune chanson en cours de lecture dans l'objet `userData`.

Si la condition est vraie, appelle la fonction `playSong` avec l'`id` de la première chanson du tableau `userData.songs` comme argument.

# --hints--

Tu devrais avoir une fonction nommée `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Quand le `currentSong` de `userData` est strictement égal à `null`, ta fonction `playNextSong` devrait appeler la fonction `playSong` avec l'`id` de la première chanson du tableau `userData.songs` comme argument.
