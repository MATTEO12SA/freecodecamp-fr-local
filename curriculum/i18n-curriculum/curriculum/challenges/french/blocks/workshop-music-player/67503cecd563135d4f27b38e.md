...
Il est maintenant temps de tester la fonction `playNextSong`.

Ajoute un écouteur d'événement `"click"` à l'élément `nextButton`, puis passe `playNextSong` comme deuxième argument de ton écouteur d'événement. C'est la fonction que l'écouteur d'événement exécutera.

Teste ton application en cliquant d'abord sur le bouton de lecture, puis sur le bouton suivant. Tu devrais voir que tout fonctionne comme prévu.

# --hints--

Tu devrais appeler la méthode `addEventListener()` sur ta variable `nextButton`.

```js
assert.match(__helpers.removeJSComments(code), /nextButton\.addEventListener\(/)
```

Ton écouteur d'événement devrait écouter un événement `"click"`.

```js
assert.match(__helpers.removeJSComments(code), /nextButton\.addEventListener\(\s*('|"|`)click\1/)
```

Tu devrais passer `playNextSong` comme deuxième argument de ta méthode `addEventListener`.
