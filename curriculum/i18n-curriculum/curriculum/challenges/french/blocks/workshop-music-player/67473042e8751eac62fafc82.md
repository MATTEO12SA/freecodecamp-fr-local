...
Il est temps de commencer à implémenter la fonctionnalité pour lire les chansons affichées.

Définis une fonction `playSong` qui prend un seul paramètre représentant l'identifiant unique de la chanson que tu veux lire.

# --hints--

Tu devrais avoir une fonction vide nommée `playSong`.

```js
assert.isFunction(playSong);
```

Ta fonction `playSong` devrait prendre un seul paramètre.
