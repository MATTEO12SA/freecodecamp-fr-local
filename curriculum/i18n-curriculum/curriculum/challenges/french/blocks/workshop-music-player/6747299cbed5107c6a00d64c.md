...
Le reste des chansons a été ajouté au tableau `allSongs` pour toi.

Dans les leçons précédentes, tu as appris l'<dfn>API Web Audio</dfn> et comment l'utiliser pour lire des chansons. Tous les navigateurs modernes prennent en charge l'API Web Audio, qui te permet de générer et de traiter de l'audio dans des applications web.

Crée une variable nommée `audio` et définis-la égale à `new Audio()`. Cela créera un nouvel élément HTML5 `audio`.

# --hints--

Tu devrais avoir une variable nommée `audio`.

```js
assert.isDefined(audio);
```

Tu devrais assigner une nouvelle instance de l'objet `Audio` à la variable nommée `audio`.
