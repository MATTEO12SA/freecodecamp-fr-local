...
Tu dois obtenir l'`id` de la chanson en cours de lecture. Pour cela, tu peux utiliser `userData.currentSong?.id`.

Utilise `getElementById()` pour sélectionner la chanson en cours de lecture : passe un littéral de gabarit pour préfixer l'`id` avec `song-`. Assigne-le à une variable nommée `songToHighlight`.

# --hints--

Tu ne devrais pas modifier la fonction `highlightCurrentSong` existante ni son contenu.
...
Tu devrais utiliser `document.getElementById()` et passer `` `song-${userData.currentSong?.id}` ``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Tu devrais assigner ton `getElementById()` à une variable `songToHighlight`.
