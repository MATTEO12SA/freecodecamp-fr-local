...
Ajoute un troisième objet avec les propriétés et valeurs suivantes :
...
Ton tableau `allSongs` devrait avoir un objet comme troisième valeur.

```js
assert.isObject(allSongs[2]);
```

Le troisième objet de ton tableau `allSongs` devrait avoir une propriété `id` définie au nombre `2`.

```js
assert.equal(allSongs[2].id, 2);
```

Le troisième objet de ton tableau `allSongs` devrait avoir une propriété `title` définie à la chaîne `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Le troisième objet de ton tableau `allSongs` devrait avoir une propriété `artist` définie à la chaîne `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Le troisième objet de ton tableau `allSongs` devrait avoir une propriété `duration` définie à la chaîne `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Le troisième objet de ton tableau `allSongs` devrait avoir une propriété `src` définie à la chaîne `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
