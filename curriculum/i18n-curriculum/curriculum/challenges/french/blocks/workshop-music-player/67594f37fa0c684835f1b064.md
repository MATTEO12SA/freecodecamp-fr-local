...
Quand tu cliques sur une chanson de ta playlist, tu veux lire cette chanson. Pour cela, tu auras besoin d'ajouter un écouteur d'événement pour l'événement `click` sur chaque élément `button` enfant d'un élément `song`.

Complète le rappel de ton `forEach` pour faire cela. À l'intérieur du rappel de `addEventListener`, appelle `playSong(n)`, où `n` est l'id de la chanson que tu peux obtenir depuis l'attribut `id` de `song`.

Note que chaque `song` a un attribut `id` de la forme `song-n` : tu auras besoin d'obtenir la partie `n` et de la convertir en nombre avant de la passer à `playSong()`.


# --hints--

Tu devrais ajouter un écouteur d'événement pour l'événement `click` sur chaque élément `button` enfant d'un élément `.playlist-song` et appeler `playSong` avec l'`id` de la chanson correspondante.
