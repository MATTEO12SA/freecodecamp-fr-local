...
Maintenant, utilise la méthode `querySelectorAll` pour cibler les éléments `.playlist-song` et stocke-les dans une variable nommée `songs`.

Ensuite, appelle la méthode `forEach` sur `songs`. Passe une fonction de rappel qui utilise `song` comme paramètre de la méthode. Laisse les accolades vides pour l'instant.

# --hints--

Tu devrais avoir une variable nommée `songs`.

```js
assert.isDefined(songs);
```

Tu devrais assigner la collection de tous les éléments `.playlist-song` à ta variable `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Tu devrais appeler la méthode `forEach` sur `songs` et passer un rappel qui utilise `song` comme paramètre de la méthode.
