...
Ajoute un deuxième objet avec les clés et valeurs suivantes :
...
Ton tableau `allSongs` devrait avoir deux valeurs.

```js
assert.equal(allSongs.length, 2);
```

Ton tableau `allSongs` devrait avoir un objet comme deuxième valeur.

```js
assert.isObject(allSongs[1]);
```

Le deuxième objet de ton tableau `allSongs` devrait avoir une propriété `id` définie au nombre `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

Le deuxième objet de ton tableau `allSongs` devrait avoir une propriété `title` définie à la chaîne `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Le deuxième objet de ton tableau `allSongs` devrait avoir une propriété `artist` définie à la chaîne `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Le deuxième objet de ton tableau `allSongs` devrait avoir une propriété `duration` définie à la chaîne `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Le deuxième objet de ton tableau `allSongs` devrait avoir une propriété `src` définie à la chaîne `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
