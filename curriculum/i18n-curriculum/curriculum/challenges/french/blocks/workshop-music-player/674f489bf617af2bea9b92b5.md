...
Maintenant, crée une fonction nommée `getNextSong`. Fais-lui renvoyer la chanson qui se trouve à `getCurrentSongIndex()` plus un dans `userData.songs`, ou `undefined` si la chanson actuelle est la dernière de la playlist.

# --hints--

Tu devrais avoir une fonction nommée `getNextSong`.

```js
assert.isFunction(getNextSong);
```

Ta fonction `getNextSong` devrait appeler `getCurrentSongIndex`.
...
Ta fonction `getNextSong` devrait renvoyer la chanson suivante de la chanson actuelle.
...
Ta fonction `getNextSong` devrait renvoyer `undefined` quand la chanson actuelle est la dernière de la playlist.
