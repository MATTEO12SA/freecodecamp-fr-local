...
Si tu joues un peu avec ton lecteur, tu remarqueras quelques bugs. D'abord, quand tu lis une chanson, la mets en pause, puis appuies sur le bouton précédent ou suivant, la chanson ne redémarre pas depuis le début.

Pour commencer à corriger cela, ajoute un deuxième paramètre nommé `start` à ta fonction `playSong`. Donne-lui la valeur par défaut `true`.

# --hints--

Ta fonction `playSong` devrait avoir un deuxième paramètre nommé `start` avec la valeur par défaut `true`.
