...
Avant de lire la chanson, tu dois t'assurer qu'elle démarre depuis le début. Cela peut se faire avec la propriété `currentTime` de l'objet `audio`.

Ajoute une instruction `if` pour vérifier qu'aucune chanson n'est en cours de lecture en vérifiant que `userData.currentSong` est falsy.
À l'intérieur du bloc `if`, définis la propriété `currentTime` de l'objet `audio` à `0`.

# --hints--

Ta fonction `playSong` devrait définir la propriété `currentTime` de l'objet `audio` à `0` quand aucune chanson n'est en cours de lecture.
