...
Ta fonction est maintenant complète. Teste-la à nouveau en affichant `genParentheses(2)` au lieu de `genParentheses(1)`.

# --hints--

Tu dois afficher `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

Tu ne dois pas avoir `console.log(genParentheses(1))` dans ton code.
