...
Crée un nouveau sélecteur `25%` entre tes sélecteurs `0%` et `50%`. Donne à ce nouveau sélecteur la propriété `background-color` définie sur `yellow`.

# --hints--

Tu devrais créer un nouveau sélecteur `25%` dans ta règle `@keyframes cabins`.
...
Ton sélecteur `25%` devrait être entre tes sélecteurs `0%` et `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

Ton sélecteur `25%` devrait avoir une propriété `background-color` définie sur `yellow`.
