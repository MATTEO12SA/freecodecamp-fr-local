...
Entre les sélecteurs `0%` et `100%`, ajoute un sélecteur `50%`. Il s'appliquera au milieu du cycle d'animation. Définis `background-color` sur `purple`.

# --hints--

Tu devrais créer un nouveau sélecteur `50%` dans ta règle `@keyframes cabins`.
...
Ton sélecteur `50%` devrait être entre tes sélecteurs `0%` et `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

Ton sélecteur `50%` devrait avoir une propriété `background-color` définie sur `purple`.
