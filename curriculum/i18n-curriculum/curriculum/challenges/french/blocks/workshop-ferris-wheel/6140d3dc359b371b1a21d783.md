...
Il est temps de positionner les cabines autour de la roue. Sélectionne le premier élément `.cabin`. Définis la propriété `right` sur `-8.5%` et la propriété `top` sur `50%`.

# --hints--

Tu devrais avoir un sélecteur `.cabin:nth-of-type(1)`.
...
Ton sélecteur `.cabin:nth-of-type(1)` devrait avoir une propriété `right` définie sur `-8.5%`.
...
Ton sélecteur `.cabin:nth-of-type(1)` devrait avoir une propriété `top` définie sur `50%`.
