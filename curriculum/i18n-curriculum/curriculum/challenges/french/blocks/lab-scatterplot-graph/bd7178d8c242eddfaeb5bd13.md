...
title: Construire un nuage de points
challengeType: 25
...
Dans ce projet, tu utiliseras les données de `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` pour construire un nuage de points.

Les tests exigent que les axes soient générés avec la propriété d'axes de D3, qui génère automatiquement des ticks le long de l'axe. Ces ticks sont nécessaires pour réussir les tests D3, car leurs positions servent à déterminer l'alignement des éléments du graphique.

**Objectif :** Remplis les user stories ci-dessous et fais passer tous les tests pour terminer le lab.

**User Stories :**

1. Ton graphique devrait avoir un titre avec un `id="title"` correspondant.
1. Ton graphique devrait avoir un élément `g` pour l'axe des x avec un `id="x-axis"` correspondant.
1. Ton graphique devrait avoir un élément `g` pour l'axe des y avec un `id="y-axis"` correspondant.
1. Les deux axes devraient contenir plusieurs libellés de tick, chacun avec une `class="tick"` correspondante.
1. Ton graphique devrait avoir un élément `circle` pour chaque point de données avec une `class="dot"` correspondante affichant les données.
1. Chaque `.dot` devrait avoir les attributs `data-xvalue` et `data-yvalue` contenant les valeurs `year` et `time`. Pour `data-xvalue`, des entiers (années complètes) ou des objets Date sont acceptables pour l'évaluation des tests. Pour `data-yvalue` (minutes), utilise des objets Date.
1. Les attributs `data-xvalue` des éléments `.dot` devraient correspondre à l'ordre des données fournies.
1. Les attributs `data-yvalue` des éléments `.dot` devraient correspondre à l'ordre des données fournies.
1. Chaque élément `.dot` devrait s'aligner avec la valeur correspondante sur l'axe des x.
1. Chaque élément `.dot` devrait s'aligner avec la valeur correspondante sur l'axe des y.
1. Il devrait y avoir plusieurs libellés de tick sur l'axe des y au format horaire `%M:%S`.
1. Il devrait y avoir plusieurs libellés de tick sur l'axe des x qui affichent l'année.
1. La plage des libellés de l'axe des x devrait être dans la plage des données réelles de l'axe des x.
1. La plage des libellés de l'axe des y devrait être dans la plage des données réelles de l'axe des y.
1. Il devrait y avoir une légende contenant du texte descriptif avec `id="legend"`.
1. Tu devrais pouvoir survoler une zone avec la souris et voir une infobulle avec un `id="tooltip"` correspondant qui affiche plus d'informations sur la zone.
1. Ton infobulle devrait avoir un attribut `data-year` qui correspond au `data-xvalue` de la zone active.
...
Le graphique devrait avoir un élément avec l'`id` `title` pour contenir le titre.

```js
assert.exists(document.getElementById('title'));
```

Le graphique devrait avoir un `x-axis` avec un `id="x-axis"` correspondant.
...
Le graphique devrait avoir un `y-axis` avec un `id="y-axis"` correspondant.
...
L'axe des x devrait contenir plusieurs libellés de tick, chacun avec la `class="tick"` correspondante.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

L'axe des y devrait contenir plusieurs libellés de tick, chacun avec la `class="tick"` correspondante.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Ton graphique devrait avoir des points, chacun avec une classe `dot`, qui représentent les données tracées.

```js
assert.isNotEmpty(document.querySelectorAll('circle.dot'));
```

Chaque point devrait avoir les propriétés `data-xvalue` et `data-yvalue` contenant leurs valeurs x et y correspondantes.
...
Les `data-xvalue` et `data-yvalue` de chaque point devraient être dans la plage des données réelles et au bon format. Pour `data-xvalue`, des entiers (années complètes) ou des objets Date sont acceptables pour l'évaluation des tests. Pour `data-yvalue` (minutes), utilise des objets Date.
...
Le `data-xvalue` et le point correspondant devraient s'aligner avec le point/la valeur correspondant(e) sur l'axe des x.
...
Le `data-yvalue` et le point correspondant devraient s'aligner avec le point/la valeur correspondant(e) sur l'axe des y.
...
Tu peux voir plusieurs libellés de tick sur l'axe des y au format horaire `%M:%S`.
...
Tu peux voir plusieurs libellés de tick sur l'axe des x qui affichent l'année.
...
La plage des libellés de l'axe des x est dans la plage des données réelles de l'axe des x.
...
La plage des libellés de l'axe des y est dans la plage des données réelles de l'axe des y.
...
Tu peux voir une légende contenant du texte descriptif avec `id="legend"`.
...
Lorsque tu survoles une zone, ton nuage de points devrait avoir une infobulle avec un `id="tooltip"` correspondant qui affiche plus d'informations sur la zone.
...
L'infobulle devrait avoir un attribut `data-year` qui correspond au `data-xvalue` de la zone active.
