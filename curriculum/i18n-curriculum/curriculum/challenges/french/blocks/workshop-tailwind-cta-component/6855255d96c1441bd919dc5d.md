...
En passant à l’élément `span` avec le texte `Soundflow`, le texte doit être en majuscules.

Voici un exemple montrant comment utiliser la classe utilitaire `uppercase` :

```html
<p class="uppercase">Quincy Larson</p>
```

Ajoute la classe `uppercase` à l’élément `span`.

# --hints--

Ton élément `span` doit avoir la classe utilitaire `uppercase`.
