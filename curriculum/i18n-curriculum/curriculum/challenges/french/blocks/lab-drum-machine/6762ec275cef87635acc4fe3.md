...
title: Construire une drum machine
challengeType: 25
...
1. Tu dois avoir un élément `div` avec un `id` de `drum-machine` qui contient tous les autres éléments.

2. À l'intérieur de l'élément `#drum-machine`, tu dois avoir un autre `div` avec un `id` de `pad-bank`.

3. À l'intérieur de l'élément `#drum-machine`, tu dois avoir un élément `p` avec un `id` de `display`.

4. À l'intérieur de ton élément `#pad-bank`, tu dois avoir neuf éléments de pad de batterie cliquables, chacun avec une classe `drum-pad`, un `id` unique qui décrit le clip audio que le pad est configuré pour déclencher, et un texte interne qui correspond à l'une des touches suivantes du clavier : `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`. Les pads DOIVENT être dans cet ordre.

5. Chaque `.drum-pad` doit avoir un élément `audio` avec une classe `clip`, un attribut `src` qui pointe vers un clip audio, et un `id` correspondant au texte interne de son élément parent `.drum-pad` (par ex. `id="Q"`, `id="W"`, `id="E"`, etc.).

6. Quand tu cliques sur un élément `.drum-pad`, le clip audio contenu dans son élément audio enfant doit être déclenché.

7. Quand tu appuies sur la touche de déclenchement associée à chaque `.drum-pad`, le clip audio contenu dans son élément audio enfant doit être déclenché (par ex. appuyer sur la touche `Q` doit déclencher le pad qui contient la chaîne `Q`, appuyer sur la touche `W` doit déclencher le pad qui contient la chaîne `W`, etc.).

8. Quand un `.drum-pad` est déclenché, tu dois afficher une chaîne décrivant le clip audio associé comme texte interne de l'élément `#display` (chaque chaîne doit être unique).

Tu peux trouver des échantillons audio pour ta drum machine à `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`, où `<fileName>` est le suivant :
...
Tu dois avoir un élément `div` avec un `id` de `drum-machine` qui contient tous les autres éléments.

```js
assert.isNotNull(document.querySelector('div#drum-machine'));
```

À l'intérieur de l'élément `#drum-machine`, tu dois avoir un autre `div` avec un `id` de `pad-bank`.

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

À l'intérieur de l'élément `#drum-machine`, tu dois avoir un élément `p` avec un `id` de `display`.

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

À l'intérieur de ton élément `#pad-bank`, tu dois avoir neuf éléments de pad de batterie cliquables, chacun avec une classe `drum-pad`.

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

Chaque `.drum-pad` doit être un élément `button`.
...
Chaque `.drum-pad` doit avoir l'une des lettres suivantes comme `innerText`, dans l'ordre : `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`.
...
Chaque `.drum-pad` doit avoir un élément `audio` avec une classe `clip`, un attribut `src` qui pointe vers un clip audio, et un `id` correspondant au texte interne de son élément parent `.drum-pad` (par ex. `id="Q"`, `id="W"`, `id="E"`, etc.).
...
Quand tu cliques sur un élément `.drum-pad`, le clip audio contenu dans son élément audio enfant doit être déclenché.
...
Quand tu appuies sur l'une des touches Q, W, E, A, S, D, Z, X, C de ton clavier, l'élément `audio` correspondant doit jouer le son correspondant.
...
Quand un `.drum-pad` est déclenché, tu dois afficher une chaîne décrivant le clip audio associé comme texte interne de l'élément `#display` (chaque chaîne doit être unique).
