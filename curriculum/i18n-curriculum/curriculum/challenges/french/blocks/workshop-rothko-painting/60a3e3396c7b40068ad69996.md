...
Fais pivoter l’élément `.two` dans le sens horaire de 0,4 degré.

# --hints--

Tu devrais définir la propriété `transform` sur `rotate(0.4deg)`.
...
Ton élément `.two` devrait avoir la propriété `transform` avec la valeur `rotate(0.4deg)`.
