...
Définis le `background-color` de `.one` sur `#efb762`.

# --hints--

Tu devrais définir la propriété `background-color` sur `#efb762`.
...
Ton élément `.one` devrait avoir la propriété `background-color` avec la valeur `#efb762`.
