...
Écris une nouvelle règle qui cible `.one` et définis sa `width` sur 425 pixels.

# --hints--

Tu devrais avoir un sélecteur `.one`.
...
Tu devrais définir la propriété `width` sur `425px`.
...
Ton élément `.one` devrait avoir la propriété `width` avec la valeur `425px`.
