...
Centre l’élément `.three` sur la toile en définissant sa `margin` sur `auto`.

# --hints--

Tu devrais définir la propriété `margin` sur `auto`.
...
Ton élément `.three` devrait avoir la propriété `margin` avec la valeur `auto`.
