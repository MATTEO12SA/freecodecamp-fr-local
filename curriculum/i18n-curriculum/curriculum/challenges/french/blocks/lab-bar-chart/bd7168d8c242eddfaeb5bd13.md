...
Dans ce projet, tu utiliseras les données de `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` pour construire un diagramme en barres.
...
1. Ton graphique devrait avoir un élément `rect` pour chaque point de données avec une `class="bar"` correspondante affichant les données.
1. Chaque `.bar` devrait avoir les attributs `data-date` et `data-gdp` contenant les valeurs `date` et `GDP`.
1. Les attributs `data-date` des éléments `.bar` devraient correspondre à l'ordre des données fournies.
1. Les attributs `data-gdp` des éléments `.bar` devraient correspondre à l'ordre des données fournies.
1. La hauteur de chaque élément `.bar` devrait représenter correctement le `GDP` correspondant des données.
1. L'attribut `data-date` et l'élément `.bar` correspondant devraient s'aligner avec la valeur correspondante sur l'axe des x.
1. L'attribut `data-gdp` et l'élément `.bar` correspondant devraient s'aligner avec la valeur correspondante sur l'axe des y.
...
1. Ton infobulle devrait avoir un attribut `data-date` qui correspond au `data-date` de la zone active.
...
Le graphique devrait avoir un élément `g` pour l'axe des x avec un id `x-axis` correspondant.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

Le graphique devrait avoir un élément `g` pour l'axe des y avec un id `y-axis` correspondant.
...
Ton graphique devrait avoir un élément `rect` pour chaque point de données avec une `class="bar"` correspondante affichant les données.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

Chaque barre devrait avoir les attributs `data-date` et `data-gdp` contenant les valeurs de date et de GDP.
...
Les attributs `data-date` des éléments barre devraient correspondre à l'ordre des données fournies.
...
Les attributs `data-gdp` des éléments barre devraient correspondre à l'ordre des données fournies.
...
La hauteur de chaque élément barre devrait représenter correctement le GDP correspondant des données
...
L'attribut `data-date` et l'élément barre correspondant devraient s'aligner avec la valeur correspondante sur l'axe des x. Tu peux utiliser `d3.scaleTime()` pour créer une échelle de temps pour l'axe des x.
...
L'attribut `data-gdp` et l'élément barre correspondant devraient s'aligner avec la valeur correspondante sur l'axe des y. Tu peux utiliser `d3.scaleLinear()` pour créer une échelle linéaire pour l'axe des y.
...
Lorsque tu survoles une zone, ton diagramme en barres devrait avoir une infobulle avec un `id="tooltip"` correspondant qui affiche plus d'informations sur la zone.
...
L'infobulle devrait avoir un attribut `data-date` qui correspond au `data-date` de la zone active.
