...
Maintenant que tu as enregistré la tâche dans le tableau `taskData`, tu dois afficher la tâche sur la page en le parcourant.

Utilise `forEach()` sur `taskData`, puis déstructure `id`, `title`, `date`, `description` comme paramètres. Ne renvoie encore rien.

# --hints--

Tu dois utiliser `forEach()` sur `taskData`.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

Tu dois utiliser `{id, title, date, description}` comme paramètres déstructurés de la fonction de rappel `forEach()`.
