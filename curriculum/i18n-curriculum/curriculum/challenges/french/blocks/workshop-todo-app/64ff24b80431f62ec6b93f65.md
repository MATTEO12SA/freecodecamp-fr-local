...
Supprime le tableau `myTaskArr` et tout le code lié à `localStorage` car tu n'en as plus besoin.

# --hints--

Tu dois supprimer `myTaskArr` et tout le code lié à `localStorage` que tu viens d'apprendre.
...
Tu dois supprimer toute référence restante à `myTaskArr` partout dans le code.

```js
assert.notMatch(code, /myTaskArr/);
```

Tu dois supprimer toute référence à `localStorage.getItem` pour l'élément `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Tu dois supprimer toute référence restante à `getTaskArrObj` partout dans le code.
