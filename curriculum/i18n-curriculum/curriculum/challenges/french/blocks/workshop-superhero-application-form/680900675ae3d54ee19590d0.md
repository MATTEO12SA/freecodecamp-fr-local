...
Pour corriger l'erreur, crée une fonction fléchée `handlePowersChange` avec un paramètre `e`.

Dans la fonction, destructure `value` et `checked` depuis `e.target` pour obtenir la valeur de la case et si elle est cochée ou non.

# --hints--

Tu dois créer une fonction `handlePowersChange` avec un paramètre `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Tu dois destructurer `value` et `checked` depuis `e.target`.
