...
Ensuite, dis au formulaire comment se soumettre. Ajoute un attribut `method` avec une valeur de `post` et un attribut `action` avec une valeur de `https://superhero-application-form.freecodecamp.org` à l'élément form.

Maintenant, soumettre le formulaire l'enverra à cette URL. Tu le feras à l'étape suivante.

# --hints--

Ton élément `form` doit avoir un attribut `method` défini à `post`.
...
Ton élément `form` doit avoir un attribut `action` défini à `https://superhero-application-form.freecodecamp.org`.
