...
Sous les `value` et `checked` destructurés, mets à jour la liste des pouvoirs cochés.

Appelle la fonction `setPowers`, qui prend un argument tableau. Utilise un opérateur ternaire pour vérifier si `checked` est vrai. Si c'est le cas, étends le tableau `powers` existant dans un nouveau tableau et ajoute `value`. Sinon, filtre `value` hors de `powers` avec `powers.filter(p => p !== value)`.

# --hints--

Tu dois utiliser un opérateur ternaire pour vérifier si `checked` est vrai. Si c'est le cas, étends les `powers` et `value` existants dans un tableau. Sinon, filtre `value` hors de `powers`.
