...
Le prochain `input` est une case à cocher pour marquer tous les pouvoirs de super-héros qui s'appliquent à l'utilisateur.

Crée un élément `label` avec un `className` de `section` et `column`, et le texte `List your powers (select all that apply):`.

# --hints--

Tu dois créer un élément `label` avec un `className` de `section` et `column`.
...
Ton élément `label` doit avoir le texte `List your powers (select all that apply):`.
