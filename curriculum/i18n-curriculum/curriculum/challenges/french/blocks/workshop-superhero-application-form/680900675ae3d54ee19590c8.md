...
Ensuite, crée un `label` avec le `className` de `section` et `column`, et le texte `How did you get your powers?`. Dans le `label`, crée un élément `select`.
...
Ton élément `label` doit avoir le texte `How did you get your powers?`.
...
Tu dois créer un élément `select` à l'intérieur de ton élément `label`.
