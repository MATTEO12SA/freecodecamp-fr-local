...
Crée un élément `button` avec le `className` de `submit-btn` et un `type` de `submit`. Donne au bouton le texte `Join the League`.

Après cela, essaie de soumettre le formulaire et vois ce qui se passe.
...
Ton élément button doit avoir un attribut `className` défini à `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Ton élément `button` doit avoir `Join the League` comme texte.
