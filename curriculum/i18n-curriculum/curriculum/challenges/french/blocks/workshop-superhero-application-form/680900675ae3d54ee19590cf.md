...
Pour marquer les cases, ajoute un attribut `checked` à l'`input` checkbox. Utilise la méthode `includes` pour vérifier si le `power` actuel est dans le tableau `powers`.

Le `onChange` sera une fonction séparée, définis-le à `handlePowersChange` pour l'instant. Cela mènera à une erreur que tu corrigeras à l'étape suivante.

# --hints--

Tu dois définir l'état `checked` de la case pour refléter si la valeur `power` actuelle existe dans le tableau powers.

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

Tu dois définir l'attribut `onChange` de la case à `{handlePowersChange}`.
