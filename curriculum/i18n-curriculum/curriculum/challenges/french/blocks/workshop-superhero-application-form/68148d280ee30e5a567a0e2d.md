...
Retire le texte `Hello` à l'intérieur du `label`, puis crée un `input` de type `checkbox` et une `value` de `power`.

Enfin, affiche les valeurs du tableau à côté de chaque case en créant un élément `span` juste avant la fermeture du `label` avec un contenu texte de `{power}`.

# --hints--

Tu dois retirer le texte `Hello` de l'élément `label`.

```js
assert.notMatch(code, /hello/i)
```

Tu dois créer un `input` avec son `type` défini à `checkbox` et une `value` définie à `power` comme premier enfant du label.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Tu dois créer un élément `span` avec `{power}` pour son contenu texte comme dernier enfant du label.
