...
Ce serait bien d'afficher dynamiquement les mots `Show Message` ou `Hide Message` selon l'état `isVisible`.

Plus tôt, tu as revu comment travailler avec le rendu conditionnel en ligne. Tu vas appliquer une logique similaire au texte du bouton.

Voici un exemple qui utilise l'opérateur ternaire pour afficher conditionnellement les mots `Hide` et `Show` :

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

Pour la dernière étape de l'atelier, utilise l'opérateur ternaire pour afficher conditionnellement les mots `Hide` ou `Show` pour le bouton du message. Place la condition avant le texte `Message` pour qu'il affiche `Show Message` ou `Hide Message`.

Et avec ce dernier changement, ton atelier est terminé ! Teste ton bouton pour le voir en action.

# --hints--

Quand `isVisible` est `false`, le texte du bouton devrait dire `Show Message`.
...
Quand `isVisible` est `true`, le texte du bouton devrait dire `Hide Message`.
