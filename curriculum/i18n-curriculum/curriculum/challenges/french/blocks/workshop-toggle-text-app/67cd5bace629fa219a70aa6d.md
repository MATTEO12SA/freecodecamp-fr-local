...
Il est maintenant temps d'utiliser ta fonction `handleToggleVisibility`.

Dans les leçons précédentes, tu as appris le Synthetic Event System de React et comment travailler avec des attributs comme `onClick` et `onSubmit`.

Souviens-toi que tu peux passer une référence de fonction à un attribut `onClick` comme ceci :
...
Mets à jour ton élément `button` existant pour ajouter un attribut `onClick` et passer une référence à la fonction `handleToggleVisibility`.

Maintenant, quand tu cliques sur le bouton, tu verras la visibilité du texte du paragraphe basculer entre masqué et affiché sur la page.

# --hints--

Quand le `toggle-button` est cliqué, le message `"I love freeCodeCamp!"` devrait apparaître à l'écran.
