...
Une façon d'afficher le texte du paragraphe est de changer la valeur initiale du hook `useState`.

Change la valeur actuelle de `useState` de `false` à `true` et tu devrais voir que le texte du paragraphe s'affiche maintenant à l'écran.

# --hints--

Tu devrais changer la valeur initiale de `useState` en `true`.
