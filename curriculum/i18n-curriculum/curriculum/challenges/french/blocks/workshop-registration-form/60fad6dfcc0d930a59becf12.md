...
Le HTML du formulaire d'inscription est terminé. Maintenant, tu peux l'améliorer un peu.

Commence par changer la police en `Tahoma`, et la taille de police en `16px` dans le `body`.

# --hints--

Tu dois utiliser la propriété `font-family` pour changer la police.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily);
```

Tu dois définir la propriété `font-family` sur `Tahoma`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'Tahoma');
```

Tu dois définir la propriété `font-size` sur `16px`.
