...
Ce serait plus agréable si le texte des `label` apparaissait au-dessus des éléments du formulaire.

Sélectionne tous les éléments `input`, `textarea` et `select`, et fais-leur occuper toute la largeur de leurs éléments parents.

Ajoute aussi `10px` de `margin` en haut des éléments sélectionnés. Définis les autres marges sur `0`.

# --hints--

Tu dois utiliser un sélecteur d'éléments séparés par des virgules pour sélectionner les éléments `input`, `textarea` et `select`.
...
Tu dois définir la propriété `width` sur `100%`.
...
Tu dois définir la propriété `margin-top` sur `10px`.
...
Tu dois définir la propriété `margin-bottom` sur `0`.
...
Tu dois définir la propriété `margin-left` sur `0`.
...
Tu dois définir la propriété `margin-right` sur `0`.
