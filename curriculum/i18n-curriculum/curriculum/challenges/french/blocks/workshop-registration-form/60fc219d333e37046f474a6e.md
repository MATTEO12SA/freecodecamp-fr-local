...
Sélectionne seulement les éléments `.inline`, et donne-leur une `width` de `unset`. Cela supprimera la règle précédente qui définissait tous les éléments `input` sur `width: 100%`.

# --hints--

Tu dois utiliser le sélecteur `.inline`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

Tu dois donner aux éléments `.inline` une `width` de `unset`.
