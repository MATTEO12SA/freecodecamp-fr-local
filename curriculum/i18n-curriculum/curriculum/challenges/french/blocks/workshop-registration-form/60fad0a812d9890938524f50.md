...
Pour donner aux apprenants une idée de ce qu'ils peuvent mettre dans leur bio, on utilise l'attribut `placeholder`. Le `placeholder` accepte une valeur textuelle, qui s'affiche jusqu'à ce que l'utilisateur commence à saisir du texte.

Donne au `textarea` un `placeholder` de `I like coding on the beach...`.

# --hints--

Tu dois donner au `textarea` un attribut `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Tu dois donner au `placeholder` une valeur de `I like coding on the beach...`.
