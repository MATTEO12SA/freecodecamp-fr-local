...
Pour styliser le bouton d'envoi, tu peux utiliser un sélecteur d'_attribut_, qui sélectionne un élément selon la valeur d'attribut donnée. Voici un exemple :

```css
input[name="password"]
```

Ce qui précède sélectionne les éléments `input` dont la valeur de l'attribut `name` est `password`.

Maintenant, utilise le sélecteur d'attribut pour styliser le bouton d'envoi avec un `display` de `block` et une `width` de `60%`.

# --hints--

Tu dois utiliser un sélecteur d'attribut `input[type="submit"]` pour styliser le bouton d'envoi.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

Tu dois donner au bouton d'envoi un `display` de `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

Tu dois donner au bouton d'envoi une `width` de `60%`.
