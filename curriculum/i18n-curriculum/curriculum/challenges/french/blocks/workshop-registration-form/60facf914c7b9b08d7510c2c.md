...
Le `textarea` semble trop petit. Pour lui donner une taille initiale, tu peux ajouter les attributs `rows` et `cols`.

Ajoute une taille initiale de `3` lignes et `30` colonnes.

# --hints--

Tu dois donner au `textarea` un attribut `rows` avec la valeur `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Tu dois donner au `textarea` un attribut `cols` avec la valeur `30`.
