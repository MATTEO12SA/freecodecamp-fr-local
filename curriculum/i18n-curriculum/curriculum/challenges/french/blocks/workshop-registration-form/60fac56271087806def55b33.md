...
Imbrique l'élément `select` (avec ses éléments `option`) dans un élément `label` avec le texte `How did you hear about us?`. Le texte doit venir avant l'élément `select`.

# --hints--

Tu dois imbriquer uniquement l'élément `select` dans un élément `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

Tu dois donner à l'élément `label` le texte `How did you hear about us?`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(3)')?.innerText?.trim(), 'How did you hear about us?');
```

Tu dois placer le texte avant l'élément `select`.
...
Tu dois avoir 5 éléments `option` dans l'élément `select`.
