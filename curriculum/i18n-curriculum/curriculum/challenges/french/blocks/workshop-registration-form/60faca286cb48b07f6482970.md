...
Soumettre le formulaire avec une option sélectionnée n'enverrait pas une valeur utile au serveur. Pour cette raison, chaque élément `option` a besoin d'un attribut `value`. Sans cet attribut, le contenu textuel de l'`option` sera soumis au serveur.

Donne à la première `option` une `value` de `""`, et aux éléments `option` suivants des attributs `value` allant de `1` à `4`.

# --hints--

Tu dois donner à la première `option` une `value` de `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Tu dois donner à la deuxième `option` une `value` de `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Tu dois donner à la troisième `option` une `value` de `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Tu dois donner à la quatrième `option` une `value` de `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Tu dois donner à la cinquième `option` une `value` de `4`.
