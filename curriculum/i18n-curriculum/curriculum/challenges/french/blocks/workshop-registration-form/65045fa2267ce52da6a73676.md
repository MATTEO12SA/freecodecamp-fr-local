...
Suis les bonnes pratiques d'accessibilité en reliant les éléments `input` et les éléments `label` dans le deuxième `fieldset`.

Utilise `personal-account` et `business-account` comme valeurs pour les attributs `id` respectifs.

# --hints--

Le premier élément `input` doit avoir un `id` de `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[0]?.matches('#personal-account'))
```

Le deuxième élément `input` doit avoir un `id` de `business-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[1]?.matches('#business-account'))
```

Le premier élément `label` doit avoir un attribut `for` avec une valeur de `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="personal-account"]'))
```

Le deuxième élément `label` doit avoir un attribut `for` avec une valeur de `business-account`.
