...
Ajoute de l'espace entre les éléments `.inline` et le texte du `label`, en donnant une `margin` droite de `0.5em`. Définis aussi toutes les autres marges sur `0`.

# --hints--

Tu dois donner aux éléments `.inline` une `margin-top` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

Tu dois donner aux éléments `.inline` une `margin-right` de `0.5em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

Tu dois donner aux éléments `.inline` une `margin-bottom` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

Tu dois donner aux éléments `.inline` une `margin-left` de `0`.
