...
Centre les éléments `h1` et `p` en leur donnant une `margin` de `1em auto`. Ensuite, aligne aussi leur texte au `center`.

# --hints--

Tu dois utiliser un sélecteur d'éléments séparés par des virgules pour styliser les éléments `h1` et `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

Tu dois utiliser une `margin` de `1em auto` pour centrer les éléments `h1` et `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

Tu dois utiliser un `text-align` de `center` pour centrer le texte de `h1` et `p`.
