...
Pour le deuxième `fieldset`, tu veux que les `input` et le texte des `label` apparaissent sur la même ligne.

Commence par donner aux éléments `input` du deuxième `fieldset` une classe de `inline`.

# --hints--

Tu dois donner au premier `input` une classe de `inline`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Tu dois donner au deuxième `input` une classe de `inline`.
