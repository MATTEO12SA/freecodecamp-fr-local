...
Pendant le développement, il est utile de voir les bordures par défaut des `fieldset`. Cependant, elles donnent l'impression que le contenu est trop séparé.

Supprime la `border`, et ajoute `2rem` de padding uniquement en haut et en bas de chaque `fieldset`. Assure-toi de supprimer le `padding` à gauche et à droite.

# --hints--

Tu peux utiliser soit une valeur de `none`, soit `0`, pour supprimer la `border`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

Tu dois ajouter un `padding` de `2rem` en haut et en bas de chaque `fieldset`.
...
Tu dois supprimer le `padding` à gauche et à droite de chaque `fieldset`.
