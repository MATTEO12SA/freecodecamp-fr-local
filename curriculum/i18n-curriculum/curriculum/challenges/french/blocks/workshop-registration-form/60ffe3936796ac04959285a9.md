...
Actuellement, si tu saisis du texte dans les éléments `input` ou `textarea`, tu ne pourras pas le voir. De plus, leur hauteur est trop petite pour être facile à utiliser.

Corrige cela en définissant la `color` sur `#ffffff`, et leur `min-height` sur `2em`.

# --hints--

Tu dois définir la `color` sur `#ffffff`.
...
Tu dois définir la `min-height` sur `2em`.
