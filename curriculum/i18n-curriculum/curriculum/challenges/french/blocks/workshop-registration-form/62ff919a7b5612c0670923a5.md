...
Relie les éléments de formulaire concernés avec leurs éléments `label`.

Utilise `profile-picture`, `age`, `referrer` et `bio` comme valeurs pour les attributs `id` respectifs.

# --hints--

Le premier élément `input` doit avoir un `id` de `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

Le deuxième élément `input` doit avoir un `id` de `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

L'élément `select` doit avoir un `id` de `referrer`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

L'élément `textarea` doit avoir un `id` de `bio`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) textarea')?.matches('#bio'))
```

Le premier élément `label` doit avoir un attribut `for` avec une valeur de `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

Le deuxième élément `label` doit avoir un attribut `for` avec une valeur de `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

Le troisième élément `label` doit avoir un attribut `for` avec une valeur de `referrer`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Le quatrième élément `label` doit avoir un attribut `for` avec une valeur de `bio`.
