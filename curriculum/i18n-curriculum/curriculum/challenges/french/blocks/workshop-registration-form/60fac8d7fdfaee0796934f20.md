...
Les options de la liste déroulante sont actuellement vides. Pour leur donner du contenu, ajoute le texte suivant à chaque élément `option` suivant :
...
Tu dois donner au quatrième élément `option` le texte `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Tu dois donner au cinquième élément `option` le texte `Other`.
