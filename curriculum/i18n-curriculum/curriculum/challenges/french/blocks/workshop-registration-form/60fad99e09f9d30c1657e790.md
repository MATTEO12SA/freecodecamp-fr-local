...
Centre l'élément `form` en lui donnant une `margin` de `0 auto`. Ensuite, fixe sa taille à une largeur maximale de `500px` et une largeur minimale de `300px`. Entre ces deux limites, autorise-le à avoir une `width` de `60vw`.

# --hints--

Tu dois utiliser un sélecteur `form` pour styliser l'élément `form`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

Tu dois donner au `form` une `margin` de `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

Tu dois donner au `form` une `max-width` de `500px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

Tu dois donner au `form` une `min-width` de `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

Tu dois donner au `form` une `width` de `60vw`.
