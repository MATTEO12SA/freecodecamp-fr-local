...
Avec un `display` de `block`, le bouton d'envoi est collé au bord gauche de son parent.

Utilise la même technique que celle utilisée pour centrer le `form` afin de centrer le bouton d'envoi.

# --hints--

Tu dois donner au bouton d'envoi une `margin` de `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.margin, '0px auto');
```

Tu ne dois pas donner au bouton d'envoi de `min-width` ni de `max-width`.
