...
Passons au dernier `fieldset`. Et si tu voulais permettre à un utilisateur de téléverser une photo de profil ?

Eh bien, le `type` d'`input` `file` permet justement cela. Ajoute un `label` avec le texte `Upload a profile picture: `, et imbrique un `input` acceptant le téléversement d'un fichier.

# --hints--

Tu dois ajouter un `label` avec le texte `Upload a profile picture: `.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label')?.innerText, /Upload a profile picture:/i);
```

Tu dois imbriquer un élément `input` dans l'élément `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label > input'));
```

Tu dois donner à l'élément `input` un `type` de `file`.
