...
La bordure du dernier élément `fieldset` semble un peu déplacée.

Dans les leçons précédentes, tu as appris à travailler avec la pseudo-classe CSS `last-of-type` comme ceci :

```css
p:last-of-type { }
```

Cela sélectionnera le dernier élément `p`.

Crée un nouveau sélecteur qui cible le dernier élément `fieldset` et définis sa `border-bottom` sur `none`.

# --hints--

Tu peux utiliser la pseudo-classe `:last-of-type` pour sélectionner seulement le dernier élément d'un certain type.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

Ton `fieldset:last-of-type` doit avoir `border-bottom` défini sur `none`.
