...
Tu veux que l'élément `select` garde un arrière-plan blanc, mais pour l'instant il ne reçoit pas la même `min-height` que les éléments `input` et `textarea`.

Déplace la propriété et la valeur `min-height` pour que les trois types d'éléments aient la même valeur de `min-height`, tout en laissant l'élément `select` avec un arrière-plan blanc.

# --hints--

Tu dois déplacer la propriété et la valeur `min-height` vers le sélecteur `input, textarea, select`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight);
```

Tu dois donner au sélecteur `input, textarea, select` une `min-height` de `2em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight, '2em');
```

Tu dois supprimer la déclaration `min-height` du sélecteur `input, textarea`.
