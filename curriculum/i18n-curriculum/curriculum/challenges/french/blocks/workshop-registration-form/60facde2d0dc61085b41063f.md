...
L'élément `textarea` agit comme un élément `input` de type `text`, mais il a l'avantage supplémentaire de pouvoir recevoir du texte sur plusieurs lignes, ainsi qu'un nombre initial de lignes et de colonnes de texte.

Les utilisateurs pourront s'inscrire avec une bio. Ajoute un `label` avec le texte `Provide a bio:` à la fin du `fieldset`. Ajoute un élément `textarea` dans l'élément `label`. Note que le `textarea` nécessite une balise de fermeture.

# --hints--

Tu dois ajouter un élément `label` à la fin du troisième `fieldset`, après les éléments `label` existants.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)'));
```

Tu dois donner au `label` un texte de `Provide a bio:`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)')?.innerText, /Provide a bio/);
```

Tu dois imbriquer un élément `textarea` dans le `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea'));
```

Tu dois donner à l'élément `textarea` des balises d'ouverture et de fermeture.
