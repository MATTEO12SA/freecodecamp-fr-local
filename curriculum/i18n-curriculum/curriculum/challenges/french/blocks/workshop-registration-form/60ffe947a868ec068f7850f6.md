...
Pour rendre le bouton d'envoi plus distinct, donne-lui un `background-color` de `#3b3b4f`, et une `border-color` de `white`.

# --hints--

Tu dois donner au bouton d'envoi un `background-color` de `#3b3b4f`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

Tu dois donner au bouton d'envoi une `border-color` de `white`.
