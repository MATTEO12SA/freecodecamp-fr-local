...
Pour terminer, définis le texte `Calendar` pour la première balise ancre, `Chat` pour la deuxième, et `Email` pour la troisième.

Avec ça, ton composant mega navbar est terminé !

# --hints--

Tu dois définir le texte `Calendar` pour le premier élément ancre.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Tu dois définir le texte `Chat` pour le deuxième élément ancre.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Tu dois définir le texte `Email` pour le troisième élément ancre.
