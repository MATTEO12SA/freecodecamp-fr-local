...
Crée un élément `ul` à l'intérieur de l'élément `nav`. À l'intérieur de ce `ul`, crée trois éléments de liste.

# --hints--

Tu dois créer un élément `ul` à l'intérieur de ton élément `nav`.

```js
assert.exists(document.querySelector('nav ul'));
```

Tu dois créer trois éléments `li` à l'intérieur de ton élément `ul`.
