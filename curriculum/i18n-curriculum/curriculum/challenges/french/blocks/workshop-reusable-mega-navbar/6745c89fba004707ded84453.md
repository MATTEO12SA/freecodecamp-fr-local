...
Crée un élément ancre à l'intérieur du premier élément `li`. Donne à l'élément `a` un attribut `href` de `#` et le texte `Dashboard`.

# --hints--

Tu dois créer un élément `a` à l'intérieur du premier élément de liste.

```js
assert.exists(document.querySelector('li a'));
```

Ton élément `a` doit avoir son attribut `href` défini à `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Ton élément `a` doit avoir le texte `Dashboard`.
