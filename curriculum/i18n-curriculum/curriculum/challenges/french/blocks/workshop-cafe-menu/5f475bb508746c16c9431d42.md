...
L'image que vous avez ajoutée n'est pas centrée horizontalement comme la `Coffee` allant au-dessus. `img` les éléments sont des éléments en ligne "comme".

Pour faire en sorte que l'image se comporte comme des éléments de niveau bloc tels que des entêtes, créez un `img` sélecteur de type. Définissez la `display` propriété `block`, et utilisation `margin-left` et `margin-right` propriétés pour le centrer horizontalement.

# --hints--

Vous devriez utiliser un `img` sélecteur.
...
Vous devriez mettre le `display` propriété `block`.
...
Votre `img` élément devrait avoir un `display` des `block`.
...
Votre `img` élément devrait avoir un `margin-left` et `margin-right` des `auto`.
