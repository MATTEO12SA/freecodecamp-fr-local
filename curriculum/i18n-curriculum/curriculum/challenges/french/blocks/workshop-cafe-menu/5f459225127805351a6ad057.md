...
Modifier la couleur de fond de la `hr` élément à `brown` Il correspond donc à la couleur des grains de café.

# --hints--

Vous devriez définir la valeur de la `background-color` propriété `brown`.

```js
const hasBackgroundColor = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['background-color'] === 'brown');
```

Votre `hr` élément devrait avoir un `background-color` des `brown`.
