...
Vers le bas `footer` élément, faire tout le texte ont une valeur de `14px` pour la taille de la police.

# --hints--

Vous devriez avoir un `footer` sélecteur.
...
Votre `footer` sélecteur doit être en dessous de votre commentaire.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Vous devriez mettre le `font-size` propriété `14px`.
...
Votre `footer` élément devrait avoir un `font-size` des `14px`.
