...
En utilisant le même sélecteur de style dans l'étape précédente, rendre la taille de police des articles et les prix plus grands en utilisant une valeur de `18px`.

# --hints--

Vous devriez mettre le `font-size` propriété `18px`.
...
Votre `p` éléments imbriqués dans votre `.item` les éléments devraient avoir une `font-size` des `18px`.
