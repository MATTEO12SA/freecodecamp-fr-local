...
Maintenant appliquer la `established` à la classe `Est. 2020` Texte.

# --hints--

Vous devriez mettre le `class` des `p` élément à `established`.

```js
assert.match(code,/<p class=('|")established\1>/i);
```

Votre `established` classe doit être sur l'élément avec le texte `Est. 2020`.

```js
assert.match(document.querySelector('.established')?.innerText, /Est\.\s2020/i);
```

Votre `established` l'élément de classe doit avoir un texte en italique.
