...
Vous pouvez donner à votre menu un espace entre le contenu et les côtés avec divers `padding` propriétés.

Donnez le `menu` classe a `padding-left` et a `padding-right` ayant la même valeur `20px`.

# --hints--

Vous devriez mettre le `padding-left` propriété `20px`.
...
Vous devriez mettre le `padding-right` propriété `20px`.
...
Votre `.menu` élément devrait avoir un `padding-left` des `20px`.
...
Votre `.menu` élément devrait avoir un `padding-right` des `20px`.
