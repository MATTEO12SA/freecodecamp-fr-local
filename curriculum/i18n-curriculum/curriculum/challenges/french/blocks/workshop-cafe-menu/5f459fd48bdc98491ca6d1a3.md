...
Allez-y et ajoutez un autre `hr` entre les `main` élément et `footer` élément.

# --hints--

Vous devriez ajouter une seconde `hr` élément.

```js
assert.lengthOf(document.querySelectorAll('hr'), 2);
```

Votre nouveau `hr` l'élément doit être entre `main` élément et `footer` élément.
