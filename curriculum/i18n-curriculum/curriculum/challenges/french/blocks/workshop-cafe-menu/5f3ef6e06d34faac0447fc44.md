...
Italiquement, `Est. 2020` en créant une `established` sélecteur de classe et lui donner un `font-style` biens de `italic`.

# --hints--

Vous devriez avoir un `.established` sélecteur.
...
Vous devriez mettre le `font-style` propriété `italic`.
...
Votre `.established` sélecteur doit définir le `font-style` propriété `italic`.
