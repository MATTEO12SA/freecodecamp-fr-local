...
Sous le dessert que vous venez d'ajouter, ajoutez le reste des desserts et les prix en utilisant trois autres `article` éléments, chacun avec deux imbriqués `p` éléments. Chaque élément devrait avoir le bon texte dessert et prix, et tous devraient avoir les bonnes classes.
...
Tu devrais en avoir quatre. `article` éléments dans votre deuxième `section`.
...
Tu devrais en avoir quatre. `.dessert` éléments.

```js
assert.lengthOf(document.querySelectorAll('.dessert'),4);
```

Tu devrais avoir quatre nouveaux. `.price` éléments.
...
Votre `section` élément devrait avoir huit `p` éléments.
...
Votre `.dessert` les éléments devraient avoir le texte `Donut`, `Cherry Pie`, `Cheesecake`et `Cinnamon Roll`.
...
Votre nouveau `.price` les éléments devraient avoir le texte `1.50`, `2.75`, `3.00`et `2.50`.
...
Vous ne devriez pas avoir d'espace entre votre `p` éléments.
