...
Et comme troisième argument de couleur, passe la fonction `hsl` avec les valeurs `240` pour la teinte, `56%` pour la saturation et `42%` pour la luminosité.

# --hints--

Ta règle CSS `.blue` devrait avoir une propriété `background` définie sur `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
