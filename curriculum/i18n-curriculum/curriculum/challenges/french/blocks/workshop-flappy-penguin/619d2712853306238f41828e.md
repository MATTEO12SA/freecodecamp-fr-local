...
Crée une variable CSS personnalisée nommée `--penguin-skin`, et définis-la sur `gray`. Ensuite, remplace toutes les valeurs de propriété pertinentes par celle-ci.

# --hints--

Tu devrais donner à `:root` une propriété `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Tu devrais donner à `--penguin-skin` une valeur de `gray`, mais `--fcc-actual--` a été trouvé.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Tu devrais donner à `.penguin-head` un `background` de `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Tu devrais donner à `.penguin-body::before` un `background-color` de `var(--penguin-skin)`, mais `--fcc-actual--` a été trouvé.
...
Tu devrais donner à `.arm` un `background` de `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
