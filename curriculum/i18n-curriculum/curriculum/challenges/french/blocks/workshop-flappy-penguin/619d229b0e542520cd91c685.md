...
Pour que les pieds du pingouin aient une allure plus _pingouinesque_, fais pivoter le pied gauche de `80deg`, et le droit de `-80deg`.

# --hints--

Tu devrais donner à `.foot.left` un `transform` de `rotate(80deg)`.
...
Tu devrais donner à `.foot.right` un `transform` de `rotate(-80deg)`.
