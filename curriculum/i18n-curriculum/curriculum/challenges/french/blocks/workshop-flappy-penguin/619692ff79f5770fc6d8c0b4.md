...
Au-dessus de l'élément `.ground`, ajoute un `div` avec une `class` de `penguin`. Ce `div` contiendra Flappy Penguin.

# --hints--

Tu devrais ajouter un nouveau `div` dans le `body`.

```js
assert.lengthOf(document.querySelectorAll('body > div'), 2);
```

Tu devrais donner au `div` une `class` de `penguin`.

```js
assert.include(document.querySelector('body > div:not(.ground)')?.className, 'penguin');
```

Tu devrais placer `div.penguin` avant `div.ground`.
