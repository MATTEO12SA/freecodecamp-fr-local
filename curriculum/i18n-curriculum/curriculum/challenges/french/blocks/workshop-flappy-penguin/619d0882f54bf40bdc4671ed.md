...
Là où c est pertinent, remplace les valeurs de propriété par ta variable `--penguin-face`.

# --hints--

Tu devrais donner à `.face` un `background-color` de `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Tu devrais donner à `.chin` un `background-color` de `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Tu ne devrais pas utiliser `var(--penguin-face)` dans le sélecteur `.penguin-body`.
