...
Cible l'élément `.penguin`, et définis son `width` et son `height` sur `300px`.

# --hints--

Tu devrais utiliser le sélecteur `.penguin`.

```js
assert.match(code, /\.penguin\s*\{/);
```

Tu devrais donner à `.penguin` un `width` de `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.width, '300px');
```

Tu devrais donner à `.penguin` un `height` de `300px`.
