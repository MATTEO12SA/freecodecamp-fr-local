...
Donne à l'élément `.sun` un `width` et un `height` de `200px`, ainsi qu'un `background-color` de `yellow`.

# --hints--

Tu devrais utiliser le sélecteur `.sun`.

```js
assert.match(code, /\.sun\s*\{/);
```

Tu devrais donner à `.sun` un `width` de `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

Tu devrais donner à `.sun` un `height` de `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

Tu devrais donner à `.sun` un `background-color` de `yellow`.
