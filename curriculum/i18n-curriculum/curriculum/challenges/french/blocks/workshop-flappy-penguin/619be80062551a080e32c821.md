...
Arrondis la huppe en donnant aux coins inférieurs du pseudo-élément un rayon de `100%`, tout en laissant les coins supérieurs à `0%`.

# --hints--

Tu devrais utiliser la propriété `border-radius` pour arrondir la huppe.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Tu devrais donner à `.penguin-body::before` un `border-radius` de `0% 0% 100% 100%`.
