...
Positionne l'élément `.chin` de sorte qu'il soit à `25%` du haut et à `5%` de la gauche de son parent. Ensuite, donne aux coins supérieurs un rayon de `70%`, et aux coins inférieurs un rayon de `100%`.

# --hints--

Tu devrais donner à `.chin` une propriété `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Tu devrais donner à `.chin` un `top` de `--fcc-expected--`, mais `--fcc-actual--` a été trouvé.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Tu devrais donner à `.chin` une propriété `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Tu devrais donner à `.chin` un `left` de `--fcc-expected--`, mais `--fcc-actual--` a été trouvé.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Tu devrais donner à `.chin` un `border-radius` de `70% 70% 100% 100%`.
