...
Ajoute un nouveau sélecteur qui change la `background-color` de l'élément `.card` en `khaki` quand il est survolé.

# --hints--

Tu devrais avoir un sélecteur `.card:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card:hover"));
```

Le sélecteur `.card:hover` devrait avoir une `background-color` définie sur `khaki`.
