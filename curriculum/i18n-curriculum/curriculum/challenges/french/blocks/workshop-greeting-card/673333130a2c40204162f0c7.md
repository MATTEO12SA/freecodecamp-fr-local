...
L'élément `.message` a besoin de style. Donne-lui :

- une `font-size` de `1.2em`,
- une `margin-bottom` de `20px`.

# --hints--

Tu devrais avoir un sélecteur `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

Le sélecteur `.message` devrait avoir une propriété `font-size` définie sur `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

Le sélecteur `.message` devrait avoir une propriété `margin-bottom` définie sur `20px`.
