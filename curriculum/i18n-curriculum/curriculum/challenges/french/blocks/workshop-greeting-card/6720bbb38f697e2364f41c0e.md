...
Maintenant, ajoutons de la couleur. Donne au `body` une `background-color` de `brown` et donne aussi à `.card` une `background-color` de `white`.

# --hints--

L'élément `body` devrait avoir une `background-color` de `brown`.
...
Tu devrais avoir un sélecteur `.card`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

L'élément `.card` devrait avoir une `background-color` de `white`.
