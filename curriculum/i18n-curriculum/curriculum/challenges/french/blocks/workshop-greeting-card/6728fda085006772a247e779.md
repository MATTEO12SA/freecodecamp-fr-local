...
Donne à `.card` une `max-width` de `400px`, un `padding` de `40px` sur tous les côtés et une `margin` de `0` en haut et en bas, puis `auto` à gauche et à droite (utilise la propriété raccourcie).

# --hints--

`.card` devrait avoir une `max-width` de `400px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("max-width"), "400px");
```

`.card` devrait avoir un `padding` de `40px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("padding"), "40px");
```

`.card` devrait avoir une `margin` de `0 auto`.
