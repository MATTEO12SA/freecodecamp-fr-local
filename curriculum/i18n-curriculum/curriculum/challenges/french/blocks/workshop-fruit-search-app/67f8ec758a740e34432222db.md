...
Ajoute un attribut `onChange` à l'input pour capturer ce que l'utilisateur tape. Définis-le comme une fonction fléchée qui prend `e` en argument. À l'intérieur, mets à jour l'état `query` en passant `e.target.value` à la fonction setter, qui contient la valeur actuelle de l'input. Voici un exemple :
...
Tu dois t'assurer que le gestionnaire `onChange` est défini.
...
Ton gestionnaire `onChange` doit avoir un paramètre `e`.
...
Ton gestionnaire `onChange` doit appeler `setQuery()` avec `e.target.value`.
