...
À l'intérieur du callback de `useEffect`, ajoute une condition `if` pour vérifier si `query.trim() === ''`. Cela vérifie si `query` est une chaîne vide ou ne contient que des espaces. Si c'est le cas, appelle `setResults([])` et sors tôt avec `return`.

# --hints--

Tu dois vérifier si `query.trim() === ''`.
...
Si `query.trim() === ''` est vrai, appelle `setResults()` avec un tableau vide, puis utilise le mot-clé `return` juste après.
