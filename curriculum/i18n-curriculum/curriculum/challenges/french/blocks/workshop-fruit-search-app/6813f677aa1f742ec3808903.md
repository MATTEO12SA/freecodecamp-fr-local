...
Pour le bloc `catch`, ajoute un paramètre `error`, puis utilise `console.error` pour journaliser toute erreur survenue pendant la récupération ou le traitement des données.

# --hints--

Ton bloc `catch` doit avoir un paramètre `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Tu dois journaliser toute erreur dans le bloc `catch` avec `console.error`.
