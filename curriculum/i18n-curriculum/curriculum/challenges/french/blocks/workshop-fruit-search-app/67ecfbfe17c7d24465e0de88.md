...
À l'intérieur de l'élément `form`, après le label, ajoute un input avec un `id` de `search-input` et un `type` de `search`.

# --hints--

Tu dois avoir un `input` à l'intérieur de l'élément `form` avec un `id` de `search-input`.
...
Ton `input` doit avoir une valeur `type` de `search`.
