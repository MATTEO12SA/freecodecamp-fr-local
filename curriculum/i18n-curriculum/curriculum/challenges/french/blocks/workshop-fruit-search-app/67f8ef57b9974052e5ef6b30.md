...
À l'intérieur de l'élément `#results`, utilise un opérateur ternaire pour vérifier si la longueur du tableau `results` est supérieure à zéro. Si c'est le cas, parcours les éléments avec `map` et affiche chacun dans un élément `p` avec une classe `result-item`. N'oublie pas d'inclure aussi un attribut `key` sur chaque paragraphe.

Si `results` est vide, affiche un élément `p` avec le message `No results found`.

# --hints--

Ton conteneur `#results` doit contenir une liste de fruits correspondant à la requête. Chaque résultat doit être un élément `p` avec la classe `result-item` et doit montrer le nom du fruit.
...
Ton élément `#results` doit contenir un élément `p` avec le texte `No results found` quand le tableau `results` est vide.
