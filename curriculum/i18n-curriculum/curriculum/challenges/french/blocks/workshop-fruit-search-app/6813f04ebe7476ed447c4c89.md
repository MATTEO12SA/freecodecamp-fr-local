...
Ensuite, appelle `.json()` sur la réponse et stocke le résultat dans une variable appelée `data`. Comme tu es toujours dans une fonction `async`, tu dois utiliser `await` devant `response.json()`.

# --hints--

Tu dois appeler `json()` sur `response` et le stocker dans une variable `data`.
