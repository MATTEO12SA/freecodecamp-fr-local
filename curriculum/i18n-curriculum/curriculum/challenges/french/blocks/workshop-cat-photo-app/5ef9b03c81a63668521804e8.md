...
Imbrique un `p` avec le texte `No Copyright - freeCodeCamp.org` à l'intérieur du `footer`.

# --hints--

Le `footer` doit toujours être présent (balise ouvrante et fermante).

```js
assert(document.querySelector('footer') && code.match(/<\/footer>/));
```

Le `footer` doit contenir un `p` (avec balises ouvrante et fermante).

```js
assert(document.querySelector('footer > p'));
```

Le `p` doit avoir une balise fermante.
...
Le texte du `p` doit être `No Copyright - freeCodeCamp.org`.
