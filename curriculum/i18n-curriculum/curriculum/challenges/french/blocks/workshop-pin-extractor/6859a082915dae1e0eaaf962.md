...
Enfin, renvoie `secret_codes`, puis décommente l'appel de fonction et mets à jour l'argument pour qu'il soit `[poem, poem2, poem3]`. Maintenant, ta fonction fonctionne avec plusieurs poèmes à la fois, et peut extraire plusieurs codes PIN !

La fonction `pin_extractor` est terminée.

# --hints--

La fonction devrait renvoyer `secret_codes` à la fin.
...
Tu devrais avoir l'appel de fonction `print(pin_extractor([poem, poem2, poem3]))`.
...
`pin_extractor([poem, poem2, poem3])` devrait renvoyer `['5202', '3346', '50000']`.
...
La fonction devrait fonctionner correctement avec d'autres listes de poèmes.
