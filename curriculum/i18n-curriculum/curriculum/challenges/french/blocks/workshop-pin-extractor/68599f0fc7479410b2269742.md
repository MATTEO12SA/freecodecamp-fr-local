...
Modifie l'argument de la fonction pour qu'il soit `poems`, puis crée une boucle `for` autour de tout le contenu actuel de la fonction qui itère sur `poems` et utilise `poem` comme variable de boucle.

# --hints--

La fonction `pin_extractor` devrait avoir un paramètre nommé `poems`.
...
Tu devrais avoir une boucle `for` qui parcourt `poems`.
...
La boucle devrait avoir `poem` comme variable de boucle.
...
Tu devrais déplacer le corps existant de la fonction dans la nouvelle boucle `for`.
