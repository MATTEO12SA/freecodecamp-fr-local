...
À l’intérieur du constructeur, ajoute des appels à `fetchCardsData()` et `initializeEventListeners()` avec le mot-clé `this`, pour qu’ils s’exécutent à l’initialisation de la classe.

La méthode `fetchCardsData` gérera le chargement des données des cartes de tarot depuis une API, et `initializeEventListeners` configurera les gestionnaires de clic des boutons. Tu créeras ces deux méthodes dans les prochaines étapes.

# --hints--

Tu dois appeler la méthode `fetchCardsData()` à l’intérieur du `constructor`.
...
Tu dois appeler la méthode `initializeEventListeners()` à l’intérieur du constructeur.
