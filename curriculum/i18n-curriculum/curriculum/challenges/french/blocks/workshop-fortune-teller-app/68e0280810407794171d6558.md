...
Maintenant, ajoute un `div` avec la classe `img-loader`. Ensuite, crée un élément `img` et donne-lui une URL conditionnelle pour l’image avec `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

Tu dois avoir un élément `div` à l’intérieur de ton élément `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Ton `div` doit avoir une classe `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Tu dois avoir un élément `img` à l’intérieur de ton élément `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

L’élément `img` doit utiliser `CDN_URL` et `img` pour son `src` lorsque `img` est fourni, et se rabattre sur `LOCAL_DEFAULT_IMG` sinon.
...
Ton élément `img` doit avoir les classes `card-img` et `hidden`.
