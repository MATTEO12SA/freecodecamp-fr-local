...
À l’intérieur de la méthode `initializeEventListeners`, attache un écouteur d’événement `click` à `singleCardBtn`. Dans le gestionnaire d’événement, appelle la méthode `singleCardSelected`. N’oublie pas de récupérer `singleCardBtn` depuis `elements`.

Tu créeras cette méthode dans les prochaines étapes.

# --hints--

Tu dois attacher un événement `click` à l’élément `singleCardBtn`. N’oublie pas que tu as besoin du mot-clé `this` ici.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

À l’intérieur du rappel d’événement, tu dois appeler la méthode `singleCardSelected`.
