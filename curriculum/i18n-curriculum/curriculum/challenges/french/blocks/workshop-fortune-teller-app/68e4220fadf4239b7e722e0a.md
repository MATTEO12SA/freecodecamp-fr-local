...
Puisque l’application de voyance n’a que trois moments — Passé, Présent et Futur —, déclare un enum nommé `DrawingType` avec trois propriétés : `Past`, `Present` et `Future`. Chaque propriété doit avoir une valeur chaîne correspondant à son nom.

# --hints--

Tu dois avoir un `enum` nommé `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Ton enum doit avoir une propriété `Past` avec une valeur chaîne `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Ton enum doit avoir une propriété `Present` avec une valeur chaîne `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Ton enum doit avoir une propriété `Future` avec une valeur chaîne `future`.
