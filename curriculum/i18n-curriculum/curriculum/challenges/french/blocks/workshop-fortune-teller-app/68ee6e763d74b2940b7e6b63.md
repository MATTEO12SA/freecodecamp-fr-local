...
L’application de voyance prend en charge le rendu de plusieurs cartes, donc tu as besoin d’une méthode pour aider à rendre plusieurs cartes. Déclare une méthode appelée `multipleCardSelected`.

À l’intérieur de la méthode, appelle la fonction `hideElements` et passe les propriétés `singleCard`, `singleCardBtn`, `multipleCardsBtn` et `headerTitle`. N’oublie pas que tu dois y accéder via `this.elements`.

# --hints--

Tu dois déclarer une méthode `multipleCardSelected`.
...
Tu dois appeler la fonction `hideElements` à l’intérieur de la méthode `multipleCardSelected`.
...
Tu dois passer `singleCard` comme premier argument à la fonction `hideElements`.
...
Tu dois passer `singleCardBtn` comme argument à la fonction `hideElements`.
...
Tu dois passer `multipleCardsBtn` comme argument à la fonction `hideElements`.
...
Tu dois passer `headerTitle` comme argument à la fonction `hideElements`.
