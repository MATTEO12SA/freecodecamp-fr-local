...
Déclare une variable nommée `cardElement` et utilise la classe `card_container` pour obtenir l’élément. Utilise le sélecteur `closest` pour le récupérer depuis `target`.

# --hints--

Tu dois déclarer une variable nommée `cardElement` à l’intérieur de `showFortune`.
...
Ton `cardElement` doit utiliser `target?.closest` pour trouver `.card_container`.
...
Tu dois passer `.card_container` comme argument à `closest`.
