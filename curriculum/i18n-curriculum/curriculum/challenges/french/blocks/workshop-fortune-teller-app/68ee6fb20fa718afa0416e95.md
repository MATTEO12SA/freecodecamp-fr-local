...
Comme pour le `multipleCard` précédent, récupère à nouveau l’élément `multipleCard` et définis son `innerHTML` en itérant sur `DrawingType`. Utilise `Object.values()` et appelle la méthode `map` dessus, en passant `type` comme argument.

# --hints--

Tu dois assigner `multipleCard.innerHTML` en appelant `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Tu dois passer `DrawingType` à la méthode `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Tu dois appeler la méthode `map` sur `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Ta méthode `map` doit avoir un paramètre nommé `type`.
