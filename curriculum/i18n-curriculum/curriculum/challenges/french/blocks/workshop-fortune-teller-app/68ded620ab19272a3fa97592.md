...
Puisque le projet travaille maintenant avec des éléments HTML du DOM, tu as besoin d’un moyen d’afficher la prédiction dans le DOM.

Déclare une fonction nommée `renderCard` qui prend quatre arguments : `drawingType`, `isReversed`, `shortName` et `img`.

Les paramètres `drawingType`, `shortName` et `img` doivent être de type `string`, tandis que `isReversed` doit être de type `boolean`. La fonction doit avoir un type de retour `string`.

# --hints--

Tu dois avoir une fonction nommée `renderCard`.
...
Ta fonction `renderCard` doit avoir quatre paramètres.
...
Ton premier paramètre doit être `drawingType` de type `string`.
...
Ton deuxième paramètre doit être `isReversed` de type `boolean`.
...
Ton troisième paramètre doit être `shortName` de type `string`.
...
Ton quatrième paramètre doit être `img` de type `string`.
...
Ta fonction `renderCard` doit avoir un type de retour `string`.
...
Ta fonction `renderCard` doit renvoyer un littéral de gabarit.
