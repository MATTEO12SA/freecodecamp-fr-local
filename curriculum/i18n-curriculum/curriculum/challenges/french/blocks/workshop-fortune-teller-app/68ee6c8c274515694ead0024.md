...
La prédiction doit être aléatoire, donc une fonction utilitaire est nécessaire pour en sélectionner une au hasard. Déclare une variable nommée `chosenCard` à l’intérieur de la méthode `singleCardSelected`. Utilise la méthode `getRandomItem` pour obtenir une carte aléatoire et passe `cards` comme argument.

# --hints--

Tu dois déclarer une variable nommée `chosenCard` à l’intérieur de `singleCardSelected`.
...
Ta variable `chosenCard` doit être assignée en appelant `getRandomItem(this.cards)`.
...
Tu dois passer `this.cards` comme argument à `getRandomItem`.
