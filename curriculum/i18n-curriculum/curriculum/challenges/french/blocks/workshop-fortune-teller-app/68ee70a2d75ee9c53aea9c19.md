...
À l’intérieur du `map`, renvoie la fonction `renderCard` en passant `type`, `isReversed`, `name_short` et `img` comme arguments, et ajoute la méthode `join` sur le `map` avec une chaîne vide.

# --hints--

Tu dois renvoyer la fonction `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Tu dois passer `type` comme premier argument dans la fonction `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Tu dois passer `isReversed` comme deuxième argument dans la fonction `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Tu dois passer `card.name_short` comme argument dans la fonction `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Tu dois passer `card.img` comme argument dans la fonction `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Tu dois attacher `join("")` à la méthode `map`.
