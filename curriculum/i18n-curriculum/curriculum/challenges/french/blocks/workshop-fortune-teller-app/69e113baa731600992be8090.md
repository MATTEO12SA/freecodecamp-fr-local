...
Une image par défaut pour la carte et l’URL pour faire une requête API ont été fournies ; tu as seulement besoin de quelques variables pour les stocker.

Crée une variable `CDN_URL` et définis-la à `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Crée aussi une variable `defaultImg` avec la valeur `new URL("default.svg", import.meta.url)`, et une variable `LOCAL_DEFAULT_IMG` avec la valeur `defaultImg`.

# --hints--

Tu dois créer une variable nommée `CDN_URL`.
...
Ta variable `CDN_URL` doit être définie à `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`.
...
Tu dois créer une variable nommée `defaultImg`.
...
Ta variable `defaultImg` doit être définie à `new URL("default.svg", import.meta.url)`.
...
Tu dois créer une variable nommée `LOCAL_DEFAULT_IMG`.
...
Ta variable `LOCAL_DEFAULT_IMG` doit être définie à `defaultImg`.
