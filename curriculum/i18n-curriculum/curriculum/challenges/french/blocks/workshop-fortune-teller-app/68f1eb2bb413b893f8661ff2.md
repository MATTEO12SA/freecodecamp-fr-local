...
À ce stade, tu devrais pouvoir voir la prédiction que chaque carte montre une fois que tu cliques sur une carte. Il ne reste plus qu’à faire fonctionner le bouton de nouvelle lecture.

À l’intérieur de la méthode `newReading`, appelle `showElements()` et `hideElements()`.

# --hints--

Tu dois appeler la fonction `showElements` à l’intérieur de la méthode `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Tu dois appeler la fonction `hideElements` à l’intérieur de la méthode `newReading`.
