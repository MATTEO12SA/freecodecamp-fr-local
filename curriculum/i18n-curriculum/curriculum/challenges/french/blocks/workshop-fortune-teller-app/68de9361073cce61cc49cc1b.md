...
Dans une leçon précédente, tu as appris que l’inférence de types est la capacité de TypeScript à déterminer et assigner automatiquement des types aux variables, aux retours de fonctions et aux expressions selon leur usage et leur contexte.

En TypeScript, les génériques rendent les fonctions, les classes et les types plus flexibles. Les génériques permettent de créer des variables de type, ce qui évite de définir explicitement les types à chaque fois. Ainsi, la même fonction peut travailler en toute sécurité avec différents types.

Voici un exemple simple de génériques :
...
Les fonctions peuvent utiliser deux génériques ou plus. `T` est utilisé par convention, et d’autres variables de type comme `S` peuvent être ajoutées selon les besoins.
...
Crée une fonction `getElement` qui prend `selector` de type `string` en paramètre. Elle doit aussi prendre un type générique `T` qui étend `HTMLElement`, et avoir un type de retour `T`.

# --hints--

Tu dois avoir une fonction fléchée nommée `getElement`.
...
Ta fonction `getElement` doit prendre un paramètre nommé `selector` de type `string`.
...
Le paramètre de type générique `T` doit étendre le type `HTMLElement`.
...
Ta fonction `getElement` doit avoir un type de retour `T`.
