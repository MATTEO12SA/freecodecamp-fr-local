...
Fais maintenant la même chose pour `subTitle` en lui assignant `meaning_up`, et pour `title` en lui assignant `foundCard.name`. Ensuite, appelle la méthode `showElements` et passe `fortuneDescription` depuis les éléments.

# --hints--

Tu dois définir `this.elements.subTitle.textContent` à `foundCard.meaning_up`.
...
Tu dois définir `this.elements.title.textContent` à `foundCard.name`.
...
Tu dois appeler la fonction `showElements` à l’intérieur de `showFortune`.
...
Tu dois passer `this.elements.fortuneDescription` comme argument à `showElements`.
