...
Juste après avoir vidé `multipleCard`, à l’intérieur de la méthode `singleCardSelected`, appelle la fonction `showElements` et passe `singleCard` comme premier argument et `fortuneContainer` comme deuxième argument.

# --hints--

Tu dois appeler la fonction `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Tu dois passer `elements.singleCard` comme premier argument.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Tu dois passer `elements.fortuneContainer` comme deuxième argument.
