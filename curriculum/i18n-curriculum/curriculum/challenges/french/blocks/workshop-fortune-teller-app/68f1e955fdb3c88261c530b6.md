...
Ajoute une instruction `if` pour vérifier si `foundCard` existe avant de mettre à jour les éléments avec ses données ou son contenu.

# --hints--

Tu dois déclarer une instruction `if` avec la condition `foundCard`.
