...
À l’intérieur de la méthode `showFortune`, déclare une variable appelée `target` et assigne-lui `e.target`, puis crée une autre instruction if vide.

# --hints--

Tu dois déclarer une variable nommée `target` à l’intérieur de `showFortune`.
...
Ta variable `target` doit être assignée à `e.target`.
...
Tu dois avoir une instruction `if` vide juste après ta variable `target`.
