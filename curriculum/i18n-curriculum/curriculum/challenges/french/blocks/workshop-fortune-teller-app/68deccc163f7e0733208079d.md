...
Ensuite, tu dois créer une fonction pour masquer des éléments du DOM. Déclare une fonction `hideElements` qui prend `...elements` comme paramètre avec le type `HTMLElement[]`.

Cette fonction, avec une classe CSS `hidden`, sera utilisée dans toute l’application pour basculer la visibilité des éléments d’interface.

# --hints--

Tu dois avoir déclaré une fonction nommée `hideElements`.
...
Ta fonction `hideElements` doit avoir un paramètre nommé `...elements` avec le type `HTMLElement[]`.
