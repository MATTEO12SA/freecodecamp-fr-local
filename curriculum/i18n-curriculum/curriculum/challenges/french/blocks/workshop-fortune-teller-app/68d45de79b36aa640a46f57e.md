...
Tu as besoin d’une autre interface appelée `Deck`. Cette interface doit avoir une propriété `cards`, qui est un tableau de l’interface précédente.

# --hints--

Tu dois déclarer une interface nommée `Deck`.
...
Ton interface `Deck` doit avoir une propriété `cards` de type `Card[]`.
