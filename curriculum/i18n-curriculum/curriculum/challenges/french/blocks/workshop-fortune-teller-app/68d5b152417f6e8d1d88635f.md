...
Il y a d’autres propriétés à ajouter à l’interface `Card`. Commence par définir `value_int` avec le type `number`. Toutes les autres propriétés doivent être de type `string` : `suit`, `type`, `img`, `meaning_up`, `meaning_rev` et `desc`.

# --hints--

Tu dois avoir une propriété `value_int` de type `number`.
...
Tu dois avoir une propriété `suit` de type `string`.
...
Tu dois avoir une propriété `type` de type `string`.
...
Tu dois avoir une propriété `img` de type `string`.
...
Tu dois avoir une propriété `meaning_up` de type `string`.
...
Tu dois avoir une propriété `meaning_rev` de type `string`.
...
Tu dois avoir une propriété `desc` de type `string`.
