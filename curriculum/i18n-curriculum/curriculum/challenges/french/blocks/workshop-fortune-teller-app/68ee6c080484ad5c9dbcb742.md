...
Tu vas maintenant commencer à créer les méthodes.

Crée une méthode appelée `singleCardSelected`. À l’intérieur, appelle la fonction `hideElements` et passe les propriétés suivantes : `singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text` et `headerTitle`.

N’oublie pas que tu dois y accéder via `this.elements`.

# --hints--

Tu dois définir une méthode nommée `singleCardSelected`.
...
À l’intérieur de `singleCardSelected`, tu dois appeler la fonction `hideElements`.
...
Tu dois passer `singleCardBtn` à la fonction `hideElements`.
...
Tu dois passer `multipleCardsBtn` à la fonction `hideElements`.
...
Tu dois passer `multipleCard` à la fonction `hideElements`.
...
Tu dois passer `text` à la fonction `hideElements`.
...
Tu dois passer `headerTitle` à la fonction `hideElements`.
