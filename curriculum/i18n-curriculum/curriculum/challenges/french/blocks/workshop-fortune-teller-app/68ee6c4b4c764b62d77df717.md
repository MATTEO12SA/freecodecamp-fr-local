...
Dans certains cas, les cartes doivent être inversées, car elles ne sont pas toujours dans la bonne position.

Déclare une variable appelée `isReversed` à l’intérieur de la méthode `singleCardSelected`. Utilise la méthode `Math.random()` pour générer une valeur booléenne aléatoire. Pour rester dans les bornes, compare-la à `0.5`.
