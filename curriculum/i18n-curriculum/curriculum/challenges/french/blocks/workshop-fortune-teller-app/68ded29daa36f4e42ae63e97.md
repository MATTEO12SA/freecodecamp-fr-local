...
Termine la fonction en utilisant la méthode de tableau `forEach` pour parcourir l’argument elements. Dans la fonction de rappel, ajoute la classe hidden à chaque élément avec la méthode `.add()` de `classList`.

# --hints--

Tu dois avoir une méthode `forEach` attachée au paramètre `elements`.
...
À l’intérieur de la méthode forEach, tu dois ajouter la classe hidden avec la méthode `classList.add`.
