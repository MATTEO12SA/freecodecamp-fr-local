...
Appelle maintenant la fonction `showElements` et passe `multipleCard`, `fortuneContainer` et `text` comme propriétés.

# --hints--

Tu dois appeler la fonction `showElements` à l’intérieur de la méthode `multipleCardSelected`.
...
Tu dois passer `multipleCard` comme premier argument à la fonction `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Tu dois passer `fortuneContainer` comme argument à la fonction `showElements`.
...
Tu dois passer `text` comme argument à la fonction `showElements`.
