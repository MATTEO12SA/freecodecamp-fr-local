...
Jusqu’ici, tu as trois fonctions : `getElement`, `hideElements` et `showElements`.
Tu as maintenant besoin d’une fonction qui t’aide à obtenir une prédiction aléatoire.

Déclare une fonction nommée `getRandomItem` qui prend un tableau `items` comme argument. Utilise des génériques pour cette fonction.

À l’intérieur de la fonction, utilise `Math.random()` et `Math.floor()` pour sélectionner une prédiction aléatoire dans le tableau `items`.
N’oublie pas de renvoyer la prédiction sélectionnée.

# --hints--

Tu dois déclarer une fonction nommée `getRandomItem`.
...
Ta fonction `getRandomItem` doit utiliser le paramètre de type générique `T`.
...
Ta fonction `getRandomItem` doit avoir un paramètre nommé `items` avec le type `T[]`.
...
Ta fonction `getRandomItem` doit avoir un type de retour `T`.
...
Tu dois déclarer une variable nommée `index` à l’intérieur de la fonction `getRandomItem`.
...
La variable `index` doit utiliser `Math.floor()` et `Math.random()` avec `items.length` pour produire un indice aléatoire.
...
Ta fonction `getRandomItem` doit renvoyer `items[index]`.
