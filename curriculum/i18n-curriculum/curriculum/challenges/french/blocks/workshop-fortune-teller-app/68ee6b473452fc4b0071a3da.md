...
Fais maintenant la même chose pour `multipleCardsBtn` et `newReadingBtn` en attachant un écouteur click à `multipleCardsBtn` qui appelle `multipleCardSelected`, et un autre à `newReadingBtn` qui appelle `newReading`.

Tu créeras les deux méthodes dans les prochaines étapes.

# --hints--

Tu dois attacher un événement `click` à l’élément `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dans le rappel d’événement de `multipleCardsBtn`, tu dois appeler la méthode `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Tu dois attacher un événement `click` à l’élément `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dans le rappel d’événement de `newReadingBtn`, tu dois appeler la méthode `newReading`.
