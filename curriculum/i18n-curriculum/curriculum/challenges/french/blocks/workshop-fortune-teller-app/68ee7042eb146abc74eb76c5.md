...
Encore une fois, tu dois rendre la carte aléatoire et inversée. Déclare une variable nommée `isReversed` et une nommée `card`. Pour la première, appelle la méthode `Math.random` et compare-la avec `< 0.5`. Pour la seconde, appelle `getRandomItem` et passe `this.cards`.

# --hints--

Tu dois déclarer une variable nommée `isReversed`.
...
Ta variable `isReversed` doit utiliser `Math.random()` pour décider aléatoirement si la carte est inversée.
...
Tu dois déclarer une variable nommée `card`.
...
Ta variable `card` doit être assignée en appelant la méthode `getRandomItem` avec `this.cards` comme argument.
