...
Dans la méthode `singleCardSelected`, récupère l’élément `singleCard` et mets à jour son `innerHTML` en appelant la fonction `renderCard`. Passe `"Click the card and reveal the fortune"` comme premier argument, et `isReversed` comme deuxième argument.

# --hints--

Tu dois mettre à jour `this.elements.singleCard.innerHTML` en appelant la fonction `renderCard`.
...
Ton appel à `renderCard` doit avoir `"Click the card and reveal the fortune"` comme premier argument.
...
Tu dois passer `isReversed` comme deuxième argument à `renderCard`.
