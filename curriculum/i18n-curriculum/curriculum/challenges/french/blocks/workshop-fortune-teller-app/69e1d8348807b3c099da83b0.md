...
Tu devrais tester ton progrès jusqu’ici. Crée une instance de la classe `Game` et assigne-la à une variable `game`.

À ce stade, tu verras que tu peux voir les cartes quand tu cliques sur les boutons Single Card Reading et
Three Card Spread.

# --hints--

Tu dois déclarer une variable nommée `game`.
...
Ta variable `game` doit être initialisée avec `new Game()`.
