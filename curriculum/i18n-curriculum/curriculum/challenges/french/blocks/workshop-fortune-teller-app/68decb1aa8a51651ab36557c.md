...
À l’intérieur de la fonction `getElement`, utilise `querySelector` avec les génériques et stocke le résultat dans une variable. Ajoute une instruction `if` pour vérifier si l’élément n’existe pas, et dans ce cas, lance une erreur avec le message `Element not found: ${error message here}`. Si l’élément existe, renvoie-le.

# --hints--

Tu dois déclarer une variable nommée `el` à l’intérieur de la fonction `getElement`.
...
Ta variable `el` doit recevoir la valeur `document.querySelector<T>(selector)`.
...
Tu dois avoir une instruction if qui vérifie si `el` n’est pas trouvé.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Tu dois lancer une nouvelle Error avec le message `Element not found: ${selector}` à l’intérieur de ton instruction `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Tu dois renvoyer la variable `el` à l’intérieur de la fonction getElement.
