...
Enfin, dans `hideElements`, passe les arguments `singleCard`, `multipleCard`, `fortuneContainer` et `fortuneDescription`. N’oublie pas de les récupérer depuis les éléments.

Avec ça, ton application Fortune Telling est terminée !

# --hints--

Tu dois passer `singleCard` comme premier argument à `hideElements`.
...
Tu dois passer `multipleCard` comme argument à `hideElements`.
...
Tu dois passer `fortuneContainer` comme argument à `hideElements`.
...
Tu dois passer `fortuneDescription` comme argument à `hideElements`.
