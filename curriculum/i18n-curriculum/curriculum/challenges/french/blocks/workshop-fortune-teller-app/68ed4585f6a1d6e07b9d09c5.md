...
À l’intérieur de la méthode `fetchCardsData`, déclare une variable nommée `response` et assigne-lui l’appel de la fonction `fetch` avec l’URL suivante : `` `${CDN_URL}/card_data.json` ``.

Ajoute le mot-clé `await` avant l’appel `fetch`, et enveloppe toute la logique dans un bloc `try...catch`.

# --hints--

Tu dois déclarer une variable `response` à l’intérieur de la méthode `fetchCardsData`.
...
Ta variable `response` doit être assignée en appelant `await fetch` avec l’URL `` `${CDN_URL}/card_data.json` ``.
...
Ton appel `fetch` doit être enveloppé dans un bloc `try...catch`, avec `error` comme paramètre du `catch`.
