...
Tu as besoin d’une autre fonction qui affiche des éléments dans le DOM. Comme pour la précédente, crée une fonction nommée `showElements`. Cette fois, utilise `classList.remove("hidden")`.

# --hints--

Tu dois avoir une fonction nommée `showElements`.
...
Ta fonction `showElements` doit avoir un paramètre nommé `...elements` avec le type `HTMLElement[]`.
...
Ta fonction `showElements` doit avoir une méthode `forEach` attachée au paramètre `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Ta méthode `forEach` doit prendre un paramètre nommé `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Tu dois retirer la classe `hidden` avec la méthode `classList.remove` à l’intérieur de la méthode `forEach`.
