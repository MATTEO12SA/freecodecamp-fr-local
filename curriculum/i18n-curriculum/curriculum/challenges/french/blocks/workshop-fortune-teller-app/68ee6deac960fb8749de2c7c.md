...
Inclus les arguments restants `chosenCard.name_short` et `chosenCard.img`. Puis, en dehors de l’appel à `renderCard`, réinitialise le `innerHTML` de l’élément `multipleCard` à une chaîne vide.

# --hints--

Tu dois passer `chosenCard.name_short` comme argument à `renderCard`.
...
Tu dois passer `chosenCard.img` comme dernier argument à `renderCard`.
...
Tu dois réinitialiser `this.elements.multipleCard.innerHTML` à une chaîne vide.
