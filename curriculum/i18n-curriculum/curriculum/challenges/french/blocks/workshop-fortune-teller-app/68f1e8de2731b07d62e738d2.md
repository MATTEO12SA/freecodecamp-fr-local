...
Déclare une autre variable nommée `foundCard`, et assigne-lui le résultat de l’appel à la méthode `find()` sur `cards`. Passe `card` comme paramètre du rappel, et compare `card.name_short` avec `cardId` pour trouver la carte correspondante avec une égalité stricte.

# --hints--

Tu dois déclarer une variable nommée `foundCard` à l’intérieur de `showFortune`.
...
Ton `foundCard` doit être assigné en appelant `find` sur `this.cards` avec le bon rappel.
...
Le rappel de `find` doit avoir un paramètre nommé `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

À l’intérieur du rappel, `card.name_short` doit être comparé avec `cardId`.
