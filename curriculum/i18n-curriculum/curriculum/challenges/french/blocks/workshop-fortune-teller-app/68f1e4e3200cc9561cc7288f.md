...
Pour la condition de l’instruction `if`, vérifie si `target` n’est pas une instance de `HTMLElement`. Puis, à l’intérieur, ajoute un `return`.

# --hints--

Tu dois vérifier dans ton instruction `if` la condition `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Tu dois avoir un `return` à l’intérieur de ton instruction `if`.
