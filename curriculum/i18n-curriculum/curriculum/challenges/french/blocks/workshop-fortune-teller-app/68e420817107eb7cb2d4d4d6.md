...
Ajoute un gestionnaire `onerror` à l’élément image pour gérer les échecs de chargement. Au premier échec, marque-le avec `dataset.failed` et remplace la source par l’image locale par défaut. Si l’image échoue à nouveau, retire la classe hidden et masque le placeholder de chargement avec `previousElementSibling`.

# --hints--

Ton élément `img` doit avoir un gestionnaire `onerror`.
...
Ton gestionnaire `onerror` doit contenir une instruction `if` qui vérifie si `dataset.failed` est falsy.
...
`dataset.failed` doit être défini à `'1'` dans ton bloc `if`. N’oublie pas le mot-clé `this` ici.
...
Ton bloc `if` doit mettre à jour `this.src` avec l’image locale par défaut. N’oublie pas le mot-clé `this` ici.
...
Tu dois avoir un bloc `else` qui retire la classe `hidden`. N’oublie pas le mot-clé `this` ici.
...
Ton bloc `else` doit définir `previousElementSibling.style.display` à `'none'`. N’oublie pas le mot-clé `this` ici.
