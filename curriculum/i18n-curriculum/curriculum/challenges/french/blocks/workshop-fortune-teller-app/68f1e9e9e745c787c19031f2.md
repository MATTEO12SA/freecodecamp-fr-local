...
À l’intérieur de l’instruction `if`, récupère l’élément `cardTitle` et définis son `textContent` à `foundCard.name`. Fais de même pour l’élément `description` en définissant son `textContent` à `foundCard.desc`.

# --hints--

Tu dois définir le `textContent` de `cardTitle` à `foundCard.name`. N’oublie pas d’utiliser le mot-clé `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Tu dois définir le `textContent` de `description` à `foundCard.desc`. N’oublie pas d’utiliser le mot-clé `this`.
