...
Dans cet atelier, tu vas continuer à pratiquer TypeScript en construisant une application de voyance.

Commence par déclarer une interface nommée `Card`. À l’intérieur de l’interface `Card`, ajoute les propriétés suivantes : `name`, `name_short` et `value`. Les propriétés `name` et `name_short` doivent avoir le type `string`, et `value` doit avoir un type union `string | number`.

# --hints--

Tu dois déclarer une interface nommée `Card`.
...
Ton interface `Card` doit avoir une propriété `name` de type `string`.
...
Ton interface `Card` doit avoir une propriété `name_short` de type `string`.
...
Ton interface `Card` doit avoir une propriété `value` avec un type union de `string` et `number`.
