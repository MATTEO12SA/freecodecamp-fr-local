...
Ajoute maintenant une autre méthode appelée `showFortune`. Comme paramètre, passe `e` et donne-lui le type `Event`. Crée aussi une méthode `newReading` et laisse-la vide pour l’instant.

# --hints--

Tu dois déclarer une méthode nommée `showFortune`.
...
Ta méthode `showFortune` doit avoir un paramètre `e` de type `Event`.
...
Tu dois déclarer une méthode nommée `newReading`.
