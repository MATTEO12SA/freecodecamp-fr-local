...
Ajoute un `constructor` à l’intérieur de la classe. Dans le constructeur, tu dois utiliser les éléments créés plus tôt pour sélectionner les éléments DOM correspondants avec la fonction `getElement`.

Pour l’instant, gère `singleCardBtn` et `singleCard`. Pour `singleCardBtn`, passe `#btn-single-card`, et pour `singleCard`, passe `.single_card`.

# --hints--

La classe `Game` doit avoir un `constructor`.
...
Ton constructeur doit assigner `this.elements`.
...
`singleCardBtn` doit être défini avec `getElement` et `'#btn-single-card'`.
...
`singleCard` doit être défini avec `getElement` et `'.single_card'`.
