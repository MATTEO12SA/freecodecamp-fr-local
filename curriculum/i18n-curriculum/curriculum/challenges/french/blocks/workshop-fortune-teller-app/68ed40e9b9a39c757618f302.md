...
Maintenant tu as besoin d’une classe principale qui aidera à rendre l’interface dynamiquement dans le DOM.

Crée une classe nommée `Game`. À l’intérieur, ajoute une propriété appelée `cards` de type `Card[]`, et pour l’instant, initialise-la avec un tableau vide.

# --hints--

Tu dois avoir une classe nommée `Game`.
...
Tu dois avoir une propriété `cards` à l’intérieur de la classe `Game`.
...
Ta propriété `cards` doit avoir le type `Card[]`.
...
Ta propriété cards doit être définie à un `array` vide.
