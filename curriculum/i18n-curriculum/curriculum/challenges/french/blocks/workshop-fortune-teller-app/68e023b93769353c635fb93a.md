...
Maintenant, à l’intérieur du littéral de gabarit de la fonction `renderCard`, crée un élément `div`. À l’intérieur du `div`, ajoute un élément `h2` qui affiche `drawingType`, et un élément `figure` avec la classe `card_container ${isReversed ? "reversed-card" : ""}` et l’attribut `data-id="${shortName}"`.

# --hints--

Tu dois avoir un élément `div` dans le littéral de gabarit de la fonction `renderCard`.
...
Tu dois avoir un élément `h2` qui interpole `drawingType` à l’intérieur de l’élément `div`.
...
Tu dois avoir un élément `figure` à l’intérieur de l’élément `div`.
...
Ton élément `figure` doit avoir la classe `card_container` et utiliser un ternaire pour ajouter conditionnellement `reversed-card`.
...
Ton élément `figure` doit avoir un attribut `data-id` défini à `${shortName}`.
