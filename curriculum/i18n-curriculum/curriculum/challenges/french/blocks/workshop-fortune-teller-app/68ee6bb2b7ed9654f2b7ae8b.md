...
Ajoute un autre écouteur d’événement sur l’objet global `document` qui appelle la fonction `showFortune`. Dans le rappel, inclus un paramètre `e` de type `Event`, et passe `e` comme argument à la méthode `showFortune`.

`showFortune` gérera les clics sur les cartes, comme lire l’ID de la carte cliquée depuis le DOM et afficher les détails de prédiction correspondants. Tu la créeras dans les prochaines étapes.

# --hints--

Tu dois assigner un écouteur d’événement `click` au `document` global.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ton écouteur d’événement doit avoir un paramètre de rappel nommé `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Ton paramètre de rappel doit avoir le type `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

À l’intérieur du rappel, tu dois appeler la fonction `showFortune`

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Dans l’appel à `showFortune`, tu dois passer l’argument `e`.
