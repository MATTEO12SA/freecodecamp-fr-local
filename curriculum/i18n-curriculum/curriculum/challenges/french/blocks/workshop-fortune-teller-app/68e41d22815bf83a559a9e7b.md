...
Maintenant, utilise l’événement `onload` pour retirer la classe `hidden` quand l’image se charge, et masquer le placeholder de chargement avec son `previousElementSibling`.

# --hints--

Ton élément `img` doit avoir un gestionnaire `onload`.
...
Ton gestionnaire `onload` doit utiliser `classList.remove` pour retirer la classe `hidden`. N’oublie pas que tu as besoin du mot-clé `this` ici.
...
Ton gestionnaire `onload` doit définir `previousElementSibling.style.display` à `'none'`. N’oublie pas que tu as besoin du mot-clé `this` pour cela.
