...
Après avoir récupéré les données, stocke la réponse dans une variable et parse-la en JSON. Donne aux données parsées le type `Deck`, puis assigne-les à la propriété `cards`. Dans le bloc `catch`, assure-toi de journaliser l’erreur avec ce message : `"Error fetching cards:", error`.

# --hints--

Tu dois déclarer une variable nommée `data` à l’intérieur de `fetchCardsData`.
...
Ta variable `data` doit avoir le type `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Ta variable `data` doit être assignée en parseant la réponse en JSON.
...
Tu dois stocker `data.cards` dans `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Ton bloc `catch` doit utiliser `console.error()` pour journaliser l’erreur avec le message `"Error fetching cards:", error`.
