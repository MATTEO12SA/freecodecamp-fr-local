...
Déclare une variable nommée `cardId`, et assigne-lui la valeur de l’attribut `data-id` de `cardElement` avec la méthode `getAttribute()`.

# --hints--

Tu dois déclarer une variable nommée `cardId` à l’intérieur de `showFortune`.
...
Ta variable `cardId` doit être assignée en appelant `cardElement.getAttribute("data-id")`.
...
Tu dois utiliser la méthode `getAttribute` sur `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Tu dois passer `'data-id'` à `getAttribute`.
