...
Pour qu'un input de type case à cocher soit coché par défaut, tu peux ajouter l'attribut `checked`.

Voici un exemple d'utilisation de l'attribut `checked` :

```html
<input checked type="checkbox" id="checked" name="checked">
```

Ajoute l'attribut `checked` à l'input de type case à cocher avec l'`id` de `"reputation"` pour qu'il soit coché par défaut.

# --hints--

Ton input de type case à cocher avec l'`id` de `"reputation"` doit avoir l'attribut `checked`.
