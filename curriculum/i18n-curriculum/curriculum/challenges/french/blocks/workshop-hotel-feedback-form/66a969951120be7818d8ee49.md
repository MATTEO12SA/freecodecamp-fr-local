...
La section suivante du formulaire permettra aux utilisateurs de laisser une note pour l'hôtel.

Commence par ajouter un nouvel élément `fieldset` avec un élément `legend` imbriqué dedans. Le `legend` doit avoir le texte `Ratings`.

Sous l'élément `legend`, ajoute un élément `label` avec le texte `How was the service?`. L'attribut `for` doit être défini sur `"service"`.
