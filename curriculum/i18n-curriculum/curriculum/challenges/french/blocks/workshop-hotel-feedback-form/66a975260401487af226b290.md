...
Pour sélectionner une `option` par défaut, tu peux ajouter l'attribut `selected` à l'élément `option` que tu veux sélectionner.

Voici un exemple utilisant l'attribut `selected` :

```html
<option selected value="amazing">Amazing</option>
```

Dans ton élément `select`, ajoute l'attribut `selected` à l'élément `option` avec la valeur `"excellent"`.

# --hints--

Ton élément `option` avec la valeur `"excellent"` doit avoir l'attribut `selected`.
