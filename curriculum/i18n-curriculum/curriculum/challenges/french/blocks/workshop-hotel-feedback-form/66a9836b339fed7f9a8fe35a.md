...
Pour ton élément `textarea`, ajoute un attribut `id` et un attribut `name` avec la valeur `"comments"`.

Pour l'attribut `cols`, définis la valeur sur `30`, et pour l'attribut `rows`, définis la valeur sur `10`.

# --hints--

Ton élément `textarea` doit avoir un attribut `id` défini sur `"comments"`.

```js
assert.exists(document.querySelector('textarea#comments'));
```

Ton élément `textarea` doit avoir un attribut `name` défini sur `"comments"`.

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

Ton élément `textarea` doit avoir un attribut `cols` défini sur `"30"`.

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

Ton élément `textarea` doit avoir un attribut `rows` défini sur `"10"`.
