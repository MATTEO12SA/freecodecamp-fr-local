...
title: Construire une application de gestion de favoris
challengeType: 25
...
Pour ce lab, tout le HTML et le CSS te sont fournis. Tu utiliseras JavaScript pour terminer l'application de gestion de favoris afin qu'elle puisse stocker une collection de favoris dans le local storage et en lire les données.

**Objectif :** Remplir les user stories ci-dessous et faire passer tous les tests pour terminer le lab.

**User Stories :**

1. Tu dois avoir une fonction `getBookmarks` qui renvoie le tableau `bookmarks` stocké dans le local storage. Si la clé `bookmarks` n'a pas encore été définie, ou si elle ne contient pas un tableau valide de favoris, la fonction `getBookmarks` doit renvoyer un tableau vide.
1. La clé `bookmarks` stockée dans le local storage doit être un tableau d'objets favoris. Chaque objet favori doit avoir trois clés : `name`, `category` et `url`.
1. Tu dois avoir une fonction nommée `displayOrCloseForm` qui bascule la classe `hidden` sur `#main-section` et `#form-section`.
1. Quand tu cliques sur `#add-bookmark-button`, tu dois mettre à jour le texte intérieur de `.category-name` avec la valeur de l'option sélectionnée de `#category-dropdown` et appeler `displayOrCloseForm` pour afficher la section du formulaire et masquer la section principale.
1. Quand tu cliques sur `#close-form-button`, tu dois exécuter ta fonction pour masquer la section du formulaire et afficher la section principale.
1. Quand tu cliques sur `#add-bookmark-button-form`, tu dois mettre à jour la clé `bookmarks` stockée dans le local storage en ajoutant un objet favori à la fin du tableau. L'objet doit avoir `name` défini à la valeur de l'input `#name`, `category` défini à la valeur de l'option sélectionnée du menu déroulant de catégorie, et `url` défini à la valeur de l'input `#url`.
1. Une fois la clé `bookmarks` mise à jour, tu dois réinitialiser la valeur de `#name` et `#url` à une chaîne vide avant d'exécuter ta fonction pour masquer la section du formulaire et afficher la section principale.
1. Tu dois avoir une fonction nommée `displayOrHideCategory` qui bascule la classe `hidden` sur `#main-section` et `#bookmark-list-section`.
1. Quand tu cliques sur `#view-category-button`, tu dois mettre à jour le texte intérieur de `.category-name` avec la valeur de l'option sélectionnée de `#category-dropdown`, modifier le HTML intérieur de `#category-list` conformément aux user stories ci-dessous, et appeler la fonction `displayOrHideCategory`.
1. Si aucun des favoris du local storage n'a la catégorie, tu dois définir le HTML intérieur de `#category-list` à un élément `p` avec le texte `No Bookmarks Found`.
1. Si un ou plusieurs favoris du local storage ont la catégorie sélectionnée, ajoute un bouton radio pour chaque favori au innerHTML de `#category-list`. Donne à chaque bouton radio un `id` et une `value` définis au nom du favori, et un `name` identique pour tous les boutons.
1. Chaque bouton radio doit avoir un label correspondant contenant un élément d'ancre avec le nom du favori et l'attribut `href` défini à l'URL du favori.
1. Quand tu cliques sur `#close-list-button`, tu dois exécuter ta fonction pour masquer `#bookmark-list-section` et afficher la section principale.
1. Quand tu cliques sur `#delete-bookmark-button`, tu dois supprimer le favori correspondant au bouton radio sélectionné et à la catégorie appropriée du local storage et mettre à jour la liste de favoris affichée.
...
Tu dois avoir une fonction `getBookmarks`.

```js
assert.isFunction(getBookmarks);
```

Ta fonction `getBookmarks` doit renvoyer un tableau.

```js
assert.isArray(getBookmarks());
```

Ta fonction `getBookmarks` doit renvoyer la clé `bookmarks` depuis `localStorage`.
...
Quand la clé `bookmarks` n'est pas définie dans `localStorage` ou est un tableau vide, la fonction `getBookmarks` doit renvoyer un tableau vide.
...
Quand la clé `bookmarks` dans le `localStorage` ne contient pas un tableau valide d'objets favoris, la fonction `getBookmarks` doit renvoyer un tableau vide.
...
Tu dois avoir une fonction nommée `displayOrCloseForm`.

```js
assert.isFunction(displayOrCloseForm);
```

Ta fonction `displayOrCloseForm` doit basculer la classe `hidden` sur `#main-section` et `#form-section`.
...
Quand tu cliques sur `#add-bookmark-button`, tu dois mettre à jour le texte intérieur de `.category-name` avec la valeur de l'option sélectionnée de `#category-dropdown`.
...
Quand tu cliques sur `#add-bookmark-button`, tu dois appeler `displayOrCloseForm` pour afficher la section du formulaire et masquer la section principale.
...
Quand tu cliques sur `#close-form-button`, tu dois appeler `displayOrCloseForm` pour masquer la section du formulaire et afficher la section principale.
...
Quand tu cliques sur `#add-bookmark-button-form`, tu dois mettre à jour la clé `bookmarks` stockée dans le local storage en ajoutant un objet à la fin du tableau. L'objet ajouté doit avoir `name` défini à la valeur de l'input `#name`, `category` défini à la valeur de l'option sélectionnée du menu déroulant de catégorie, et `url` défini à la valeur de l'input `#url`.
...
Quand tu cliques sur `#add-bookmark-button-form`, tu dois réinitialiser la valeur de `#name` et `#url` à une chaîne vide.
...
Quand tu cliques sur `#add-bookmark-button-form`, tu dois exécuter `displayOrCloseForm` pour masquer la section du formulaire et afficher la section principale.
...
Tu dois avoir une fonction nommée `displayOrHideCategory`.

```js
assert.isFunction(displayOrHideCategory);
```

Ta fonction `displayOrHideCategory` doit basculer la classe `hidden` sur `#main-section` et `#bookmark-list-section`.
...
Quand tu cliques sur `#view-category-button`, tu dois mettre à jour le texte intérieur de `.category-name` avec la valeur de l'option sélectionnée de `#category-dropdown`.
...
Quand tu cliques sur `#view-category-button`, tu dois ajouter un élément `p` avec le texte `No Bookmarks Found` au HTML intérieur de `#category-list` si aucun des favoris du local storage n'a la catégorie sélectionnée.
...
Quand tu cliques sur `#view-category-button`, tu dois modifier le HTML intérieur de l'élément `#category-list` en ajoutant un bouton radio. Le bouton radio doit avoir les attributs `id` et `value` définis au nom du favori pour chaque favori de la catégorie sélectionnée. De plus, chaque bouton radio doit avoir le même attribut `name`.
...
Chaque bouton radio ajouté au HTML intérieur de `#category-list` doit avoir un label correspondant contenant un élément d'ancre avec le nom du favori et l'attribut `href` défini à l'URL du favori.
...
Chaque élément `label` doit contenir un élément d'ancre avec le nom du favori comme texte, et l'attribut `href` défini à l'URL du favori.
...
Quand tu cliques sur `#close-list-button`, tu dois masquer `#bookmark-list-section` et afficher la section principale.
...
Quand tu cliques sur `#close-list-button` puis ouvres une catégorie quelconque, `#category-list` ne doit contenir que les données pertinentes pour la catégorie sélectionnée, sans dupliquer les entrées.
...
Quand tu cliques sur `#delete-bookmark-button`, tu dois supprimer le favori correspondant au bouton radio sélectionné et à la catégorie appropriée du local storage et mettre à jour la liste de favoris affichée.
