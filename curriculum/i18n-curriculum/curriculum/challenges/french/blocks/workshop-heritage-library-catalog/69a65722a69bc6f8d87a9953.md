...
CSV (Comma-Separated Values, valeurs séparées par des virgules) est un format utilisé pour les tableurs et l'import de données. Chaque ligne représente une entrée avec des champs séparés par des virgules.

Commence par supprimer l'appel `console.log(exportToJSON(...))`, car il n'était nécessaire que pour les tests. Puis crée une fonction nommée `exportToCSV` qui prend `catalog` comme paramètre. À l'intérieur, déclare `const header = "Title,Author,Year,Location"` et `const rows = []`.

# --hints--

Tu dois supprimer l'appel console.log de `exportToJSON`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

Tu dois avoir une fonction nommée `exportToCSV` avec un paramètre `catalog`.
...
Tu dois déclarer une variable nommée `header` avec la valeur `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Tu dois déclarer une variable nommée `rows` définie sur un tableau vide.
