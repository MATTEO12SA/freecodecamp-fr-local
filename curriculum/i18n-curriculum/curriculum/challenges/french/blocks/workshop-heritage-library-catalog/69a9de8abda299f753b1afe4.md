...
Ajoute une boucle `for` sur `catalog`. À l'intérieur de la boucle, déclare `const entry = catalog[i]`. Construis une chaîne de ligne avec les quatre champs séparés par des virgules et ajoute-la dans `rows`.

Les champs de type chaîne ont besoin de guillemets doubles littéraux dans la sortie (par exemple `"King, Stephen"`), pour que les virgules à l'intérieur des valeurs ne cassent pas le format CSV. Comme `year` est un nombre, il n'en a pas besoin.
...
Chaque ligne doit avoir les champs de l'entrée séparés par des virgules.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

Les champs de type chaîne doivent être entourés de guillemets doubles.
