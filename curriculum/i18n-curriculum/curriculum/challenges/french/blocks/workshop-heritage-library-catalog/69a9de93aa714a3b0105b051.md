...
Maintenant, combine toutes les lignes en une seule chaîne CSV que tu peux exporter ou partager.

Après la première boucle, déclare `let csv = header`. Ajoute une autre boucle `for` pour ajouter chaque ligne à `csv` avec un saut de ligne : `csv = csv + "\n" + rows[i]`. Retourne `csv`. Puis, en dessous de la fonction, affiche `exportToCSV(catalog)` pour prévisualiser la sortie formatée.

# --hints--

`exportToCSV` doit retourner une chaîne.

```js
assert.isString(exportToCSV(catalog));
```

La première ligne de la chaîne retournée doit être `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

La sortie doit contenir `"King, Stephen"` (entre guillemets pour gérer la virgule).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Tu dois afficher `exportToCSV(catalog)` dans la console.
