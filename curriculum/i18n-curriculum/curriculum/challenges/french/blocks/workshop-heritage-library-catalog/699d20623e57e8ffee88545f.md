...
L'affichage de `byDecade` a rempli son rôle. Supprime `console.log(byDecade)` de ton code.

Ensuite, pour rendre les entrées du catalogue plus faciles à parcourir, crée une fonction nommée `renderEntry` qui prend `entry` comme paramètre. Laisse le corps de la fonction vide pour l'instant. En dessous de la fonction, affiche `renderEntry(catalog[0])`.

# --hints--

`renderEntry` doit être une fonction.

```js
assert.isFunction(renderEntry);
```

`renderEntry` doit être une fonction avec un paramètre `entry`.
...
L'appel `console.log(byDecade)` doit être supprimé.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Tu dois afficher `renderEntry(catalog[0])`.
