...
La fonction `parseCard` prendra une chaîne brute de catalogue et retournera un objet structuré. Avant de développer ta fonction, mets en place un moyen de voir sa sortie au fur et à mesure que tu travailles.

Appelle `parseCard` avec `rawCatalogCards[2]` comme argument, affecte le résultat à une variable nommée `cardResult`, et affiche `cardResult` dans la console.

Pour l'instant, `parseCard` n'a pas de corps, donc elle retourne `undefined`. Au fur et à mesure que tu ajouteras du code à la fonction dans les prochaines étapes, la sortie affichée te montrera exactement ce que la fonction retourne à chaque étape.

# --hints--

Tu dois appeler `parseCard` avec `rawCatalogCards[2]` comme argument.

```js
assert.match(__helpers.removeJSComments(code), /parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)/);
```

Tu dois affecter le résultat de l'appel de `parseCard` avec `rawCatalogCards[2]` à une variable nommée `cardResult`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+cardResult\s*=\s*parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)\s*;?/);
```

Tu dois afficher `cardResult` dans la console.
