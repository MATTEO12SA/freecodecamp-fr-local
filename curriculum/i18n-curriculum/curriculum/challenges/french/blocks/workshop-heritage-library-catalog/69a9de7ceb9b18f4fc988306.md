...
À l'intérieur de la boucle, vérifie si l'auteur de l'entrée courante (converti en minuscules) inclut `searchTerm`. La méthode `.includes()` retourne `true` si une chaîne contient une autre chaîne comme sous-chaîne. Si la condition est vraie, ajoute l'entrée dans `results`. Après la boucle, retourne `results`.

# --hints--

`findByAuthor(catalog, "king").length` doit valoir `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` doit valoir `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` doit valoir `0`.
