...
La boucle est terminée. Retourne `grouped` depuis la fonction.

Puis, en dessous de la fonction, appelle `groupByDecade(catalog)` et affecte le résultat à une variable nommée `byDecade`. Affiche `byDecade` pour inspecter le résultat regroupé, et tu devrais voir un objet avec des clés comme `"1950s"`, `"1960s"`, `"1970s"`, et ainsi de suite, chacune pointant vers un tableau de livres de cette décennie.

# --hints--

`groupByDecade` doit retourner un objet.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` doit valoir `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` doit valoir `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Tu dois affecter le résultat de `groupByDecade(catalog)` à une variable nommée `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Tu dois afficher `byDecade` dans la console.
