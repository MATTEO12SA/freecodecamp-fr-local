...
Vérifions que la recherche fonctionne. Stephen King a six livres dans le catalogue.

Appelle `findByAuthor(catalog, "king")` et affecte le résultat à une variable nommée `kingBooks`. Affiche `kingBooks.length` pour confirmer le décompte.

Puis utilise une boucle `for` pour afficher chaque résultat. Utilise un littéral de gabarit pour inclure le titre et l'année, comme `` `${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

`kingBooks` doit avoir une longueur de `6`.

```js
assert.strictEqual(kingBooks.length, 6);
```

Tu dois afficher `kingBooks.length` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

Tu dois utiliser une boucle `for` pour afficher le titre et l'année de chaque livre.
