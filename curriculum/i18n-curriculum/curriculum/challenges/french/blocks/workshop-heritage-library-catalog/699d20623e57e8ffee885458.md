...
Le champ year arrive sous forme de chaîne, mais tu en as besoin sous forme de nombre pour un calcul ultérieur. Utilise `parseInt()` pour le convertir.

Le year peut aussi être vide, donc tu as besoin d'une valeur de repli. L'opérateur ternaire `condition ? valueIfTrue : valueIfFalse` retourne l'une de deux valeurs en fonction d'une condition.

Mets à jour la propriété `year` dans l'instruction return pour :
...
Lorsqu'elle est appelée avec une fiche ayant une année valide, la propriété `year` de l'objet retourné doit être un nombre, pas une chaîne.

```js
assert.isNumber(parseCard("The Shining | King, Stephen | 1977 | Shelf K1").year);
```

Lorsqu'elle est appelée avec `"The Shining | King, Stephen | 1977 | Shelf K1"`, la propriété `year` doit être `1977`.

```js
assert.strictEqual(parseCard("The Shining | King, Stephen | 1977 | Shelf K1").year, 1977);
```

Lorsqu'elle est appelée avec une chaîne ayant un champ year vide, la propriété `year` doit être `"Unknown"`.
