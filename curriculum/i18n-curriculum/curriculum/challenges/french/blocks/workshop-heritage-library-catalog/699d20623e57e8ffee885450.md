...
Le reste des fiches de catalogue a été ajouté. Remarque les trois entrées proches de la fin auxquelles il manque des champs ; tu les géreras dans une étape ultérieure.

`parseCard` prendra une chaîne brute de catalogue et retournera un objet structuré avec quatre propriétés : `title`, `author`, `year` et `location`. Pour l'instant, crée une fonction `parseCard` avec un paramètre nommé `rawString`.

# --hints--

Tu dois avoir une fonction nommée `parseCard`.

```js
assert.isFunction(parseCard);
```

`parseCard` doit être une fonction avec un paramètre nommé `rawString`.
