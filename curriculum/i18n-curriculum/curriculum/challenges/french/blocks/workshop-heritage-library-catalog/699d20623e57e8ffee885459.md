...
`parseCard` a été testée et fonctionne correctement. La variable `cardResult` n'est plus nécessaire.

Commence par supprimer la ligne `const cardResult` et la ligne `console.log(cardResult)`. Puis crée une fonction nommée `parseCatalog` qui prend `rawCards` comme paramètre. Pour l'instant, déclare un tableau vide nommé `catalog` à l'intérieur de la fonction et retourne-le.

En dessous de la fonction, appelle `parseCatalog(rawCatalogCards)`, affecte le résultat à une variable nommée `catalog`, et affiche `catalog.length`.

# --hints--

`parseCatalog` doit être une fonction qui retourne un tableau.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

`catalog` doit être défini et défini sur `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

Tu dois supprimer la variable `cardResult` et l'appel `console.log()` associé.
