...
`parseCatalog` retourne actuellement un tableau vide. Pour le remplir, ajoute une boucle `for` à l'intérieur de la fonction qui itère sur `rawCards`. À l'intérieur de la boucle, appelle `parseCard` sur chaque élément et ajoute le résultat dans `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` doit valoir `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` doit utiliser une boucle `for` pour itérer sur `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Tu dois appeler `parseCard` à l'intérieur de la boucle.
