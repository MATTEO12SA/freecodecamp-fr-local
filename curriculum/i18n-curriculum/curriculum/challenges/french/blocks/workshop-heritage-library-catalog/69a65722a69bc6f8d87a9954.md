...
Terminons par un résumé. Affiche `catalog.length` et `Object.keys(byDecade).length` pour voir le total des livres et le nombre de groupes par décennie.

Déclare `let oldestYear = Infinity` et `let newestYear = 0`. Utilise une boucle `for` sur `catalog` : pour chaque entrée dont l'année n'est pas `"Unknown"`, mets à jour `oldestYear` si l'année est plus petite et `newestYear` si elle est plus grande. Affiche les deux valeurs.

Tu as terminé le workshop Heritage Library Catalog !

# --hints--

Tu dois afficher `catalog.length` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*catalog\.length\s*\)/);
```

Tu dois afficher `Object.keys(byDecade).length` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*Object\.keys\s*\(\s*byDecade\s*\)\.length\s*\)/);
```

Tu dois utiliser une boucle `for` sur `catalog` pour trouver l'année la plus ancienne et la plus récente.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

L'année la plus ancienne du catalogue doit être `850`.

```js
assert.strictEqual(oldestYear, 850);
```

L'année la plus récente du catalogue doit être `2011`.

```js
assert.strictEqual(newestYear, 2011);
```

Tu dois afficher `oldestYear` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*oldestYear\s*\)/);
```

Tu dois afficher `newestYear` dans la console.
