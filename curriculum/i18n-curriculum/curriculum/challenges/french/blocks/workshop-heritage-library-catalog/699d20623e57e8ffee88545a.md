...
Une fois le catalogue analysé, tu peux le rechercher par champ. `findByAuthor` filtrera les entrées dont l'auteur contient un terme de recherche.

Crée une fonction nommée `findByAuthor` qui prend `catalog` et `author` comme paramètres. À l'intérieur, déclare une variable nommée `searchTerm` et définis-la sur la version en minuscules de `author` pour permettre plus tard une correspondance insensible à la casse.

Déclare aussi une variable nommée `results` et affecte-lui un tableau vide pour contenir les correspondances.

# --hints--

Tu dois avoir une fonction nommée `findByAuthor` avec `catalog` et `author` comme paramètres.
...
Tu dois déclarer une variable nommée `searchTerm` définie sur `author.toLowerCase()`.

```js
assert.match(findByAuthor.toString(), /searchTerm\s*=\s*author\.toLowerCase\s*\(\s*\)/);
```

Tu dois déclarer une variable nommée `results` définie sur un tableau vide.
