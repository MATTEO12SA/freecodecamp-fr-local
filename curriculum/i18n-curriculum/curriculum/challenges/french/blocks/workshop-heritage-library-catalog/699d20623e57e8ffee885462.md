...
Voyons le validateur détecter un vrai problème. `catalog[0]` est « From a Buick 8 » de Stephen King, dont les quatre champs sont remplis. `catalog[16]` est « Unknown Title », dont le champ author est vide.

Affiche `validateEntry(catalog[0])` et `validateEntry(catalog[16])`. Le premier doit retourner `true` ; le second doit retourner `false`.

# --hints--

Tu dois afficher `validateEntry(catalog[0])` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Tu dois afficher `validateEntry(catalog[16])` dans la console.
