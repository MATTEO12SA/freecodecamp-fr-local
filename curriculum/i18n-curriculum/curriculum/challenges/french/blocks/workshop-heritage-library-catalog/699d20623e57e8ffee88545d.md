...
Après la vérification `"Unknown"`, ajoute la logique de regroupement par décennie pour les livres ayant des années numériques. Déclare `const decade = Math.floor(book.year / 10) * 10`. Par exemple, `Math.floor(1977 / 10)` donne `197`, fois `10` donne `1970`.

Puis déclare `const decadeKey =` `` `${decade}s` `` pour produire une chaîne comme `"1970s"`.

Utilise la notation par crochets pour regrouper le livre : vérifie si `grouped[decadeKey]` n'existe pas encore et initialise-le comme un tableau vide, puis ajoute `book` dedans. La notation par crochets est requise ici parce que la clé est stockée dans une variable.

# --hints--

Tu dois déclarer une variable nommée `decade` définie sur `Math.floor(book.year / 10) * 10`.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=\s*Math\.floor\s*\(\s*book\.year\s*\/\s*10\s*\)\s*\*\s*10/);
```

Tu dois déclarer une variable nommée `decadeKey` qui inclut `decade` et la lettre `"s"`.
...
Tu dois vérifier si `grouped[decadeKey]` n'existe pas encore et l'initialiser comme un tableau vide.
...
Tu dois ajouter `book` dans `grouped[decadeKey]`.
