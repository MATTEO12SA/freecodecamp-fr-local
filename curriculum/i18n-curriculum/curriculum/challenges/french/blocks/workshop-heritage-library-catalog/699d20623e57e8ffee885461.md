...
Le même schéma s'applique aux trois autres champs. Ajoute des vérifications équivalentes pour `author`, `year` et `location`.

Note que `isValid` s'accumule sur les quatre vérifications : si un champ échoue, il devient `false` et reste `false` même si les vérifications restantes réussissent. Cela signifie que la fonction inspecte toujours chaque champ au lieu de s'arrêter au premier problème.

# --hints--

`validateEntry(catalog[0])` doit retourner `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` doit retourner `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` doit retourner `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` doit retourner `false` car la location est `"Unknown"`.
