...
Maintenant, remplis `renderEntry`. Déclare une constante de repli pour chaque champ : `const title = entry.title || "Unknown"`, et applique le même schéma pour `author`, `year` et `location`.

Retourne un littéral de gabarit multi-lignes qui formate l'entrée comme une fiche de bibliothèque. Utilise `"-".repeat(25)` pour les bordures du haut et du bas, et inclus des lignes étiquetées pour `Title`, `Author`, `Year` et `Location`. Par exemple, une ligne étiquetée pour le titre ressemblerait à `Title: ${title}`.

# --hints--

`renderEntry` doit retourner une chaîne.

```js
assert.isString(renderEntry(catalog[0]));
```

Lorsqu'elle est appelée avec `catalog[0]`, la chaîne retournée doit contenir `"From a Buick 8"`.

```js
assert.include(renderEntry(catalog[0]), "From a Buick 8");
```

Lorsqu'elle est appelée avec `catalog[0]`, la chaîne retournée doit contenir `"King, Stephen"`.

```js
assert.include(renderEntry(catalog[0]), "King, Stephen");
```

Lorsqu'elle est appelée avec `catalog[0]`, la chaîne retournée doit contenir l'année `2002`.

```js
assert.include(renderEntry(catalog[0]), "2002");
```

Lorsqu'elle est appelée avec `catalog[0]`, la chaîne retournée doit contenir `"Shelf K7"`.

```js
assert.include(renderEntry(catalog[0]), "Shelf K7");
```

La chaîne retournée doit inclure une bordure en haut et en bas en utilisant `"-".repeat(25)`.
