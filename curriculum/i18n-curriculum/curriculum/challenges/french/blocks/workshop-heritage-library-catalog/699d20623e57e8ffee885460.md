...
Avant de faire confiance aux données analysées, commence par valider les champs requis.

Crée une fonction nommée `validateEntry` qui prend `entry` comme paramètre. Déclare `let isValid = true`.

Ajoute une vérification de validation pour `title` : définis `isValid = false` si `title` est manquant, falsy, ou égal à `"Unknown"`.

Retourne `isValid`. Tu ajouteras les vérifications pour les autres champs dans les prochaines étapes.

# --hints--

`validateEntry` doit retourner un booléen.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` doit retourner `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` doit retourner `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` doit retourner `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` doit retourner `false`.
