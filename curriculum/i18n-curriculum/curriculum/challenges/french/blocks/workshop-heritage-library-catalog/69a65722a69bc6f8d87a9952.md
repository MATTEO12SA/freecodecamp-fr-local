...
JSON (JavaScript Object Notation) est un format standard pour partager des données entre des systèmes. `JSON.stringify` convertit une valeur JavaScript en une chaîne au format JSON.

Commence par supprimer les deux appels console.log de `validateEntry`, car ils n'étaient nécessaires que pour les tests. Puis crée une fonction nommée `exportToJSON` qui prend `catalog` comme paramètre. Retourne `JSON.stringify(catalog, null, 2)`. Le premier argument est la donnée à convertir. Le deuxième argument (`null`) signifie aucun filtrage personnalisé. Le troisième (`2`) ajoute une indentation de deux espaces, ce qui rend la sortie lisible.

En dessous de la fonction, affiche `exportToJSON(catalog.slice(0, 2))` pour prévisualiser la sortie JSON des deux premières entrées.

# --hints--

Tu dois supprimer les appels console.log de `validateEntry`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON` doit retourner une chaîne.

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))` doit produire un tableau valide.

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

Tu dois utiliser `JSON.stringify` avec `null` et `2` comme deuxième et troisième arguments.

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

Tu dois afficher `exportToJSON(catalog.slice(0, 2))` dans la console.
