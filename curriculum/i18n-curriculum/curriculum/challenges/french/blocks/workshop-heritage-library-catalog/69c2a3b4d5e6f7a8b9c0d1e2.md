...
Le code de test `kingBooks` a confirmé que `findByAuthor` fonctionne correctement. Avant de passer à `groupByDecade`, nettoie la sortie de la console en la supprimant.

Supprime la ligne `const kingBooks = ...`, `console.log(kingBooks.length)`, et la boucle `for` qui affiche les titres et les années des livres.

# --hints--

La variable `kingBooks`, l'instruction `console.log(kingBooks.length)` et la boucle `for` doivent toutes être supprimées.
