...
Maintenant, tu vas construire `groupByDecade`, qui organise le catalogue par décennie. Chaque clé sera une chaîne comme `"1970s"` et chaque valeur un tableau de livres de cette période.

Crée une fonction nommée `groupByDecade` qui prend `catalog` comme paramètre. À l'intérieur, déclare `const grouped = {}`. Ajoute une boucle `for` pour itérer sur `catalog`, et à l'intérieur de la boucle déclare `const book = catalog[i]`.

# --hints--

Tu dois avoir une fonction nommée `groupByDecade` avec un paramètre `catalog`.
...
Tu dois déclarer une variable nommée `grouped` définie sur un objet vide.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+grouped\s*=\s*\{\s*\}/);
```

Tu dois utiliser une boucle `for` pour itérer sur `catalog`.

```js
assert.match(groupByDecade.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Tu dois déclarer une variable nommée `book` définie sur l'entrée courante du catalogue.
