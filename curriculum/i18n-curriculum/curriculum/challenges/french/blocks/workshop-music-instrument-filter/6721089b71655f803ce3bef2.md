...
Ajoute maintenant quatre options à ton menu déroulant : `All`, `Woodwinds`, `Brass` et `Percussion`, dans cet ordre. Pour chaque option, utilise la version en minuscules pour les attributs `id` et `value`.

# --hints--

Tu devrais avoir quatre éléments `option` dans ton élément `select`.

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

Ton premier élément `option` devrait avoir le texte `All`.

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

Ton option `All` devrait avoir un attribut `id` et un attribut `value` avec la valeur `all`.
...
Ton deuxième élément `option` devrait avoir le texte `Woodwinds`.

```js
assert.equal(document.querySelectorAll("select>option")[1].innerText,"Woodwinds");
```

Ton option `Woodwinds` devrait avoir un attribut `id` et un attribut `value` avec la valeur `woodwinds`.
...
Ton troisième élément `option` devrait avoir le texte `Brass`.

```js
assert.equal(document.querySelectorAll("select>option")[2].innerText,"Brass");
```

Ton option `Brass` devrait avoir un attribut `id` et un attribut `value` avec la valeur `brass`.
...
Ton quatrième élément `option` devrait avoir le texte `Percussion`.

```js
assert.equal(document.querySelectorAll("select>option")[3].innerText,"Percussion");
```

Ton option `Percussion` devrait avoir un attribut `id` et un attribut `value` avec la valeur `percussion`.
