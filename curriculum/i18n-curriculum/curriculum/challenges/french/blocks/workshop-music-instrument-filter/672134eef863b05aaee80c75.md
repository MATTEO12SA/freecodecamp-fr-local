...
Pour implémenter la fonctionnalité de filtre, tu auras besoin d'une fonction. Déclare une fonction vide nommée `instrumentCards` qui prend un seul paramètre.

# --hints--

Tu devrais avoir une fonction nommée `instrumentCards`.

```js
assert.isFunction(instrumentCards);
```

Ta fonction `instrumentCards` devrait prendre un seul paramètre.
