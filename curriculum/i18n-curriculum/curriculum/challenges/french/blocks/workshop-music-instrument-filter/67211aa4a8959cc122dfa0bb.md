...
Modifie ton appel `console.log` pour afficher la valeur sélectionnée dans le menu déroulant.

Tu peux accéder à la valeur actuelle d'un élément `select` avec sa propriété `.value`. Par exemple :
...
Tu devrais afficher la valeur sélectionnée du menu déroulant quand un événement `change` est déclenché sur `selectContainer`.
