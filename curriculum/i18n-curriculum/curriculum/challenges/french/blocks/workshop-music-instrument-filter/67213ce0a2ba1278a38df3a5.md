...
Retire ton `console.log` de l'écouteur d'événement et définis le HTML interne de `productsContainer` avec le résultat de l'appel à `instrumentCards` avec l'option de catégorie sélectionnée.

Ensuite, sélectionne différentes options dans ton menu déroulant et regarde le résultat dans la fenêtre d'aperçu.
...
Quand l'option du menu déroulant change, tu devrais définir le HTML interne de `productsContainer` avec le résultat de la fonction `instrumentCards` appelée avec l'option sélectionnée en argument.
...
Tu devrais retirer `console.log(instrumentCards(selectContainer.value))` du callback de ton écouteur d'événement.
