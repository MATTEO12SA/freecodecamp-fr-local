...
Maintenant, ajoute un sélecteur `.logo` à la requête `@media`, et définis la propriété `width` sur `150px`.

# --hints--

Ta règle `@media` devrait avoir un sélecteur `.logo`.
...
Ton nouveau sélecteur `.logo` devrait avoir une propriété `width` de `150px`.
