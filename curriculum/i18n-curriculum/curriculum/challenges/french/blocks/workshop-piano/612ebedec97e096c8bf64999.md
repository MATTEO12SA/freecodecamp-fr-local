...
Dans la requête `@media`, ajoute un sélecteur `.keys` et définis `width` sur `318px`.

# --hints--


Ta règle `@media` devrait avoir un sélecteur `.keys`.
...
Ton nouveau sélecteur `.keys` devrait avoir une propriété `width` de `318px`.
