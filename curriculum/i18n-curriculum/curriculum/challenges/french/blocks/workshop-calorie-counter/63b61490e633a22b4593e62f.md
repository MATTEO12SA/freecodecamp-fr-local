...
En suivant le même schéma, assigne ton élément `#add-entry` à `addEntryButton`, ton élément `#clear` à `clearButton`, et ton élément `#output` à `output`.

# --hints--

Tu devrais déclarer une variable appelée `addEntryButton`.

```js
assert.isDefined(addEntryButton);
```

Tu devrais utiliser `document.getElementById()` pour récupérer l'élément `#add-entry`.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

Tu devrais assigner l'élément `#add-entry` à `addEntryButton`.

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

Tu devrais déclarer une variable appelée `clearButton`.

```js
assert.isDefined(clearButton);
```

Tu devrais utiliser `document.getElementById()` pour récupérer l'élément `#clear`.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

Tu devrais assigner l'élément `#clear` à `clearButton`.

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

Tu devrais déclarer une variable appelée `output`.
...
Tu devrais utiliser `document.getElementById()` pour récupérer l'élément `#output`.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

Tu devrais assigner l'élément `#output` à `output`.
