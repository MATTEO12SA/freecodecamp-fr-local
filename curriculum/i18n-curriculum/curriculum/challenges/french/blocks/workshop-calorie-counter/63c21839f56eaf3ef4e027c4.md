...
Crée un autre élément `label` (sur une nouvelle ligne) à la fin de ton `HTMLString`. Ce `label` devrait avoir le texte `Entry # Calories`, en utilisant ta syntaxe de littéral de modèle pour remplacer `#` par la valeur de `entryNumber`, et l'attribut `for` défini à `X-#-calories`, où `X` est la `value` de `entryDropdown` et `#` est la valeur de `entryNumber`.

# --hints--

Tu devrais avoir deux éléments `label` dans ton `HTMLString`.
...
Ton nouvel élément `label` devrait être sur une nouvelle ligne.
...
Ton nouvel élément `label` devrait venir après ton élément `input`.
...
Ton nouvel élément `label` devrait avoir un attribut `for` défini à `${entryDropdown.value}-${entryNumber}-calories`.
...
Ton nouvel élément `label` devrait avoir le texte `Entry ${entryNumber} Calories`.
...
Tu ne devrais pas modifier tes éléments existants.
