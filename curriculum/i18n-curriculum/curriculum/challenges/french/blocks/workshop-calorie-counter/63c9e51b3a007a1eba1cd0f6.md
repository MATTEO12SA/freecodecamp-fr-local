...
Maintenant déclare une variable `remainingCalories`, et donne-lui la valeur de la soustraction de `consumedCalories` de `budgetCalories` et de l'ajout de `exerciseCalories`.

# --hints--

Ta fonction `calculateCalories` devrait avoir une variable `remainingCalories`.

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

Tu devrais trouver la valeur de `budgetCalories - consumedCalories + exerciseCalories`.
...
Tu devrais assigner la valeur de `budgetCalories - consumedCalories + exerciseCalories` à `remainingCalories`.
