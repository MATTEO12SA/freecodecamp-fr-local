...
En utilisant la même syntaxe d'interpolation, ajoute un deuxième élément `p` avec le texte `consumedCalories Calories Consumed` et un troisième avec le texte `exerciseCalories Calories Burned`. Souviens-toi de remplacer tes variables `consumedCalories` et `exerciseCalories` par les valeurs appropriées.

# --hints--

Tu devrais ajouter un deuxième élément `p` à ton littéral de modèle.
...
Ton deuxième élément `p` devrait être sur une nouvelle ligne.
...
Ton deuxième élément `p` devrait avoir le texte `${consumedCalories} Calories Consumed`.
...
Tu devrais ajouter un troisième élément `p` à ton littéral de modèle.
...
Ton troisième élément `p` devrait avoir le texte `${exerciseCalories} Calories Burned`.
