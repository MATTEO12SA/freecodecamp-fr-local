...
En continuant le même schéma, crée un `fieldset` pour `dinner` avec les mêmes éléments imbriqués.

# --hints--

Tu devrais créer un troisième élément `fieldset` dans ton `form`.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]);
```

Ton troisième élément `fieldset` devrait venir après ton deuxième élément `fieldset`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.previousElementSibling?.tagName, "FIELDSET");
```

Ton troisième élément `fieldset` devrait avoir un `id` défini à `dinner`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.id, "dinner");
```

Ton troisième élément `fieldset` devrait contenir un élément `legend`.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('legend'));
```

Ton nouvel élément `legend` devrait avoir le texte `Dinner`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Dinner");
```

Ton troisième élément `fieldset` devrait contenir un élément `div`.
