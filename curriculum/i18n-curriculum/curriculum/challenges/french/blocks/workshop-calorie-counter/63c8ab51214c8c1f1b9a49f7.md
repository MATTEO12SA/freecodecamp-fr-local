...
Dans ta nouvelle fonction, déclare une variable `calories` et assigne-lui la valeur `0`. Utilise `let` pour la déclarer, car tu la réassigneras plus tard.

# --hints--

Tu devrais déclarer une variable `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Ta variable `calories` devrait être assignée à la valeur `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Tu devrais déclarer ta variable `calories` avec `let`.
