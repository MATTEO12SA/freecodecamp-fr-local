...
En suivant le même schéma, interroge tes saisies `number` dans les éléments `#dinner`, `#snacks`, et `#exercise`. Assigne-les à des variables en suivant le schéma de nommage des deux précédentes.

# --hints--

Tu devrais déclarer une variable `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Tu devrais appeler `document.querySelectorAll()` avec `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Tu devrais assigner le résultat de ton appel `document.querySelectorAll()` à `dinnerNumberInputs`.
...
Tu devrais déclarer une variable `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Tu devrais appeler `document.querySelectorAll()` avec `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Tu devrais assigner le résultat de ton appel `document.querySelectorAll()` à `snacksNumberInputs`.
...
Tu devrais déclarer une variable `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Tu devrais appeler `document.querySelectorAll()` avec `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Tu devrais assigner le résultat de ton appel `document.querySelectorAll()` à `exerciseNumberInputs`.
