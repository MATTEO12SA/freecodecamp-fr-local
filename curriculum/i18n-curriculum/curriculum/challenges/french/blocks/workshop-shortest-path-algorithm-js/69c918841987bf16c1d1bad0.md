...
La fonction doit renvoyer les données calculées pour que les appelants puissent les utiliser de façon programmatique.

Après la boucle `for...of`, ajoute une instruction `return` qui renvoie un tableau contenant `distances` et `paths`, dans cet ordre.

# --hints--

Tu dois renvoyer `[distances, paths]` après la boucle `for...of`.
