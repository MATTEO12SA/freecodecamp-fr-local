...
Une fois que tu as confirmé qu'un nœud valide a été trouvé, marque-le comme visité pour que l'algorithme ne le traite plus.

Après l'instruction `if` avec la condition `current === -1`, définis `visited[current]` à `true`.

# --hints--

Tu dois définir `visited[current]` à `true`.
