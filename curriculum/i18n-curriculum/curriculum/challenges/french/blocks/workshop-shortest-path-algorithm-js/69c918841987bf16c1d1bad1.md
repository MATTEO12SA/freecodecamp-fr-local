...
La fonction est complète. Appelle-la maintenant pour voir les résultats.

En bas du fichier (en dehors de la fonction), appelle `shortestPath` avec `adjMatrix` comme matrice, `0` comme nœud de départ, et `5` comme nœud cible.

Cela affichera le plus court chemin du nœud `0` au nœud `5` dans la matrice d'adjacence.

Avec cela, ton atelier sur l'algorithme du plus court chemin est terminé !

# --hints--

Tu dois appeler la fonction `shortestPath` avec `adjMatrix, 0, 5`.
