...
Pour la dernière étape, tu dois rendre le panneau visible quand son onglet associé est cliqué. Tu peux le faire en définissant la propriété `hidden` du `panel` à `false`.

Clique sur n'importe lequel des onglets, et tu devrais voir le contenu du panneau correspondant s'afficher.

Et avec ce dernier changement, ton workshop est terminé !

# --hints--

Quand un onglet est cliqué, tu devrais définir la propriété `hidden` du panneau à `false`.
