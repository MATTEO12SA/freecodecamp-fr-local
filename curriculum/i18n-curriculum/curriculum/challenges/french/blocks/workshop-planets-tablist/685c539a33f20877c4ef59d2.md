...
Il est maintenant temps de travailler sur l'affichage du bon panneau quand un onglet est cliqué. Chacun des panneaux a un `id` qui correspond à l'attribut `aria-controls` de l'onglet correspondant. Par exemple, le premier panneau a un `id` de `panel-earth`, qui correspond à la valeur de l'attribut `aria-controls` du premier onglet, qui est `panel-earth`.

Crée une variable appelée `associatedPanel` et assigne-lui la valeur `tab.getAttribute("aria-controls")`. Cela te donnera l'`id` du panneau qui correspond à l'onglet cliqué.

# --hints--

Tu devrais avoir une variable appelée `associatedPanel` qui contient l'`id` du panneau associé à l'onglet cliqué.
