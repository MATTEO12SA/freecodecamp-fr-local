...
Pour l'instant, les trois faits sont visibles en même temps. Mais un seul fait devrait être visible à la fois.

Pour y parvenir, tu dois définir l'attribut `hidden` sur les deuxième et troisième éléments `tabpanel`.

# --hints--

Ton deuxième élément `tabpanel` devrait avoir l'attribut `hidden`.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

Ton troisième élément `tabpanel` devrait avoir l'attribut `hidden`.
