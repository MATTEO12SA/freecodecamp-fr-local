...
Dans des modules précédents, tu as appris à parcourir une `NodeList` comme ceci :
...
Utilise la méthode `forEach` pour itérer sur les `tabs`. Pour la fonction de rappel, utilise une fonction fléchée qui prend un seul paramètre, `tab`.

# --hints--

Tu devrais avoir un `tabs.forEach(tab => {})` dans ton code.
