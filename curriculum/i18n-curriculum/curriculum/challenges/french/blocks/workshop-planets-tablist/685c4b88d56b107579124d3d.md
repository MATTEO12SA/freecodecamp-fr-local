...
Si tu regardes le HTML, tu verras que les deuxième et troisième panneaux sont masqués. Cela se fait avec l'attribut `hidden`. Le premier panneau est visible par défaut. Quand un utilisateur clique sur un onglet, tous les panneaux devraient être masqués.

Utilise la méthode `forEach` pour parcourir les `panels` et définir l'attribut `hidden` de chaque panneau à `true`.

# --hints--

Tu devrais parcourir les `panels` et définir l'attribut `hidden` de chaque panneau à `true`.
