...
Comme `associatedPanel` est un `id`, cela te permettra d'interroger le document et de trouver l'élément panneau correspondant.

Crée une nouvelle variable appelée `panel` et assigne-lui la valeur de `document.getElementById(associatedPanel)`.

# --hints--

Tu devrais avoir une variable appelée `panel` qui contient l'élément panneau associé à l'onglet cliqué.
