...
Il est maintenant temps d'associer les panneaux d'onglets à leurs onglets respectifs.

Pour le premier élément `tabpanel`, ajoute l'attribut `id` avec la valeur `"panel-earth"` et un attribut `aria-labelledby` avec la valeur `"tab-earth"`.

# --hints--

Ton premier élément `tabpanel` devrait avoir un attribut `id`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

Ton attribut `id` devrait avoir la valeur `"panel-earth"`.

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

Ton premier élément `tabpanel` devrait avoir un attribut `aria-labelledby`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

Ton attribut `aria-labelledby` devrait avoir la valeur `"tab-earth"`.
