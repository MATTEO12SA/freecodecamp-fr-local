...
Il est maintenant temps de définir l'onglet cliqué comme sélectionné.

Utilise la méthode `setAttribute` sur le `tab` pour définir l'attribut `aria-selected` à `"true"`.

# --hints--

Quand un `tab` est cliqué, l'attribut `aria-selected` de l'onglet cliqué devrait être défini à `"true"`.
