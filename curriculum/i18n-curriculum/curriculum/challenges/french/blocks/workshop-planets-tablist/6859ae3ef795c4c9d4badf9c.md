...
Il est maintenant temps d'ajouter la fonctionnalité à la tablist. La première étape est d'accéder à tous les éléments d'onglet et de panneau de la tablist.

Crée une variable appelée `tabs` et assigne-lui le résultat d'une requête pour tous les éléments avec l'attribut `role` défini à `tab`.

Ensuite, crée une variable appelée `panels` et assigne-lui le résultat d'une requête pour tous les éléments avec l'attribut `role` défini à `tabpanel`.

Souviens-toi que tu peux utiliser la méthode `querySelectorAll` pour sélectionner plusieurs éléments à la fois.

# --hints--

Tu devrais avoir une variable appelée `tabs`.

```js
assert.isDefined(tabs);
```

Tu devrais assigner `document.querySelectorAll('[role="tab"]')` à la variable `tabs`.
...
Tu devrais avoir une variable appelée `panels`.

```js
assert.isDefined(panels);
```

Tu devrais assigner `document.querySelectorAll('[role="tabpanel"]')` à la variable `panels`.
