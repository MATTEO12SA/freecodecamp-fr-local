...
Chaque `tab` représente un des boutons de la liste d'onglets. L'utilisateur devrait pouvoir cliquer sur un onglet pour afficher le panneau correspondant et masquer les autres.

Dans des modules précédents, tu as appris à attacher un écouteur d'événement `"click"` à chaque bouton d'une `NodeList` comme ceci :
...
Utilise la méthode `addEventListener` pour attacher un écouteur d'événement `"click"` à chaque bouton `tab`. Le deuxième argument de la méthode `addEventListener` devrait être une fonction fléchée vide `() => {}` pour l'instant.

# --hints--

Tu devrais avoir un `tab.addEventListener('click', () => {})` à l'intérieur de la boucle `forEach`.
