...
Maintenant, tu vas travailler sur une fonction pour retirer un nœud de la liste.

Crée une fonction `remove` avec les paramètres `list` et `element`.

# --hints--

Tu dois créer une fonction `remove`.

```js
assert.isFunction(remove)
```

Ta fonction `remove` doit avoir les paramètres `list` et `element`.
