...
Pour l'instant, à chaque fois que l'utilisateur édite une note et clique à l'extérieur, la variable `currentContent` est mise à jour. Mais que se passe-t-il si l'utilisateur focalise la note et décide de partir sans rien modifier ?

Dans cette situation, tu peux ajouter une condition pour gérer ce cas.

Au-dessus de ta ligne `currentContent = newContent;`, ajoute une instruction `if` pour vérifier si `currentContent` est égal à `newContent`. Si c'est le cas, fais un return.

# --hints--

Tu devrais ajouter une instruction `if` pour vérifier si `currentContent` est égal à `newContent`. Si c'est le cas, fais un return.
