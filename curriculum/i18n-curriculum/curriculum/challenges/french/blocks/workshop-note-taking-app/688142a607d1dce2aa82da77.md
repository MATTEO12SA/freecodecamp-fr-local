...
Ce serait bien de voir les modifications éditées dans la console.

Ajoute un `console.log(currentContent);`.

Pour tester tes changements, essaie d'éditer la note puis de cliquer à l'extérieur. Tu devrais maintenant voir la note éditée dans la console.

# --hints--

Tu devrais ajouter un `console.log(currentContent);` à l'intérieur de l'écouteur d'événement, après avoir assigné `newContent` à `currentContent`.
