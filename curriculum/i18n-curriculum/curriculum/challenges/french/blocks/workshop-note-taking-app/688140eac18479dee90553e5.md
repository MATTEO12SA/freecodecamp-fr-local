...
Comme tu utilises un autre élément `div`, tu devras le rendre plus accessible pour les lecteurs d'écran.

Dans une leçon précédente, tu as appris l'attribut `aria-live`, qui sert à créer une région live sur une page. Cela permet aux utilisateurs de lecteurs d'écran d'être notifiés automatiquement quand le contenu de la région live change, sans avoir à la focaliser ni à interagir avec elle manuellement.

Dans ton élément `#status`, ajoute un attribut `aria-live` et définis sa valeur à `"polite"`.

La valeur `polite` dira aux lecteurs d'écran d'attendre la fin de toute annonce en cours, ou que l'utilisateur arrête de taper, avant d'annoncer la mise à jour.

# --hints--

Ton élément `#status` devrait avoir l'attribut `aria-live`.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

Ton élément `#status` devrait avoir l'attribut `aria-live` défini à `"polite"`.
