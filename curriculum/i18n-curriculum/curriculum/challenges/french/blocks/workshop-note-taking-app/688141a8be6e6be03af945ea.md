...
Quand un utilisateur édite la note, tu veux suivre la version éditée.

Utilise `let` pour créer une variable appelée `currentContent` et assigne-lui une chaîne vide.

*NOTE* : Dans une application réelle, tu enregistrerais normalement tes notes dans une base de données. Ou tu pourrais même les enregistrer dans le local storage. Cependant, travailler avec les bases de données et le local storage dépasse le cadre de ce workshop, et ces concepts seront enseignés plus tard.

# --hints--

Tu devrais utiliser `let` pour créer la variable `currentContent`.

```js
assert.match(code, /let\s+currentContent/);
```

Ta variable `currentContent` devrait être une chaîne.

```js
assert.isString(currentContent);
```

Ta variable `currentContent` devrait être initialisée avec une chaîne vide.
