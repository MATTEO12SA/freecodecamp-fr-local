...
Comme un élément `div` est utilisé pour la note, les lecteurs d'écran ne comprendront pas son rôle. Tu peux corriger ça en ajoutant un attribut `aria-label`. Cela améliorera l'accessibilité et donnera un sens à cet élément `div`.

Dans ta balise `div` ouvrante, ajoute un attribut `aria-label` et définis sa valeur à `"Note editor"`.

# --hints--

Ta balise `div` ouvrante devrait avoir l'attribut `aria-label`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

Ta balise `div` ouvrante devrait avoir l'attribut `aria-label` défini à `"Note editor"`.
