...
Dans ce workshop, tu vas t'entraîner à utiliser les attributs `aria-live` et `aria-label` en construisant une application de prise de notes accessible.

La majeure partie du HTML et tout le CSS t'ont été fournis. Les premières étapes consisteront à ajouter le markup restant.

Si tu regardes l'aperçu, un texte te dit de cliquer sur la carte pour l'éditer. Mais si tu essaies, ça ne marchera pas. Pour rendre cet élément `div` éditable, tu peux utiliser l'attribut `contenteditable` comme ceci :

```html
<div contenteditable="true"></div>
```

Dans la balise `div` ouvrante, ajoute l'attribut `contenteditable` et définis sa valeur à `"true"`. Tu devrais maintenant pouvoir cliquer sur l'élément et éditer le texte.

# --hints--

Ta balise `div` ouvrante devrait avoir l'attribut `contenteditable`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("contenteditable"));
```

Ton élément `div` devrait avoir l'attribut `contenteditable` défini à `"true"`.
