...
Les étapes restantes du workshop consisteront à ajouter le code pour afficher dynamiquement le message de notes enregistrées et logger les notes actuelles dans la console.

Commence par créer une variable appelée `noteEl` et assigne-lui le résultat d'une requête sur le document pour l'élément avec l'`id` `note`.

Puis crée une autre variable appelée `statusEl` et assigne-lui le résultat d'une requête sur le document pour l'élément avec l'`id` `status`.

# --hints--

Tu devrais avoir une variable `noteEl` et lui assigner le résultat d'une requête sur le document pour l'élément avec l'`id` `note`.

```js
assert.equal(noteEl, document.getElementById("note"));
```

Tu devrais avoir une variable `statusEl` et lui assigner le résultat d'une requête sur le document pour l'élément avec l'`id` `status`.
