...
Crée un sélecteur `.publish-date` et donne-lui une propriété `color` de `rgba(255, 255, 255, 0.5)`.

# --hints--

Tu dois avoir un sélecteur `.publish-date`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Ton sélecteur `.publish-date` doit avoir une propriété `color` définie sur `rgba(255, 255, 255, 0.5)`.
