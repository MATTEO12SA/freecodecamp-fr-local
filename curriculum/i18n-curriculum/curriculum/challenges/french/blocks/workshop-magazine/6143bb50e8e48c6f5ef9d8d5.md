...
Dans ton élément `aside`, crée deux éléments `img`, un élément `blockquote` et un troisième élément `img`. Donne à l'élément `blockquote` une `class` définie sur `image-quote`.

# --hints--

Tu dois créer trois éléments `img` dans ton élément `aside`.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

Tu dois créer un élément `blockquote` dans ton élément `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Ton élément `blockquote` doit avoir la `class` définie sur `image-quote`.

```js
assert.isTrue(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

Tes nouveaux éléments doivent être dans le bon ordre.
