...
Crée une troisième requête `@media` pour `only screen` avec une `max-width` de `550px`. À l'intérieur, crée un sélecteur `.hero-title` avec un `font-size` défini sur `6rem`, un sélecteur `.hero-subtitle, .author, .quote, .list-title` avec un `font-size` défini sur `1.8rem`, un sélecteur `.social-icons` avec un `font-size` défini sur `2rem`, et un sélecteur `.text` avec un `font-size` défini sur `1.6rem`.

# --hints--

Tu dois avoir une nouvelle requête `@media` pour `only screen` avec un `max-width` de `550px`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

Ta nouvelle requête `@media` doit contenir un sélecteur `.hero-title`, un sélecteur `.hero-subtitle, .author, .quote, .list-title`, un sélecteur `.social-icons` et un sélecteur `.text`. Ces sélecteurs doivent être dans cet ordre.
