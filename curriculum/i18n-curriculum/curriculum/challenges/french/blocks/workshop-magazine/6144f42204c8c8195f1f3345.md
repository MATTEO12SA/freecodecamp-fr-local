...
Pour ajouter de l'espace entre les lignes de la mise en page en grille, tu peux utiliser la propriété `row-gap`. Donne au sélecteur `main` une propriété `row-gap` de `3rem`.

# --hints--

Ton sélecteur `main` doit avoir une propriété `row-gap` définie sur `3rem`.
