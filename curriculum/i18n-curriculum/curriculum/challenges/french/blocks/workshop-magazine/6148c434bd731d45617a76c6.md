...
Si tu voulais ajouter plus d'icônes sociales tout en les gardant sur la même ligne, il faudrait mettre à jour `grid-template-columns` pour créer des colonnes supplémentaires. À la place, tu peux utiliser la propriété `grid-auto-flow`.

Cette propriété prend soit `row`, soit `column` comme première valeur, avec une deuxième valeur facultative `dense`. `grid-auto-flow` utilise un algorithme d'auto-placement pour ajuster la mise en page de la grille. La définir sur `column` indique à l'algorithme de créer de nouvelles colonnes pour le contenu selon les besoins. La valeur `dense` permet à l'algorithme de revenir en arrière et de combler les trous dans la grille avec des éléments plus petits, ce qui peut faire apparaître les éléments dans un ordre différent.

Pour ton sélecteur `.social-icons`, définis la propriété `grid-auto-flow` sur `column`.

# --hints--

Ton sélecteur `.social-icons` doit avoir une propriété `grid-auto-flow` définie sur `column`.
