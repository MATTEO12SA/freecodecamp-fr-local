...
Sous ton élément `.text`, crée un nouvel élément `section` et donne-lui une `class` de `text text-with-images`. Dans celui-ci, crée un élément `article` avec une `class` définie sur `brief-history`, et un élément `aside` avec la `class` définie sur `image-wrapper`.

# --hints--

Tu dois créer un nouvel élément `section`.

```js
assert.lengthOf(document.querySelectorAll('section'), 3);
```

Ton nouvel élément `section` doit venir après ton élément `.text`.

```js
assert.equal(document.querySelectorAll('section')?.[2]?.previousElementSibling?.className, 'text');
```

Ton nouvel élément `section` doit avoir la `class` définie sur `text text-with-images`.

```js
assert.equal(document.querySelectorAll('section')?.[2]?.className, 'text text-with-images');
```

Ton nouvel élément `section` doit avoir un élément `article`.

```js
assert.exists(document.querySelector('.text-with-images article'));
```

Ton nouvel élément `section` doit avoir un élément `aside`.

```js
assert.exists(document.querySelector('.text-with-images aside'));
```

L'élément `article` doit venir avant l'élément `aside`.

```js
assert.equal(document.querySelector('.text-with-images article')?.nextElementSibling?.localName, 'aside');
```

Ton élément `article` doit avoir la `class` définie sur `brief-history`.

```js
assert.equal(document.querySelector('.text-with-images article')?.className, 'brief-history');
```

Ton élément `aside` doit avoir la `class` définie sur `image-wrapper`.
