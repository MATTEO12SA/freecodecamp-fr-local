...
Crée un sélecteur `.text` et donne-lui une propriété `grid-column` définie sur `2 / 3`.

# --hints--

Tu dois avoir un sélecteur `.text`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.text'));
```

Ton sélecteur `.text` doit avoir une propriété `grid-column` définie sur `2 / 3`.
