...
Crée un sélecteur `.social-icons`. Donne-lui une propriété `display` définie sur `grid`, et une propriété `font-size` définie sur `3rem`.

# --hints--

Tu dois avoir un sélecteur `.social-icons`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

Ton sélecteur `.social-icons` doit avoir une propriété `display` définie sur `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

Ton sélecteur `.social-icons` doit avoir une propriété `font-size` définie sur `3rem`.
