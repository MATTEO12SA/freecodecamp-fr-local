...
La propriété `gap` est un raccourci pour définir en même temps la valeur de `column-gap` et de `row-gap`. Si une seule valeur est fournie, elle définit `column-gap` et `row-gap` sur cette même valeur. Si deux valeurs sont fournies, elle définit `row-gap` sur la première valeur et `column-gap` sur la seconde.

Donne au sélecteur `.image-wrapper` une propriété `gap` définie sur `2rem`.

# --hints--

Ton élément `.image-wrapper` doit avoir une propriété `gap` définie sur `2rem`.
