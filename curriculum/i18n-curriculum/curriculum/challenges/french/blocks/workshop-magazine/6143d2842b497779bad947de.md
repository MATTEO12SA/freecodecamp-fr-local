...
Pour commencer ton CSS, normalise les règles CSS en ciblant tous les éléments avec `*`, y compris les pseudo-sélecteurs `::before` et `::after`.

Définis les propriétés `padding` et `margin` sur `0`, et définis la propriété `box-sizing` sur `border-box`.

# --hints--

Tu dois avoir un sélecteur `*, ::before, ::after`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after'));
```

Ton sélecteur `*, ::before, ::after` doit avoir une propriété `padding` définie sur `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

Ton sélecteur `*, ::before, ::after` doit avoir une propriété `margin` définie sur `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

Ton sélecteur `*, ::before, ::after` doit avoir une propriété `box-sizing` définie sur `border-box`.
