...
Utilise la fonction `minmax` pour rendre tes colonnes responsives sur n'importe quel appareil. La fonction `minmax` prend deux arguments : le premier est la valeur minimale et le second la valeur maximale. Ces valeurs peuvent être une longueur, un pourcentage, `fr`, ou même un mot-clé comme `max-content`.

Enveloppe chacune des valeurs déjà définies de la propriété `grid-template-columns` dans une fonction `minmax`, en utilisant chaque valeur comme deuxième argument. Le premier argument doit être respectivement `2rem`, `min-content` et `2rem`.

# --hints--

Ton sélecteur `main` doit avoir une propriété `grid-template-columns` définie sur `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
