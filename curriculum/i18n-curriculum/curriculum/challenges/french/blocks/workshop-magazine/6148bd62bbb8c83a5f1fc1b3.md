...
Crée un sélecteur `.hero-title` et donne-lui une propriété `text-align` définie sur `center`, une propriété `color` définie sur `orangered` et une propriété `font-size` définie sur `8rem`.

# --hints--

Tu dois avoir un sélecteur `.hero-title`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

Ton sélecteur `.hero-title` doit avoir une propriété `text-align` définie sur `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

Ton sélecteur `.hero-title` doit avoir une propriété `color` définie sur `orangered`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

Ton sélecteur `.hero-title` doit avoir une propriété `font-size` définie sur `8rem`.
