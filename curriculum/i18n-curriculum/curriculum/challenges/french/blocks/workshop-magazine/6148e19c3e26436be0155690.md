...
Crée une règle `.lists li` pour cibler les éléments de liste dans ton élément `.lists`. Donne-lui une propriété `margin-bottom` définie sur `1.5rem`.

# --hints--

Tu dois avoir un sélecteur `.lists li`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Ton sélecteur `.lists li` doit avoir une propriété `margin-bottom` définie sur `1.5rem`.
