...
Les magazines utilisent souvent du texte justifié dans leur contenu imprimé pour structurer leur mise en page et contrôler le flux de leur contenu. Même si cela fonctionne en version imprimée, le texte justifié sur les sites web peut poser un problème d'accessibilité, par exemple en créant des difficultés pour les personnes dyslexiques.

Pour donner à ton projet l'apparence d'un magazine imprimé, donne au sélecteur `.text` une propriété `text-align` définie sur `justify`.

# --hints--

Ton sélecteur `.text` doit avoir une propriété `text-align` définie sur `justify`.
