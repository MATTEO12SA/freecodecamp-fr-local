...
Dans ton élément `article`, crée un élément `h3` avec la `class` définie sur `list-title` et le texte `A Brief History`. En dessous, crée un élément `p` avec le texte `Of the Curriculum`. Puis crée un élément `ul` avec la classe `lists`.

# --hints--

Tu dois créer un élément `h3` dans ton élément `article`.

```js
assert.exists(document.querySelector('article h3'));
```

Tu dois créer un élément `p` dans ton élément `article`.

```js
assert.exists(document.querySelector('article p'));
```

Tu dois créer un élément `ul` dans ton élément `article`.

```js
assert.exists(document.querySelector('article ul'));
```

Tes éléments dans l'élément `article` doivent être dans le bon ordre.
...
Ton nouvel élément `h3` doit avoir la `class` définie sur `list-title`.

```js
assert.equal(document.querySelector('article h3')?.className, 'list-title');
```

Ton nouvel élément `h3` doit avoir le texte `A Brief History`.

```js
assert.equal(document.querySelector('article h3')?.innerText, 'A Brief History');
```

Ton nouvel élément `p` doit avoir le texte `Of the Curriculum`.

```js
assert.equal(document.querySelector('article p')?.innerText, 'Of the Curriculum');
```

Ton nouvel élément `ul` doit avoir la `class` définie sur `lists`.
