...
Ajoute un troisième élément `p` à la fin de ton élément `.text`, et donne-lui le texte suivant :
...
Tu dois créer un troisième élément `p` dans ton élément `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 3);
```

Ton troisième élément `p` doit avoir le texte fourni.
