...
Donne au sélecteur `.text` une propriété `font-size` définie sur `1.8rem` et une propriété `letter-spacing` définie sur `0.6px`.

# --hints--

Ton sélecteur `.text` doit avoir une propriété `font-size` définie sur `1.8rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

Ton sélecteur `.text` doit avoir une propriété `letter-spacing` définie sur `0.6px`.
