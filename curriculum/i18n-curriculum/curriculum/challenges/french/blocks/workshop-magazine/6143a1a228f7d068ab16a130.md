...
Sous ton élément `blockquote`, ajoute un autre élément `p` avec le texte suivant :
...
Tu dois ajouter un quatrième élément `p` à ton élément `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 5);
```

Ton nouvel élément `p` doit venir après ton élément `blockquote`.
