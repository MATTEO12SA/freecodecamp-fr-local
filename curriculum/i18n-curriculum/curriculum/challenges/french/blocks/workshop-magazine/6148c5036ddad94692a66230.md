...
L'algorithme d'auto-placement se déclenchera maintenant quand tu ajouteras un nouvel élément d'icône. Cependant, l'algorithme définit par défaut la largeur de la nouvelle colonne sur `auto`, ce qui ne correspondra pas à tes colonnes actuelles.

Tu peux remplacer cela avec la propriété `grid-auto-columns`. Donne au sélecteur `.social-icons` une propriété `grid-auto-columns` définie sur `1fr`.

# --hints--

Ton sélecteur `.social-icons` doit avoir une propriété `grid-auto-columns` définie sur `1fr`.
