...
Tu peux maintenant styliser la disposition de ta grille. CSS Grid ressemble à Flexbox dans le sens où il possède une propriété spéciale pour l'élément parent comme pour les éléments enfants.

Dans ce cas, ton élément parent est l'élément `main`. Définis le contenu sur une mise en page en trois colonnes en ajoutant une propriété `grid-template-columns` avec une valeur de `1fr 94rem 1fr`. Cela créera trois colonnes où la colonne centrale fait `94rem` de large, et où la première et la dernière colonne occupent chacune une fraction de l'espace restant dans le conteneur de grille.

# --hints--

Ta section `main` doit avoir une propriété `grid-template-columns` définie sur `1fr 94rem 1fr`.
