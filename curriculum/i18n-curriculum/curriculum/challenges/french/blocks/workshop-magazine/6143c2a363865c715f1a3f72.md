...
Dans l'élément `.image-wrapper`, donne à ton premier élément `img` un `src` de `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, un `alt` de `image of the quote machine project`, une `class` de `image-1`, un attribut `loading` défini sur `lazy`, un attribut `width` de `600` et un attribut `height` de `400`.

# --hints--

Ton premier élément `img` doit avoir un attribut `src` défini sur `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Ton premier élément `img` doit avoir un attribut `alt` défini sur `image of the quote machine project`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt'), 'image of the quote machine project');
```

Ton premier élément `img` doit avoir un attribut `class` défini sur `image-1`.

```js
assert.isTrue(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Ton premier élément `img` doit avoir un attribut `loading` défini sur `lazy`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading'), 'lazy');
```

Ton premier élément `img` doit avoir un attribut `width` défini sur `600`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width'), '600');
```

Ton premier élément `img` doit avoir un attribut `height` défini sur `400`.
