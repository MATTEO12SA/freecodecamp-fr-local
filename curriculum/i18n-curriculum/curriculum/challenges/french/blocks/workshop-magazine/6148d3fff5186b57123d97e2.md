...
Une citation n'en est pas vraiment une sans guillemets appropriés. Tu peux les ajouter avec des pseudo-sélecteurs CSS.

Crée un sélecteur `.quote::before` et définis la propriété `content` sur `"` avec une espace après.

Crée aussi un sélecteur `.quote::after` et définis la propriété `content` sur `"` avec une espace avant.

# --hints--

Tu dois avoir un sélecteur `.quote::before`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Ton sélecteur `.quote::before` doit avoir une propriété `content` définie sur `'" '`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content, /\"\\"\s\"/);
```

Tu dois avoir un sélecteur `.quote::after`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Ton sélecteur `.quote::after` doit avoir une propriété `content` définie sur `' "'`.
