...
Maintenant que la mise en page du magazine est terminée, tu dois la rendre responsive.

Commence avec une requête `@media` pour `only screen` avec une `max-width` de `720px`. À l'intérieur, crée un sélecteur `.image-wrapper` et donne-lui une propriété `grid-template-columns` de `1fr`.

Cela regroupera les trois images en une seule colonne sur les petits écrans.

# --hints--

Tu dois avoir une nouvelle requête `@media` pour `only screen and (max-width: 720px)`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

Ta nouvelle requête `@media` doit contenir un sélecteur `.image-wrapper`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

Ton nouveau sélecteur `.image-wrapper` doit avoir une propriété `grid-template-columns` définie sur `1fr`.
