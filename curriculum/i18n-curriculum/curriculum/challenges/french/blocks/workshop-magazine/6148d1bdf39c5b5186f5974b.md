...
Crée un sélecteur `hr`, et donne-lui une propriété `margin` définie sur `1.5rem 0`.

# --hints--

Tu dois avoir un sélecteur `hr`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('hr'));
```

Ton sélecteur `hr` doit avoir une propriété `margin` définie sur `1.5rem 0`.
