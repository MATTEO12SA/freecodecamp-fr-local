...
Crée un sélecteur `.list-title, .list-subtitle` et définis la propriété `color` sur `#00beef`.

# --hints--

Tu dois avoir un sélecteur `.list-title, .list-subtitle`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'));
```

Ton sélecteur `.list-title, .list-subtitle` doit avoir une propriété `color` définie sur `#00beef`.
