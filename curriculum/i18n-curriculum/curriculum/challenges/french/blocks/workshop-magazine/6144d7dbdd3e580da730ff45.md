...
Crée un sélecteur `body`. Définis la propriété `font-family` sur `Baskervville`, avec `serif` en police de repli. Définis la propriété `color` sur `linen` et la propriété `background-color` sur `rgb(20, 30, 40)`.

# --hints--

Tu dois avoir un sélecteur `body`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('body'));
```

Ton sélecteur `body` doit avoir une propriété `font-family` définie sur `Baskervville` avec `serif` comme police de repli.
...
Ton sélecteur `body` doit avoir une propriété `color` définie sur `linen`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.color, 'linen');
```

Ton sélecteur `body` doit avoir une propriété `background-color` définie sur `rgb(20, 30, 40)`.
