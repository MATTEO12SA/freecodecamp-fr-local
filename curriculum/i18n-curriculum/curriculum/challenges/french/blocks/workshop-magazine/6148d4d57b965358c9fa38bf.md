...
Il est maintenant temps de styliser ta troisième `section`. Note qu'elle possède les valeurs `text` et `text-with-images` pour l'attribut `class`, ce qui signifie qu'elle hérite déjà des styles de ta règle `.text`.

Crée un sélecteur `.text-with-images` et définis la propriété `display` sur `grid`.

# --hints--

Tu dois avoir un sélecteur `.text-with-images`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Ton sélecteur `.text-with-images` doit avoir une propriété `display` définie sur `grid`.
