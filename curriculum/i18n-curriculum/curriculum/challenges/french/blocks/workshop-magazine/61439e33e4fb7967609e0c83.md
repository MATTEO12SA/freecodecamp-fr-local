...
Après les trois éléments `p` dans ton élément `.text`, crée un élément `blockquote`. Dans celui-ci, ajoute un élément `hr`, un élément `p` avec la `class` définie sur `quote`, puis un second élément `hr`.

Donne à l'élément `.quote` le texte `The entire curriculum should be a series of projects`.

# --hints--

Tu dois créer un nouvel élément `blockquote` dans ton élément `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

Ton élément `blockquote` doit venir après tes trois éléments `p`.

```js
assert.equal(document.querySelector('.text')?.children?.[3]?.localName, 'blockquote');
```

Ton élément `blockquote` doit avoir deux éléments `hr`.

```js
assert.lengthOf(document.querySelectorAll('.text blockquote hr'), 2);
```

Ton élément `blockquote` doit avoir un élément `p`.

```js
assert.exists(document.querySelector('.text blockquote p'));
```

Les enfants de ton élément `blockquote` doivent être dans le bon ordre.
...
Ton nouvel élément `p` doit avoir le texte `The entire curriculum should be a series of projects`.
