...
Crée un dernier élément `p` à la fin de ton élément `.text` et donne-lui le texte suivant :
...
Tu dois ajouter un sixième élément `p` à ton élément `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Ton sixième élément `p` doit avoir le texte fourni.
