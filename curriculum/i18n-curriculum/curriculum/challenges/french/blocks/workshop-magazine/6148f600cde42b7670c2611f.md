...
Crée une autre requête `@media` pour `only screen` avec une `max-width` de `600px`. À l'intérieur, crée une règle `.text-with-images` et donne-lui une propriété `grid-template-columns` de `1fr`.

Cela regroupera ta zone de texte du bas en une seule colonne sur les petits écrans.

# --hints--

Tu dois créer une nouvelle requête `@media` pour `only screen and (max-width: 600px)`. Elle doit se trouver sous ta requête `@media` précédente.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText, 'only screen and (max-width: 600px)');
```

Ta nouvelle requête `@media` doit contenir un sélecteur `.text-with-images`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

Ton nouveau sélecteur `.text-with-images` doit avoir une propriété `grid-template-columns` définie sur `1fr`.
