...
Crée un sélecteur `.author-name a:hover` et donne-lui une propriété `background-color` définie sur `#306203`.

Cela créera un effet de survol uniquement pour l'élément `a` dans `.author-name`, affichant le vert freeCodeCamp d'origine en arrière-plan.

# --hints--

Tu dois avoir un sélecteur `.author-name a:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Ton sélecteur `.author-name a:hover` doit avoir une propriété `background-color` définie sur `#306203`.
