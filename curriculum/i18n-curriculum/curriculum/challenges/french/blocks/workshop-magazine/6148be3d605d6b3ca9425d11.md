...
Le sous-titre doit aussi être stylisé. Crée un sélecteur `.hero-subtitle` et donne-lui une propriété `font-size` définie sur `2.4rem`, une propriété `color` définie sur `orangered`, et une propriété `text-align` définie sur `center`.

# --hints--

Tu dois avoir un sélecteur `.hero-subtitle`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

Ton sélecteur `.hero-subtitle` doit avoir une propriété `font-size` définie sur `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize, '2.4rem');
```

Ton sélecteur `.hero-subtitle` doit avoir une propriété `color` définie sur `orangered`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color, 'orangered');
```

Ton sélecteur `.hero-subtitle` doit avoir une propriété `text-align` définie sur `center`.
