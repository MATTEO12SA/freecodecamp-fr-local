...
Ton élément `.text` n'est pas une grille CSS, mais tu peux créer des colonnes dans un élément sans utiliser Grid grâce à la propriété `column-width`.

Donne à ton sélecteur `.text` une propriété `column-width` définie sur `25rem`.

# --hints--

Ton sélecteur `.text` doit avoir une propriété `column-width` définie sur `25rem`.
