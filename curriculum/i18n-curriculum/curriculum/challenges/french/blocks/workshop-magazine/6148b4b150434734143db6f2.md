...
Rappelle-toi que la propriété `grid-column` détermine à quelles colonnes un élément commence et se termine. Il peut arriver que tu ne saches pas combien de colonnes ta grille aura, mais que tu veuilles qu'un élément s'arrête à la dernière colonne. Pour cela, tu peux utiliser `-1` comme colonne de fin.

Crée un sélecteur `.hero` et donne-lui une propriété `grid-column` définie sur `1 / -1`. Cela indiquera à l'élément de s'étendre sur toute la largeur de la grille.

# --hints--

Tu dois avoir un sélecteur `.hero`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Ton sélecteur `.hero` doit avoir une propriété `grid-column` définie sur `1 / -1`.
