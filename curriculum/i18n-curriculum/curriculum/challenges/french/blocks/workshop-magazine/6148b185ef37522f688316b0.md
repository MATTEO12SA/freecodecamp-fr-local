...
Tu peux maintenant styliser le contenu de l'élément `.heading` avec CSS Grid.

La fonction CSS `repeat()` est utilisée pour répéter une valeur au lieu de l'écrire manuellement, et elle est utile pour les mises en page en grille. Par exemple, définir la propriété `grid-template-columns` sur `repeat(20, 200px)` créerait 20 colonnes de `200px` de large chacune.

Donne à ton élément `.heading` une propriété `grid-template-columns` définie sur `repeat(2, 1fr)` pour créer deux colonnes de largeur égale.

# --hints--

Ton sélecteur `.heading` doit avoir une propriété `grid-template-columns` définie sur `repeat(2, 1fr)`.
