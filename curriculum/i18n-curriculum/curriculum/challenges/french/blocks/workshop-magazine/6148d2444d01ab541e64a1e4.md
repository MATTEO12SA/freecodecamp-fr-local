...
Crée un sélecteur `.quote`. Donne-lui une propriété `color` définie sur `#00beef`, une propriété `font-size` définie sur `2.4rem` et une propriété `text-align` définie sur `center`.

# --hints--

Tu dois avoir un sélecteur `.quote`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Ton sélecteur `.quote` doit avoir une propriété `color` définie sur `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

Ton sélecteur `.quote` doit avoir une propriété `font-size` définie sur `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

Ton sélecteur `.quote` doit avoir une propriété `text-align` définie sur `center`.
