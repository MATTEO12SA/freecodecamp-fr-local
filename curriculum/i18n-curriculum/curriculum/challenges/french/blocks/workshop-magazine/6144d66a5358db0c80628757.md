...
Crée un sélecteur `html` et donne-lui une propriété `font-size` définie sur `62.5%`. Cela définira la taille de police par défaut de ta page web à 10px (la valeur par défaut du navigateur est 16px).

Cela te facilitera le travail avec les unités `rem` plus tard, car `2rem` vaudrait 20px.

# --hints--

Tu dois créer un sélecteur `html`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('html'));
```

Ton sélecteur `html` doit avoir une propriété `font-size` définie sur `62.5%`.
