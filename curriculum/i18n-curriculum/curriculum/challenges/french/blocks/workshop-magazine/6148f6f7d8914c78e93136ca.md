...
Crée une dernière requête `@media` pour `only screen` avec une `max-width` de `420px`. À l'intérieur, crée un sélecteur `.hero-title` avec une propriété `font-size` définie sur `4.5rem`.

Félicitations ! Ton magazine est maintenant terminé.

# --hints--

Tu dois avoir une nouvelle requête `@media` pour `only screen and (max-width: 420px)`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Ta nouvelle requête `@media` doit contenir un sélecteur `.hero-title`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Ton sélecteur `.hero-title` doit avoir une propriété `font-size` définie sur `4.5rem`.
