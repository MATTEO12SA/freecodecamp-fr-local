...
Le pseudo-sélecteur `::first-letter` te permet de cibler la première lettre du contenu textuel d'un élément.

Crée un sélecteur `.first-paragraph::first-letter` et définis la propriété `font-size` sur `6rem`. Donne-lui aussi une propriété `color` définie sur `orangered` pour la faire ressortir.

# --hints--

Tu dois avoir un sélecteur `.first-paragraph::first-letter`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Ton sélecteur `.first-paragraph::first-letter` doit avoir une propriété `font-size` définie sur `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

Ton sélecteur `.first-paragraph::first-letter` doit avoir une propriété `color` définie sur `orangered`.
