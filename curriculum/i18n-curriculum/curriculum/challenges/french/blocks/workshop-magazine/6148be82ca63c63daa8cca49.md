...
Crée un sélecteur `.author` et donne-lui une propriété `font-size` définie sur `2rem` et une propriété `font-family` définie sur `Raleway` avec `sans-serif` comme police de repli.

# --hints--

Tu dois avoir un sélecteur `.author`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author'));
```

Ton sélecteur `.author` doit avoir une propriété `font-size` définie sur `2rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

Ton sélecteur `.author` doit avoir une propriété `font-family` définie sur `Raleway` avec `sans-serif` comme police de repli.
