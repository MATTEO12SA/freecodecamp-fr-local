...
Ton magazine aura trois sections principales. Tu as déjà défini la mise en page globale dans la règle `main`, mais tu peux ajuster le placement dans les règles des enfants.

Une option est la propriété `grid-column`, qui est un raccourci pour `grid-column-start` et `grid-column-end`. La propriété `grid-column` indique à l'élément de grille à quelle ligne de grille il doit commencer et à quelle ligne il doit se terminer.

Crée une règle `.heading` et définis la propriété `grid-column` sur `2 / 3`. Cela indiquera à l'élément `.heading` de commencer à la ligne de grille 2 et de se terminer à la ligne de grille 3.

# --hints--

Tu dois avoir un sélecteur `.heading`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.heading'));
```

Ton sélecteur `.heading` doit avoir une propriété `grid-column` définie sur `2 / 3`.
