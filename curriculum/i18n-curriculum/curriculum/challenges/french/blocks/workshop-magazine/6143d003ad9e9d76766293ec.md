...
Dans ton élément `.image-quote`, imbrique un élément `hr`, un élément `p` et un second élément `hr`. Donne à l'élément `p` une `class` définie sur `quote` et le texte `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Tu dois ajouter deux éléments `hr` à ton élément `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

Tu dois ajouter un élément `p` à ton élément `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

Les enfants de ton élément `.image-quote` doivent être dans le bon ordre.
...
Ton nouvel élément `p` doit avoir la `class` définie sur `quote`.

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Ton nouvel élément `p` doit avoir le texte `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
