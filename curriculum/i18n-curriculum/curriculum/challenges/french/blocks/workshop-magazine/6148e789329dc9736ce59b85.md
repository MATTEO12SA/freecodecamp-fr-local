...
Crée une règle `.image-1, .image-3` et donne-lui une propriété `grid-column` définie sur `1 / -1`. Cela permettra aux première et troisième images de s'étendre sur toute la largeur de la grille.

# --hints--

Tu dois avoir un sélecteur `.image-1, .image-3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Ton sélecteur `.image-1, .image-3` doit avoir une propriété `grid-column` définie sur `1 / -1`.
