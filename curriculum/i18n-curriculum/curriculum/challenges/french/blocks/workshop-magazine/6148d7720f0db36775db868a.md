...
Tu devras avoir une colonne pour le texte et une colonne pour les images. Donne au sélecteur `.text-with-images` une propriété `grid-template-columns` définie sur `1fr 2fr`. Définis aussi la propriété `column-gap` sur `3rem` pour fournir plus d'espace entre les colonnes.

# --hints--

Ton sélecteur `.text-with-images` doit avoir une propriété `grid-template-columns` définie sur `1fr 2fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

Ton sélecteur `.text-with-images` doit avoir une propriété `column-gap` définie sur `3rem`.
