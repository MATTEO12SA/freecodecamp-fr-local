...
Crée un autre élément `p` sous ton élément `.first-paragraph`, et donne-lui le texte suivant :
...
Tu dois créer un deuxième élément `p` dans ton élément `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 2);
```

Ton deuxième élément `p` doit avoir le texte fourni.
