...
Crée un sélecteur `.lists` et définis la propriété `list-style-type` sur `none`. Cela supprimera les puces des éléments de liste.

# --hints--

Tu dois avoir un sélecteur `.lists`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Ton sélecteur `.lists` doit avoir une propriété `list-style-type` définie sur `none`.
