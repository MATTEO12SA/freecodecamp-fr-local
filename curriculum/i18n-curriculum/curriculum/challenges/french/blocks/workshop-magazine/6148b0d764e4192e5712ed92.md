...
Pour mieux contrôler la disposition de ton contenu, tu peux avoir une grille CSS dans une autre grille CSS.

Définis la propriété `display` de ton sélecteur `.heading` sur `grid`.

# --hints--

Ton sélecteur `.heading` doit avoir une propriété `display` définie sur `grid`.
