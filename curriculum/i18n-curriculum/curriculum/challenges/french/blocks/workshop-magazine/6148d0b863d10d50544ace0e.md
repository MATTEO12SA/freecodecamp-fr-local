...
L'autre texte a été déplacé. Remets-le en position en donnant au sélecteur `.first-paragraph::first-letter` une propriété `float` définie sur `left` et une propriété `margin-right` définie sur `1rem`.

# --hints--

Ton sélecteur `.first-paragraph::first-letter` doit avoir une propriété `float` définie sur `left`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

Ton sélecteur `.first-paragraph::first-letter` doit avoir une propriété `margin-right` définie sur `1rem`.
