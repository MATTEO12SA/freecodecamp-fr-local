...
Il est temps de styliser la dernière section du magazine : les images.

Les images sont enveloppées dans un élément `aside` qui utilise la classe `image-wrapper`, donc crée un sélecteur `.image-wrapper`. Définis la propriété `display` sur `grid`.

# --hints--

Tu dois avoir un sélecteur `.image-wrapper`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Ton sélecteur `.image-wrapper` doit avoir une propriété `display` définie sur `grid`.
