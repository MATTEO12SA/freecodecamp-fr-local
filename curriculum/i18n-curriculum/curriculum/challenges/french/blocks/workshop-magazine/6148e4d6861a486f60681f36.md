...
Les images doivent être dans une mise en page à deux colonnes et trois lignes.

Donne au sélecteur `.image-wrapper` une propriété `grid-template-columns` définie sur `2fr 1fr` et une propriété `grid-template-rows` définie sur `repeat(3, min-content)`. Cela donnera à notre grille des lignes qui ajustent leur hauteur selon le contenu, mais des colonnes qui restent d'une largeur fixe selon le conteneur.

# --hints--

Ton sélecteur `.image-wrapper` doit avoir une propriété `grid-template-columns` définie sur `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Ton sélecteur `.image-wrapper` doit avoir une propriété `grid-template-rows` définie sur `repeat(3, min-content)`.
