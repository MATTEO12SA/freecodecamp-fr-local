...
Dans ton élément `ul`, crée six éléments `li`. Ajoute un élément `h4` avec une `class` définie sur `list-subtitle` et un élément `p` à chacun de tes éléments `li`.

Donne ensuite aux éléments `h4` et `p` le contenu textuel suivant, dans l'ordre, chaque `h4` utilisant ce qui se trouve à gauche des deux-points et chaque `p` utilisant ce qui se trouve à droite :
...
Ton élément `ul` doit avoir six éléments `li`.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Chacun de tes nouveaux éléments `li` doit avoir un élément `h4` et un élément `p`.
...
Ton premier `h4` doit avoir le texte `V1 - 2014`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

Ton premier `p` doit avoir le texte `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`.
...
Ton deuxième `h4` doit avoir le texte `V2 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

Ton deuxième `p` doit avoir le texte `We added interactive algorithm challenges.`.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

Ton troisième `h4` doit avoir le texte `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

Ton troisième `p` doit avoir le texte `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`.
...
Ton quatrième `h4` doit avoir le texte `V4 - 2016`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

Ton quatrième `p` doit avoir le texte `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.`.
...
Ton cinquième `h4` doit avoir le texte `V5 - 2017`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

Ton cinquième `p` doit avoir le texte `We added the back-end and data visualization challenges.`.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

Ton sixième `h4` doit avoir le texte `V6 - 2018`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[5]?.innerText, 'V6 - 2018');
```

Ton sixième `p` doit avoir le texte `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`.
...
Tes six éléments `h4` doivent avoir chacun la classe `list-subtitle`.
