...
Crée un sélecteur `a`, et donne-lui une propriété `text-decoration` définie sur `none` et une propriété `color` définie sur `linen`.

# --hints--

Tu dois avoir un sélecteur `a`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Ton sélecteur `a` doit avoir une propriété `text-decoration` définie sur `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Ton sélecteur `a` doit avoir une propriété `color` définie sur `linen`.
