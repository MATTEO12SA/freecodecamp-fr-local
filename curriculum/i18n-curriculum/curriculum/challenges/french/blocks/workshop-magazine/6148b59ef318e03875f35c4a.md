...
Crée un sélecteur `img` et donne-lui une propriété `width` définie sur `100%`, et une propriété `object-fit` définie sur `cover`.

La propriété `object-fit` indique au navigateur comment positionner l'élément dans son conteneur. Dans ce cas, `cover` définira l'image pour remplir le conteneur, en la rognant si nécessaire pour éviter de modifier le ratio d'aspect.

# --hints--

Tu dois avoir un sélecteur `img`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('img'));
```

Ton sélecteur `img` doit avoir une propriété `width` définie sur `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('img')?.width, '100%');
```

Ton sélecteur `img` doit avoir une propriété `object-fit` définie sur `cover`.
