...
Ajoute deux éléments `span` avec le `className` `stat-value` et `stat-label` respectivement à chaque `div.stat-row`.

# --hints--

Chaque `.stat-row` doit contenir un `span` avec le `className` `stat-value` et un `span` avec le `className` `stat-label`.
