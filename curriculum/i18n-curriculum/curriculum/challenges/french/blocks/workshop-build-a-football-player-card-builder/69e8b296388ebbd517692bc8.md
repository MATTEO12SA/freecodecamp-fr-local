...
Les dernières valeurs en dur sont les six stats du joueur. Remplace les trois premiers nombres en dur par la propriété `player` correspondante :
...
Tu dois remplacer la valeur `97` pour `PAC` par `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Tu dois remplacer la valeur `98` pour `SHO` par `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Tu dois remplacer la valeur `83` pour `PAS` par `player.pas`.
