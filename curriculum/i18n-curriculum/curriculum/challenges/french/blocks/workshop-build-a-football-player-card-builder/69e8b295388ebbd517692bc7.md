...
Remplace le nom de joueur en dur `PELE` à l’intérieur de `card-name` par `player.name`.

# --hints--

Tu dois remplacer le `PELE` en dur par `player.name` à l’intérieur de `.card-name`.
