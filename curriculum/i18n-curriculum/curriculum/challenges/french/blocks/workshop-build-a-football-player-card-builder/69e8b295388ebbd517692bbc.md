...
Ajoute maintenant un `div` avec le `className` `card-stats`. À l’intérieur, ajoute un `div` avec le `className` `stat-col`. Dedans, ajoute trois éléments `div` chacun avec le `className` `stat-row`.

# --hints--

Tu dois ajouter un `div` avec le `className` `card-stats` après `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Tu dois ajouter un `div` avec le `className` `stat-col` à l’intérieur de `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Tu dois ajouter trois éléments `div` chacun avec le `className` `stat-row` à l’intérieur de `.stat-col`.
