...
Tu vas maintenant commencer à construire le formulaire. À l’intérieur du `div` avec le `className` `form-panel`, ajoute un `div` pour envelopper la section "Player Info". Dans ce nouveau `div`, ajoute un `p` avec le `className` `form-section-title` et le texte `Player Info`.

Tu continueras à ajouter des champs à cette section dans les prochaines étapes.

# --hints--

Ton `div.form-panel` doit contenir un `div` pour envelopper la section Player Info.
...
Le `div` à l’intérieur de `div.form-panel` doit contenir un `p` avec le `className` `form-section-title` et le texte `Player Info`.
