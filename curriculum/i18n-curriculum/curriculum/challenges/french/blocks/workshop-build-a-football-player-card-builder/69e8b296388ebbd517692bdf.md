...
Ajoute les attributs `value` et `onChange` aux inputs DEF et PHY.

Pour DEF, définis `value` à `player.def` et `onChange` à une fonction fléchée qui appelle `setPlayer` avec un spread de `player` et `def` défini à `Number(e.target.value)`.

Pour PHY, définis `value` à `player.phy` et `onChange` à une fonction fléchée qui appelle `setPlayer` avec un spread de `player` et `phy` défini à `Number(e.target.value)`.

Avec cette étape, tous les inputs sont maintenant connectés à l’état et l’aperçu en direct se mettra à jour pour chaque champ !

# --hints--

Ton `input` DEF doit avoir une `value` égale à la stat `def` du joueur.
...
Ton `input` DEF doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec le spread de `player` et `def` défini à `Number(e.target.value)`.
...
Ton `input` PHY doit avoir une `value` égale à la stat `phy` du joueur.
...
Ton `input` PHY doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec le spread de `player` et `phy` défini à `Number(e.target.value)`.
