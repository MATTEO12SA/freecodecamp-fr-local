...
Le badge de tier affiche encore le texte en dur `GOLD`. Tu dois le remplacer par une expression dynamique pour que le bon tier s’affiche.

Appelle `getPlayerTier()` avec `player.overallRating` pour obtenir la chaîne de tier, puis appelle `.toUpperCase()` sur le résultat pour l’afficher en majuscules. N’oublie pas d’envelopper toute l’expression dans des accolades.

# --hints--

Tu dois utiliser `getPlayerTier(player.overallRating).toUpperCase()` à l’intérieur de `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

L’élément `.card-tier-badge` doit afficher `ELITE` pour un joueur avec une note de 98.
