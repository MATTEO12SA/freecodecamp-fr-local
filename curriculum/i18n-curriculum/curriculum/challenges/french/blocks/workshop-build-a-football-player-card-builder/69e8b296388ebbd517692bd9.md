...
Ajoute un attribut `value` de `player.club` à l’`input` Club. Ensuite, ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `club` défini à `e.target.value`.

# --hints--

Ton `input` Club doit avoir une `value` égale au club du joueur.
...
Ton `input` Club doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `club` défini à `e.target.value`.
