...
Maintenant que l’interface `PlayerData` est définie, mets à jour le composant `PlayerCard` pour accepter une prop `player` typée comme `PlayerData` afin de correspondre à l’interface `PlayerData`.

Souviens-toi que tu peux déstructurer les props comme ceci :
...
Tu dois mettre à jour `PlayerCard` pour accepter une prop `player` typée comme `PlayerData`.
