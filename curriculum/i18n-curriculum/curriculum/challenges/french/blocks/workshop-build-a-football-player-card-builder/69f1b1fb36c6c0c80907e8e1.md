...
À l’intérieur de l’élément `preview-panel`, ajoute un élément `p` avec le `className` `preview-label` et le texte `Live Preview`, un autre élément `p` avec le `className` `preview-hint` et le texte `Updates as you type`, et un `div` avec le `className` défini au littéral de gabarit `preview-box tier-${getPlayerTier(player.overallRating)}`.

Dans le dernier `div`, déplace le composant `PlayerCard` et sa prop.

# --hints--

Ton élément `div.preview-panel` doit contenir un `p` avec le `className` `preview-label` et le texte `Live Preview`.
...
Ton `div.preview-panel` doit contenir un `p` avec le `className` `preview-hint` et le texte `Updates as you type`.
...
Ton `div.preview-panel` doit contenir un `div` avec le `className` `preview-box` et un `className` de tier dynamique préfixé par `tier-`.
...
Ton `PlayerCard` doit être rendu à l’intérieur du `div` avec le `className` `preview-box`.
