...
Dans le bloc de l’instruction `if`, renvoie un objet qui spread à la fois `defaultPlayer` et `JSON.parse(saved)` séparément. Assure-toi qu’ils sont séparés par une virgule, car ce sont deux objets distincts fusionnés en un seul.

# --hints--

Ton bloc `if (saved)` doit renvoyer un objet qui spread à la fois `defaultPlayer` et les données sauvegardées parsées.
