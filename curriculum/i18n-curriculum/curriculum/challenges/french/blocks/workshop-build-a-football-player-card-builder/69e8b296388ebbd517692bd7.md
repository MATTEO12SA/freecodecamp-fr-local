...
Tu vas maintenant commencer le processus pour changer l’input de position en select.

Change l’input en un élément `select` avec l’id `position`, un `className` `input`, et une `value` de `player.position`.

# --hints--

Tu dois changer l’`input` de position en un élément `select` avec un `id` `position` et un `className` `input`.
...
Ton `select` de position doit avoir une `value` de `player.position`.
