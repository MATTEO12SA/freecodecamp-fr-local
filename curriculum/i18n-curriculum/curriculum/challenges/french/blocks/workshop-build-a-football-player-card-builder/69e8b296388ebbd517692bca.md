...
`FootballPlayerCard` rend actuellement la carte avec un `defaultPlayer` fixe. Pour rendre l’application interactive, tu dois gérer les données du joueur dans l’état React afin que les mises à jour déclenchent des re-rendus.

À l’intérieur du composant `FootballPlayerCard`, crée une variable d’état appelée `player` avec un setter `setPlayer` qui contiendra les données actuelles du joueur. Initialise cet état avec `defaultPlayer`, et précise que l’état utilise le type `PlayerData` comme générique pour que TypeScript connaisse la forme de l’objet joueur.

Ensuite, mets à jour le return pour passer la variable d’état `player` comme prop à `PlayerCard`.

# --hints--

Tu dois utiliser la déstructuration de tableau pour définir une variable d’état `player` et un setter `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Ton `player` et `setPlayer` doivent utiliser le hook `useState`.
...
Ton `useState` doit être initialisé avec `defaultPlayer` et avoir `PlayerData` comme type générique.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Tu dois avoir `player` comme valeur de la prop `player` passée à `PlayerCard`.
