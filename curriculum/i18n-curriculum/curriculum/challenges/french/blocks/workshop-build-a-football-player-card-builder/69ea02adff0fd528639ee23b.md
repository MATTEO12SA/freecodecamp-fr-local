...
Ensuite, crée une fonction `loadPlayer` avec `PlayerData` comme type de retour. À l’intérieur, ajoute une syntaxe `try...catch` avec `error` comme paramètre du bloc `catch`. Laisse tout le reste vide pour l’instant.

# --hints--

Tu dois créer une fonction appelée `loadPlayer`.
...
Ta fonction `loadPlayer` doit avoir un type de retour `PlayerData`.
...
Ta fonction `loadPlayer` doit contenir un bloc `try...catch`.
