...
Tu vas maintenant gérer la sélection de la position du joueur. Tu dois refactoriser l’input en select, puis mapper un tableau de positions de football valides comme valeur des options du select.

Pour commencer, crée un tableau `POSITIONS` avec les valeurs chaînes `GK`, `CB`, `LB`, `RB`, `CDM`, `CM`, `CAM`, `LW`, `RW`, `ST` et `CF`. Ajoute `as const` juste après les crochets fermants pour que TypeScript restreigne le type d’un général `string[]` à un tuple en lecture seule de ces valeurs littérales exactes.

Cela signifie que TypeScript sait précisément quelles chaînes sont valides, plutôt que de seulement savoir que c’est un tableau de chaînes.

# --hints--

Tu dois déclarer une constante `POSITIONS`.
...
Ton tableau `POSITIONS` doit contenir toutes les chaînes de position de football requises ; utilise `as const` pour restreindre son type à des littéraux de chaîne en lecture seule.
