...
À l’intérieur du bloc `try`, sauvegarde les données actuelles du joueur dans `localStorage` en appelant `localStorage.setItem` avec la clé de stockage et l’objet joueur stringifié.

Et dans le bloc `catch`, journalise la chaîne `Failed to save player data:` suivie du paramètre `error` dans la console.

# --hints--

Ton bloc try de `useEffect` doit appeler `localStorage.setItem` avec `STORAGE_KEY` et l’objet `player` stringifié.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Ton bloc catch de `useEffect` doit journaliser `'Failed to save player data:'` et l’`error` dans la console.
