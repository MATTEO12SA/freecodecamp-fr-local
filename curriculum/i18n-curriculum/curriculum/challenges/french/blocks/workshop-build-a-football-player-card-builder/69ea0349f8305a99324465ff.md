...
Dans le bloc `try`, crée une variable `saved`. Pour la valeur, utilise la méthode `getItem()` pour récupérer `STORAGE_KEY`. Juste sous cette variable, crée une instruction `if` avec la condition `saved`. Laisse le bloc `if` vide pour l’instant.

# --hints--

Ta fonction `loadPlayer` doit appeler `localStorage.getItem` avec `STORAGE_KEY` à l’intérieur du bloc `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Le résultat de `localStorage.getItem` doit être stocké dans une variable appelée `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Le bloc `try` doit avoir une instruction `if` avec la condition `saved`.
