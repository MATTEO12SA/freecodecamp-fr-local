...
Ajoute un attribut `value` de `player.pac` à l’`input` PAC. Puis ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `pac` défini à `Number(e.target.value)`.

# --hints--

Ton `input` PAC doit avoir une `value` égale à la stat `pac` du joueur.
...
Ton `input` PAC doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `pac` défini à `Number(e.target.value)`.
