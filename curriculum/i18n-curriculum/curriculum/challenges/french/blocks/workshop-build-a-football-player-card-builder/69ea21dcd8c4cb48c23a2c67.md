...
Ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `position` défini à `e.target.value`, casté comme `Position`.

# --hints--

Ton `select` de position doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et met à jour `position`.
