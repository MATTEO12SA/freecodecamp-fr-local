...
Ajoute un attribut `value` de `player.sho` à l’`input` SHO. Puis définis son attribut `onChange` à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `sho` défini à `Number(e.target.value)`.

# --hints--

Ton `input` SHO doit avoir une `value` égale à la stat `sho` actuelle du joueur.
...
Ton `input` SHO doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `sho` défini à `Number(e.target.value)`.
