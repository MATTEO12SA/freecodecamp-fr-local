...
Ajoute un attribut `value` de `player.imageUrl` à l’`input` Image URL. Puis définis l’attribut `onChange` à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `imageUrl` défini à `e.target.value`.

# --hints--

Ton `input` Image URL doit avoir une `value` égale à l’URL d’image du joueur.
...
Ton `input` Image URL doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `imageUrl` défini à `e.target.value`.
