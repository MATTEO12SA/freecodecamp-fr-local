...
Mappe maintenant le tableau `POSITIONS` avec une fonction fléchée qui reçoit chaque chaîne de position comme `pos` et renvoie un élément `option`. L’option doit avoir une `key` de `pos` et la value aussi `pos`. Affiche aussi `pos` comme texte de chaque `option` générée.

Tu peux utiliser n’importe quel nom pour le paramètre de ton map, ce n’est pas obligé d’être `pos`.

# --hints--

Ton `select` de position doit contenir des éléments `option` mappés depuis le tableau `POSITIONS`.
...
La `value` de chaque option doit correspondre à son texte affiché et être une position valide.
