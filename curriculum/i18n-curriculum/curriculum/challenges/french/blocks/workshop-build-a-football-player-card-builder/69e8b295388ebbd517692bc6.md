...
La photo du joueur est chargée depuis une URL en dur. Remplace la valeur de l’attribut `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` par `player.imageUrl`.

Remplace aussi le texte `alt` en dur par une valeur dynamique avec un littéral de gabarit utilisant `player.name` et le texte `headshot`.

Note que lorsque tu utilises une expression JavaScript comme valeur d’attribut en TSX, tu utilises des accolades `{}` au lieu de guillemets. Par exemple `src={player.imageUrl}`, pas `src="{player.imageUrl}"`.

# --hints--

Tu dois remplacer l’URL d’image en dur par `player.imageUrl` comme attribut `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Tu dois remplacer le texte `alt` en dur par un littéral de gabarit utilisant `player.name` et le texte `headshot`.
