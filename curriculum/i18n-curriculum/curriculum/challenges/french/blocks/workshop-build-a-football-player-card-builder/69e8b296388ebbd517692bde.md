...
Ajoute un attribut `value` de `player.dri` à l’input DRI. Puis ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `dri` défini à `Number(e.target.value)`.

# --hints--

Ton `input` DRI doit avoir une `value` égale à la stat `dri` du joueur.
...
Ton `input` DRI doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `dri` défini à `Number(e.target.value)`.
