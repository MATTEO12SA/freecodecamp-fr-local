...
Souviens-toi de JSX : les accolades te permettent d’intégrer n’importe quelle expression JavaScript directement dans le JSX. C’est ainsi que tu affiches des données dynamiques depuis les props. Tu fais la même chose en TSX.

L’en-tête de la carte affiche encore des valeurs en dur. Remplace le texte `98` dans `card-rating` par `player.overallRating`, et remplace `ST` dans `card-position` par `player.position`.

# --hints--

Tu dois remplacer le `98` en dur par `player.overallRating` dans `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Tu dois remplacer le `ST` en dur par `player.position` dans `.card-position`.
