...
Change la valeur de la clé `position` pour le `Position` dérivé.

# --hints--

Le champ `position` de ton interface `PlayerData` doit utiliser le type `Position` au lieu de `string`.
