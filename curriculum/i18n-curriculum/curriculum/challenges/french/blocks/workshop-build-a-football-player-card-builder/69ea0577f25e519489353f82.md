...
Dans le bloc `catch`, journalise la chaîne `Failed to load player data, using defaults:` suivie du paramètre `error` dans la console.

Puis, en dehors du `catch` mais toujours dans la fonction `loadPlayer`, renvoie `defaultPlayer`.

Tout cela chargera les données du joueur, les peuplera dans l’UI et chargera l’objet `defaultPlayer` si le processus échoue.

# --hints--

Ton bloc `catch` doit journaliser `'Failed to load player data, using defaults:', error` dans la console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

Ta fonction `loadPlayer` doit renvoyer `defaultPlayer` comme valeur de repli.
