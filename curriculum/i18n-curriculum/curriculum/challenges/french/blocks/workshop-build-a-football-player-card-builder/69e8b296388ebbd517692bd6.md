...
Le formulaire est maintenant terminé. Pour que la carte de prévisualisation se mette à jour pendant que tu tapes, tu dois connecter les inputs à l’état `player`.

En commençant par l’`input` Name, ajoute un attribut `value` de `player.name`. Puis ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `name` défini à `e.target.value`. N’oublie pas de mettre le spread et le réglage de `name` dans un objet.

# --hints--

La `value` de ton input name doit être liée au champ `name` de l’état player.
...
Ton input name doit avoir un gestionnaire `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Le gestionnaire `onChange` doit appeler `setPlayer` avec `setPlayer` ayant un objet qui spread `player` et définit `name` à `e.target.value`.
