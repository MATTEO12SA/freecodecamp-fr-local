...
Ajoute un attribut `value` de `player.overallRating` à l’`input` Overall. Ensuite, ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `overallRating` défini à `Number(e.target.value)`.

Puisque `overallRating` est un nombre, utilise `Number()` pour convertir la valeur chaîne de l’input.

# --hints--

Ton `input` Overall doit avoir une `value` égale à la note globale actuelle du joueur.
...
Ton `input` Overall doit avoir un attribut `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Le `onChange` de ton `input` Overall doit appeler `setPlayer` avec un spread de `player` et `overallRating` défini à `Number(e.target.value)`.
