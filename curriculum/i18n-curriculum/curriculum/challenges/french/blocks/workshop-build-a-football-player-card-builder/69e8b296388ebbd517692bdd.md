...
Ajoute un attribut `value` de `player.pas` à l’`input` PAS. Puis ajoute un attribut `onChange` défini à une fonction fléchée qui prend `e` comme paramètre et appelle `setPlayer` avec un spread de `player` et `pas` défini à `Number(e.target.value)`.

# --hints--

Ton `input` PAS doit avoir une `value` égale à la stat `pas` du joueur.
...
Ton `input` PAS doit avoir un gestionnaire `onChange` qui appelle `setPlayer` avec un spread de `player` et `pas` défini à `Number(e.target.value)`.
