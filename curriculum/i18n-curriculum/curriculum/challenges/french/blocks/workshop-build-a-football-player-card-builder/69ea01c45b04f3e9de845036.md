...
Le problème actuel est qu’une fois que l’aperçu se rafraîchit ou que tu recharges la page, tout ce que tu as ajouté pour la carte du joueur disparaît.

Sauvegarder tous les inputs dans localStorage empêchera cela. Pour commencer, crée une constante `STORAGE_KEY` avec la chaîne `football_player_card` comme valeur.

# --hints--

Tu dois déclarer une variable `STORAGE_KEY`.
...
Ta `STORAGE_KEY` doit avoir la chaîne `football_player_card` comme valeur.
