...
Ensuite, ajoute un hook `useEffect` qui s’exécute chaque fois que l’état `player` change. À l’intérieur, utilise une fonction fléchée et ajoute un `try..catch` avec `error` comme paramètre du bloc `catch`.

# --hints--

Tu dois appeler `useEffect` à l’intérieur de `FootballPlayerCard`.
...
Ton `useEffect` doit avoir `[player]` comme tableau de dépendances.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Ton rappel `useEffect` doit contenir un bloc `try...catch` avec `error` comme paramètre du bloc `catch`.
