...
L’en-tête de la carte est terminé. Tu dois maintenant ajouter la section photo du joueur.

Après la balise fermante `div` de `card-header` et toujours à l’intérieur du `div` avec le `className` `card`, ajoute un `div` avec le `className` `card-image-wrap`. Laisse-le vide pour l’instant. Tu ajouteras l’image à l’étape suivante.

# --hints--

Tu dois ajouter un `div` avec le `className` `card-image-wrap`.
