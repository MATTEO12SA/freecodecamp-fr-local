...
Pour faire enfin fonctionner la fonctionnalité localStorage, change l’initialisation de l’état de `defaultPlayer` à `loadPlayer`. Cela garantit que l’état démarre avec les données sauvegardées disponibles.

Teste maintenant la fonctionnalité localStorage. Tout ce que tu saisis doit rester après le rafraîchissement de l’aperçu ou le rechargement de la page.

Tu peux aussi changer le lien `src` de l’image pour `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` pour voir l’image changer en temps réel. Assure-toi juste de changer le nom du joueur en Maradona.

Avec ça, ton projet de générateur de cartes de joueurs de football est terminé !

# --hints--

Ton `useState` doit être initialisé avec `loadPlayer` au lieu de `defaultPlayer`.
