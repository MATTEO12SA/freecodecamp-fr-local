...
Sous l’élément `div` avec le `className` `card-tier-badge`, ajoute un `div` avec le `className` `card-club` et le texte `Santos FC`.

# --hints--

Tu dois ajouter un `div` avec le `className` `card-club` sous `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

L’élément `.card-club` doit avoir le texte `Santos FC`.
