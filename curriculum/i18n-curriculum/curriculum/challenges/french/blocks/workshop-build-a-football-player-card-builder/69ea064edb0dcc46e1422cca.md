...
Tu vas maintenant ajouter la logique de sauvegarde dans `localStorage` avec `useEffect`.

Accéder ou sauvegarder des données dans `localStorage` est considéré comme un effet de bord en React, donc tu dois utiliser `useEffect` pour cette logique.

Commence par déstructurer `useEffect` depuis React, comme cela a été fait avec `useState`.

# --hints--

Tu dois déstructurer `useEffect` depuis `React`.
