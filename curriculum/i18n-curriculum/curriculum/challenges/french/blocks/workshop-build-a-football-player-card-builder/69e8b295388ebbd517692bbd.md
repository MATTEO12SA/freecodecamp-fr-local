...
À l’intérieur du div `card-stats`, après le premier `stat-col`, tu dois ajouter deux autres enfants.

D’abord, un `div` auto-fermé avec le `className` `stat-divider` pour afficher une fine ligne verticale entre les deux colonnes de stats, et un second `div` avec le `className` `stat-col` qui contient trois autres éléments `div` chacun avec le `className` `stat-row`.

# --hints--

Tu dois ajouter un `div` auto-fermé avec le `className` `stat-divider` après le premier `.stat-col`.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

Tu dois ajouter un second `div` avec le `className` `stat-col` après `.stat-divider`.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

Le second `.stat-col` doit contenir trois éléments `div` chacun avec le `className` `stat-row`.
