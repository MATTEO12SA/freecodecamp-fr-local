...
La section Player Info est maintenant terminée. Après la balise fermante `div` de la section Player Info et toujours à l’intérieur de `form-panel`, ajoute un nouveau `div`. À l’intérieur, ajoute un élément `p` avec le `className` `form-section-title` et le texte `Player Stats`. Puis ajoute un `div` avec le `className` `stats-grid`. Laisse la grille de stats vide pour l’instant.

# --hints--

Ton `div.form-panel` doit contenir un second `div` pour la section Player Stats.
...
La section Player Stats doit contenir un `p` avec le `className` `form-section-title` et le texte `Player Stats`.
...
La section Player Stats doit contenir un `div` avec le `className` `stats-grid`.
