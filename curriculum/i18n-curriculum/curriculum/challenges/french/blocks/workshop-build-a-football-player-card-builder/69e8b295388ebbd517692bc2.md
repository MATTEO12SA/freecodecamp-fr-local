...
En JSX et TSX, tu passes des expressions JavaScript comme valeurs d’attribut avec des accolades. Les littéraux de gabarit utilisent des backticks et `${}` pour l’interpolation. Par exemple :

```tsx
<div className={`prefix-${someVariable}`}>
```

Le `className` du card wrapper est actuellement en dur : `card-wrapper tier-gold`. Remplace `gold` par une valeur dynamique en utilisant un littéral de gabarit qui appelle `getPlayerTier` avec `player.overallRating`. Cela référencera la bonne classe de tier du CSS, selon la note globale du joueur.

# --hints--

Tu dois utiliser un littéral de gabarit avec `getPlayerTier(player.overallRating)` pour le `className` de `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

L’élément `.card-wrapper` doit avoir la classe `tier-elite` quand la note du joueur est 98.
