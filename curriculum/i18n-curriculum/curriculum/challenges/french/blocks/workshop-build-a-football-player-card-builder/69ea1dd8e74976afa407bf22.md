...
Ensuite, crée un type `Position` avec la valeur `typeof POSITIONS[number]`. Cela dépaquette le tableau `POSITIONS` en une union de ses littéraux de chaîne individuels, ce qui donne effectivement quelque chose comme `"GK" | "CB" | "LB" | ...` et ainsi de suite.

# --hints--

Tu dois créer un alias de type appelé `Position`.
...
Ton type `Position` doit être défini à `typeof POSITIONS[number]`.
