...
Remplace maintenant les nombres en dur restants par ce qui suit :
...
Tu dois remplacer le `99` en dur pour la valeur `DRI` par `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Tu dois remplacer le `41` en dur pour la valeur `DEF` par `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Tu dois remplacer le `99` en dur pour la valeur `PHY` par `player.phy`.
