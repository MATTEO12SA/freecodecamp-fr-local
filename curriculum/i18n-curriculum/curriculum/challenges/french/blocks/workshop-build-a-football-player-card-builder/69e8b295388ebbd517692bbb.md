...
Après l’élément `card-image-wrap`, ajoute un `div` avec le `className` `card-name-strip`. À l’intérieur, ajoute un `span` avec le `className` `card-name` et le texte `PELE`.

# --hints--

Tu dois ajouter un `div` avec le `className` `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Tu dois ajouter un `span` avec le `className` `card-name` à l’intérieur de `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Le span `.card-name` doit avoir le texte `PELE`.
