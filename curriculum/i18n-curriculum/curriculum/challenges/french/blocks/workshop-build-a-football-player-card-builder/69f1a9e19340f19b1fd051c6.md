...
Ajoute deux éléments `span` avec le `className` `stat-value` et `stat-label` respectivement à chaque `stat-row`.

# --hints--

Chaque `.stat-row` dans le second `.stat-col` doit contenir un `span` avec la classe `stat-value` et un `span` avec la classe `stat-label`.
