...
À l’intérieur du div `card`, ajoute un `div` avec le `className` `card-header`. Cette section contiendra la note du joueur, la position, le badge de tier et le nom du club.

# --hints--

Tu dois ajouter un `div` avec le `className` `card-header` à l’intérieur du `div` avec le `className` `.card`.
