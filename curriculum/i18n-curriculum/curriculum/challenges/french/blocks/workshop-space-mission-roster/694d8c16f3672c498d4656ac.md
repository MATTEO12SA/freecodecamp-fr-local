...
Comme précédemment, commence par valider l'entrée à l'intérieur de la fonction `swapCrewMembers`. Plus précisément, tu dois valider les indices des astronautes à échanger.

Si l'une des conditions suivantes est vraie :

- `fromIndex` est négatif

- `fromIndex` est supérieur ou égal à `crew.length`

- `toIndex` est négatif

- `toIndex` est supérieur ou égal à `crew.length`

alors, tu dois afficher `"Invalid crew indices"` dans la console et appeler `return` pour sortir de la fonction.

Tu peux utiliser l'opérateur OU `||` pour vérifier plusieurs conditions à la fois :
...
Tu dois valider les indices à l'aide d'une instruction `if`.
...
Tu dois d'abord valider les indices et, s'ils sont invalides, afficher le message `"Invalid crew indices"` avant de sortir de la fonction avec `return`.
...
Immédiatement après avoir affiché le message dans la vérification de validation, invoque `return` pour sortir de la fonction de façon anticipée.
...
Lorsqu'on lui fournit des entrées valides, ta fonction `swapCrewMembers` ne doit afficher aucun message d'erreur et ne doit pas sortir de façon anticipée.
