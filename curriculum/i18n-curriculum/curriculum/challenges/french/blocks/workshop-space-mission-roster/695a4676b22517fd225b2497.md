...
Le contrôle de mission a besoin d'une fonction de plus pour afficher un résumé de ton équipage. Crée une fonction vide nommée `printCrewSummary` qui accepte un paramètre `crew`.

# --hints--

Tu dois créer une fonction nommée `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Ta fonction `printCrewSummary` doit avoir un paramètre `crew`.
