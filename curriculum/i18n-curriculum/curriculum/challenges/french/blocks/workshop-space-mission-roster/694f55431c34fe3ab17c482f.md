...
Le contrôle de mission a demandé une nouvelle fonction pour découper un équipage en morceaux de tailles variables. Crée une fonction vide nommée `chunkCrew` qui accepte deux paramètres, `crew` et `size`.

# --hints--

Tu dois créer une fonction nommée `chunkCrew`.

```js
assert.isFunction(chunkCrew);
```

Ta fonction `chunkCrew` doit avoir les paramètres `crew` et `size`.
