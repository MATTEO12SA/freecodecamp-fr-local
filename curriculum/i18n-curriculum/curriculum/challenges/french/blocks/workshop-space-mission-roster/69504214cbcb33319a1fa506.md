...
Utilise ta fonction `chunkCrew()` pour créer des morceaux de taille `3` à partir de ton tableau `EVAReadySquad` et stocke-les dans une nouvelle variable nommée `EVAChunks`.

# --hints--

Tu dois appeler `chunkCrew()` avec `EVAReadySquad` et `3` et stocker le résultat dans une nouvelle variable nommée `EVAChunks`.
