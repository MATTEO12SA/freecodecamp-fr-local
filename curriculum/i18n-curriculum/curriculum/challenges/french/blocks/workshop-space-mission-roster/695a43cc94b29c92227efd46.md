...
Tu as ajouté un appel `console.log()` à ton script pour valider ton tableau `EVAChunks`, et maintenant cet appel n'est plus nécessaire. Pour garder le terminal propre pour les prochaines étapes, supprime le bloc de boucle `for` qui affiche les données de `EVAChunks`.

# --hints--

Tu dois supprimer le bloc de boucle `for` qui affiche les données de `EVAChunks`. Supprime le bloc entier contenant les deux boucles.
