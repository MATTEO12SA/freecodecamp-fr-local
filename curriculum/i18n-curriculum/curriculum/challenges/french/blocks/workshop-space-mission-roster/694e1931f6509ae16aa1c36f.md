...
Le contrôle de mission t'a alerté que la liste des astronautes éligibles à l'EVA doit aussi être triée par priorité décroissante. Il existe plusieurs façons de trier un tableau — la plus basique est peut-être le tri à bulles (`bubble sort`).

Le tri à bulles (`bubble sort`) fonctionne en parcourant la liste de façon répétée, en comparant les éléments voisins et en les échangeant s'ils sont dans le mauvais ordre. Après chaque passage, l'élément qui devrait venir en premier selon tes critères de tri se rapproche (ou « remonte comme une bulle » vers) sa position correcte dans le tableau. Voici comment tu peux trier `crew` par priorité décroissante en utilisant le `bubble sort` :
...
Crée une nouvelle fonction utilitaire nommée `sortByPriorityDescending` qui accepte un paramètre `crew`. Cette fonction doit trier directement le tableau `crew` fourni par priorité décroissante, utiliser deux boucles `for` imbriquées, et ne rien retourner.

# --hints--

Tu dois avoir une fonction nommée `sortByPriorityDescending` avec un paramètre `crew`.
...
Ta fonction `sortByPriorityDescending` doit utiliser deux boucles `for` imbriquées pour effectuer le tri.
...
Ta fonction `sortByPriorityDescending` doit trier le `crew` fourni par `priority` dans l'ordre décroissant.
...
Ta fonction `sortByPriorityDescending` doit trier directement le tableau `crew` fourni sur place et ne pas retourner un nouveau tableau.
