...
Le contrôle de mission a besoin d'une méthode pour copier les astronautes éligibles à l'EVA depuis un équipage. Crée une fonction vide nommée `getEVAReadyCrew` qui accepte un paramètre `crew`.

# --hints--

Tu dois créer une fonction nommée `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Ta fonction `getEVAReadyCrew` doit avoir un paramètre `crew`.
