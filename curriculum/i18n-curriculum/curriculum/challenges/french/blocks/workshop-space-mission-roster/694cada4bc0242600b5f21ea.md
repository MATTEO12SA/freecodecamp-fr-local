...
Le reste de ton équipage a été créé dans un tableau nommé `remainingCrew` avec les données suivantes :
...
Parcours le tableau `remainingCrew` et ajoute chaque astronaute à `squad` en utilisant la fonction `addCrewMember()`.

# --hints--

Tu dois utiliser une boucle `for` pour appeler `addCrewMember()` avec chaque astronaute de `remainingCrew`.
