...
Comme précédemment, tu dois valider ton entrée. Plus précisément, si `size` est inférieur à `1`, tu dois afficher `"Chunk size must be >= 1"` dans la console puis appeler `return` pour sortir de la fonction.

# --hints--

Si la `size` fournie est invalide, tu dois afficher le message d'erreur suivant dans la console, `"Chunk size must be >= 1"`.
...
Après avoir affiché le message d'erreur, tu dois immédiatement appeler `return` pour sortir de la fonction de façon anticipée.
...
Ta fonction `chunkCrew` doit passer la validation sans erreurs lorsque `size` est supérieur ou égal à 1.
