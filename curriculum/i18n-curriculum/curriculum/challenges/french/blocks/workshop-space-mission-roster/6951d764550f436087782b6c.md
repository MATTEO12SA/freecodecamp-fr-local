...
Tu as ajouté un appel `console.log()` à ton script pour valider ton tableau `EVAReadySquad`, et maintenant cet appel n'est plus nécessaire. Pour éviter que le terminal ne s'encombre dans les prochaines étapes, supprime la boucle `for` actuelle qui affiche les astronautes de `EVAReadySquad` dans la console.

# --hints--

Supprime la boucle `for` qui affiche les astronautes de `EVAReadySquad` dans la console. Tu dois supprimer le bloc entier de la boucle.
