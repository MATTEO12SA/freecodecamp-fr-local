...
Invoque `getEVAReadyCrew()` avec ta liste `updatedSquad` et stocke le résultat dans une nouvelle variable nommée `EVAReadySquad`. Ensuite, utilise une boucle `for` pour afficher le `name` de chaque astronaute du tableau `EVAReadySquad`.

# --hints--

Tu dois appeler `getEVAReadyCrew()` avec `updatedSquad` et stocker le résultat dans une nouvelle variable nommée `EVAReadySquad`.
...
Tu dois utiliser une boucle `for` pour parcourir le tableau `EVAReadySquad` et afficher le `name` de chaque astronaute.
