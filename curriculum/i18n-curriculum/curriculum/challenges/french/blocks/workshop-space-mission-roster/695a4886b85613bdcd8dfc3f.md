...
D'abord, crée une copie superficielle du tableau `crew` fourni en utilisant `slice()` et affecte-la à une variable nommée `sorted`.

# --hints--

Tu dois utiliser la méthode `slice()` pour copier le tableau `crew` fourni dans une nouvelle variable nommée `sorted`.
