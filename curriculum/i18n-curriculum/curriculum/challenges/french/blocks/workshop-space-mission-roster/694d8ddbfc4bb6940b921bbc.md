...
Tu dois créer une copie superficielle du tableau `crew` en utilisant la méthode `slice()`. Nomme le nouveau tableau `updatedCrew`.

Rappelle-toi qu'appeler `slice()` sans arguments retourne une copie du tableau entier :
...
Tu dois utiliser la méthode `slice()` pour copier le tableau `crew` fourni et stocker le résultat dans une nouvelle variable nommée `updatedCrew`.
