...
Tu avais précédemment ajouté un appel `console.log()` à ta fonction `addCrewMember` pour valider son comportement, et maintenant cet appel n'est plus nécessaire. Pour éviter que le terminal ne s'encombre dans les prochaines étapes, supprime l'appel `console.log()` actuel.

# --hints--

Supprime l'appel `console.log()` de ta fonction `addCrewMember`. Tu dois supprimer la ligne entière.
