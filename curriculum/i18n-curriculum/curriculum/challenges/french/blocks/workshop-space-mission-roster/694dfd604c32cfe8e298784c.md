...
Le contrôle de mission t'a demandé d'échanger les positions de deux astronautes dans ton tableau `squad`, plus précisément les membres aux indices `2` et `5`. Utilise ta fonction `swapCrewMembers()` pour effectuer l'échange et stocke le résultat dans une nouvelle variable nommée `updatedSquad`.

# --hints--

Tu dois utiliser `swapCrewMembers()` pour échanger les membres de `squad` aux indices `2` et `5` et stocker le tableau résultant dans une nouvelle variable nommée `updatedSquad`.
