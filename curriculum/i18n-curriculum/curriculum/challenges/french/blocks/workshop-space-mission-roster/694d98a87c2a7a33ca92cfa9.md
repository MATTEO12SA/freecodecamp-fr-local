...
Utilise une boucle `for` pour afficher le `name` de chaque astronaute du tableau `updatedCrew`. Après la boucle, retourne le tableau `updatedCrew` pour compléter ta fonction `swapCrewMembers`.

# --hints--

Tu dois utiliser une boucle `for` pour parcourir le tableau `updatedCrew` et afficher le `name` de chaque astronaute.
...
Tu dois retourner le tableau `updatedCrew` après avoir affiché les noms de ses astronautes.
