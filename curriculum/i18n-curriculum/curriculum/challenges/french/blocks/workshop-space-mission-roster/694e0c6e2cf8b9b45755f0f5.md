...
D'abord, crée un tableau vide nommé `eligible`. Ensuite, parcours chaque astronaute du tableau `crew` fourni et, s'il est éligible à l'EVA (c'est-à-dire que sa propriété `isEVAEligible` vaut `true`), ajoute-le au tableau `eligible`. Après la boucle, retourne le tableau `eligible`.

# --hints--

Tu dois créer un tableau nommé `eligible`.
...
Tu dois utiliser une boucle `for` pour parcourir le tableau `crew` fourni.
...
Tu dois retourner le tableau `eligible`.
...
Après l'appel de `getEVAReadyCrew()`, le tableau retourné ne doit contenir que les astronautes éligibles à l'EVA du tableau `crew` fourni.
