...
De retour à l'intérieur de la fonction `getEVAReadyCrew`, appelle `sortByPriorityDescending()` avec le tableau local `eligible` afin qu'il soit trié avant d'être retourné.

# --hints--

Tu dois appeler `sortByPriorityDescending()` avec le tableau local `eligible`.
