...
Tu avais précédemment ajouté un appel `console.log()` à ta fonction `swapCrewMembers` pour valider son comportement, et maintenant cet appel n'est plus nécessaire. Pour éviter que le terminal ne s'encombre dans les prochaines étapes, supprime la boucle `for` actuelle qui affiche dans la console.

# --hints--

Supprime la boucle `for` de la fonction `swapCrewMembers`. Tu dois supprimer le bloc entier de la boucle.
