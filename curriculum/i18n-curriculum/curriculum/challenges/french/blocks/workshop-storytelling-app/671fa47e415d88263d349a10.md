...
Ensuite, tu vas commencer à travailler sur le `JavaScript`. Pour cela, commence par lier le fichier `script.js` à ton HTML.

Crée un élément `script` dont l'attribut `src` pointe vers le fichier `script.js`. Par exemple :
...
Tu devrais créer un élément `script`.

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

Ton élément `script` devrait avoir un attribut `src` défini à `script.js`.
