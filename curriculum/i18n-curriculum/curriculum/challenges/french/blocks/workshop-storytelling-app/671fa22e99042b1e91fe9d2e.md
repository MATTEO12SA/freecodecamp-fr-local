...
À l'intérieur de `.btn-container`, crée trois boutons pour chaque type d'histoire. Donne à chaque bouton la classe `btn` et un `id` de `scary-btn`, `funny-btn` et `adventure-btn`. Le texte de chaque bouton doit être respectivement `Scary Story`, `Funny Story` et `Adventure Story`.

# --hints--

À l'intérieur de l'élément `.btn-container`, tu devrais avoir trois boutons.

```js
assert.lengthOf(document.querySelectorAll('.btn-container > button') ,3);
```

Les trois boutons devraient avoir la classe `btn`.

```js
assert.equal(document.querySelectorAll('.btn-container > .btn').length, 3);
```

Tu devrais avoir un bouton avec l'`id` `scary-btn` et le texte `Scary Story`.
...
Tu devrais avoir un bouton avec l'`id` `funny-btn` et le texte `Funny Story`.
...
Tu devrais avoir un bouton avec l'`id` `adventure-btn` et le texte `Adventure Story`.
