...
Crée une fonction appelée `displayStory`. À l'intérieur de la fonction, affiche `"You clicked the button"` dans la console.

# --hints--

Tu devrais avoir une fonction appelée `displayStory`.

```js
assert.isFunction(displayStory);
```

Tu devrais afficher `"You clicked the button"` dans la console à l'intérieur de la fonction `displayStory`.
