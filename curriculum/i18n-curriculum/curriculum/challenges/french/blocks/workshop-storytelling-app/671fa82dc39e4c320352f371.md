...
Pour stocker les différents types d'histoires, crée un objet appelé `storyObj`. L'objet devrait avoir trois propriétés : `scary`, `funny` et `adventure`. Chaque valeur de propriété devrait être un objet avec une propriété `story` et une propriété `borderColor`.

Assigne les valeurs suivantes aux objets individuels :
...
Tu devrais avoir un objet `storyObj`.

```js
assert.isObject(storyObj);
```

Ton objet `storyObj` devrait avoir trois propriétés : `scary`, `funny` et `adventure`.

```js
assert.containsAllKeys(storyObj, ['scary', 'funny', 'adventure']);
```

Chaque propriété devrait avoir une propriété `story` et une propriété `borderColor`.
...
La propriété `story` pour `scary` devrait avoir la valeur chaîne `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`
...
La propriété `borderColor` pour `scary` devrait avoir la valeur chaîne `"#ee4b2b"`.

```js
assert.equal(storyObj.scary.borderColor, "#ee4b2b");
```

La propriété `story` pour `funny` devrait avoir la valeur chaîne `"During a camping trip, Mark decided to show off his culinary skills by cooking dinner over an open fire. However, his attempt caused him to burn the dinner as well as his eyebrows off."`
...
La propriété `borderColor` pour `funny` devrait avoir la valeur chaîne `"#f1be32"`.

```js
assert.equal(storyObj.funny.borderColor, "#f1be32");
```

La propriété `story` pour `adventure` devrait avoir la valeur chaîne `"Lost in the heart of the Amazon rain forest, Sarah and Jake stumbled upon an ancient temple. They braved deadly traps and encountered strange wildlife, all while deciphering cryptic clues left behind by a mysterious civilization."`
...
La propriété `borderColor` pour `adventure` devrait avoir la valeur chaîne `"#acd157"`.
