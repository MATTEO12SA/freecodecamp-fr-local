...
Modifie l'écouteur d'événement de `scaryStoryBtn` pour appeler la fonction `displayStory` avec l'argument `"scary"`. Ajoute aussi un écouteur d'événement `click` aux boutons `funnyStoryBtn` et `adventureStoryBtn` qui appellent la fonction `displayStory` avec les arguments `"funny"` et `"adventure"` respectivement.

Avec cela, tu as terminé le workshop avec succès.

# --hints--

Quand tu cliques sur `scaryStoryBtn`, le paragraphe `#result` devrait afficher l'histoire effrayante `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` avec une couleur de bordure `#ee4b2b`.
...
Quand tu cliques sur `funnyStoryBtn`, le paragraphe `#result` devrait afficher l'histoire drôle avec une couleur de bordure `#f1be32`.
...
Quand tu cliques sur `adventureStoryBtn`, le paragraphe `#result` devrait afficher l'histoire d'aventure avec une couleur de bordure `#acd157`.
