...
Sélectionne le paragraphe `#result` et stocke-le dans une variable appelée `resultParagraph`. Tu utiliseras cette variable pour afficher l'histoire choisie.

# --hints--

Tu devrais sélectionner le paragraphe `#result` avec la méthode `getElementById` et le stocker dans la variable `resultParagraph`.
