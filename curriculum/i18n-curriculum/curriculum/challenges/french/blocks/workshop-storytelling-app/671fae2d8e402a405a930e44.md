...
Ajoute un écouteur d'événement pour l'événement `click` à `scaryStoryBtn` et passe-lui la fonction `displayStory`. Ensuite, clique sur ton bouton et regarde la sortie dans la console.

# --hints--

Tu devrais ajouter un écouteur d'événement pour l'événement `click` à `scaryStoryBtn` en lui passant la fonction `displayStory`.
