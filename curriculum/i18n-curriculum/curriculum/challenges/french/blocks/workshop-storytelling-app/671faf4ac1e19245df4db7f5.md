...
À l'intérieur de la fonction `displayStory`, vérifie si l'objet `storyObj` a une propriété qui correspond au paramètre `genre` et définis le contenu texte du paragraphe `#result` sur l'histoire du genre sélectionné.

Définis aussi la couleur de bordure de `storyContainer` avec la propriété `borderColor` du genre sélectionné.

Teste cela en appelant la fonction `displayStory` avec le genre `scary`. La couleur de bordure devrait être `#ee4b2b` et l'histoire devrait être `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Tu devrais appeler la fonction `displayStory` avec le genre `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Quand tu appelles `displayStory("scary")`, la couleur de bordure devrait être `#ee4b2b`.
...
Quand tu appelles `displayStory("scary")`, l'histoire affichée devrait être `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
