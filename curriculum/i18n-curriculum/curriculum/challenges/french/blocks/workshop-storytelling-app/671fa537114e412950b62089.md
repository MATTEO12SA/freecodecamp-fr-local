...
Dans ton fichier JavaScript, sélectionne l'élément `.story-container` et stocke-le dans une variable appelée `storyContainer`.

# --hints--

Tu devrais avoir une variable `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Tu devrais sélectionner l'élément `.story-container` avec la méthode `querySelector` et le stocker dans la variable `storyContainer`.
