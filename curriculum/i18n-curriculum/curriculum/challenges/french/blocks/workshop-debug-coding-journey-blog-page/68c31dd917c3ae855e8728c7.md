...
Pour la dernière partie de l'atelier, il y a quelques changements nécessaires pour la section contact en bas.

Le premier changement serait d'envelopper la section contact à l'intérieur d'un élément `footer`. Cela aidera les lecteurs d'écran à identifier cette section comme étant le pied de page.

# --hints--

Ta page devrait avoir un élément `footer`.
...
Ton élément `footer` devrait contenir un élément `h3` avec le texte `Contact Me`.
...
Ton élément `footer` devrait contenir un élément `p` avec le texte `Email me at camperbot@blog.io`.
