...
Pour le moment, la section `footer` a un titre `h3`. Mais il devrait être un titre `h2` pour maintenir une hiérarchie de titres correcte. 

Change le `h3` en un `h2`.

# --hints--

Ton élément `footer` devrait contenir un élément `h2` au lieu d'un élément `h3`.
