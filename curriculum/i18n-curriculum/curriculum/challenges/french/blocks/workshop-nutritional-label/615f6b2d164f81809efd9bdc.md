...
Après ton dernier élément `.divider`, crée un élément `p` et donne-lui le texte `Total Fat 8g 10%`. Ensuite, enveloppe le texte `Total Fat` dans un élément `span`, le texte `10%` dans un autre, et donne à chacun une classe `bold`.
