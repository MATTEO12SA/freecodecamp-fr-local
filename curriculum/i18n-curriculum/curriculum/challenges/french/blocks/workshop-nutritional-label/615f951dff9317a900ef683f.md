...
Donne au sélecteur `.note` un padding gauche et droit de `8px`, en retirant les paddings haut et bas. Règle aussi la propriété `text-indent` sur `-8px`.

Avec ces derniers changements, ton étiquette nutritionnelle est terminée !

# --hints--

Ton sélecteur `.note` devrait avoir une propriété `padding` définie sur `0 8px`.
...
Ton sélecteur `.note` devrait avoir une propriété `text-indent` définie sur `-8px`.
