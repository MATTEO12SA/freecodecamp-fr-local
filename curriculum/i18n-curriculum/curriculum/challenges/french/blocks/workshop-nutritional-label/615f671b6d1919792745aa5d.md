...
Crée un nouvel élément `div` sous ton élément `.medium`. Donne-lui un attribut `class` défini sur `daily-value small-text`. Dans ce nouveau `div`, ajoute un élément `p` avec le texte `% Daily Value *`, et règle l'attribut `class` sur `bold right`.

# --hints--

Tu devrais créer un nouvel élément `div` après ton élément `.medium`.
...
Ton nouveau `div` devrait avoir un attribut `class` défini sur `daily-value small-text`.
...
Ton nouvel élément `div` devrait avoir un élément `p`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.localName === 'p');
```

Ton nouvel élément `p` devrait avoir le texte `% Daily Value *`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.textContent === '% Daily Value *');
```

Ton nouvel élément `p` devrait avoir un attribut `class` défini sur `bold right`.
