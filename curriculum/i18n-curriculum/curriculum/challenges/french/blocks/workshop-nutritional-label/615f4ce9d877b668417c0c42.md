...
Enveloppe tout ce qui est dans l'élément `.label` dans un nouvel élément `header`.

# --hints--

Tu devrais créer un nouvel élément `header`.

```js
assert(document.querySelector('header'));
```

Ton élément `header` devrait être dans ton élément `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Tes éléments `h1`, `div` et `p` devraient être dans ton nouvel élément `header`.
