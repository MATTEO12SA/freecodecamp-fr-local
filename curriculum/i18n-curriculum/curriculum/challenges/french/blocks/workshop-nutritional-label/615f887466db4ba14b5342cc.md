...
Après ton dernier diviseur, crée un autre élément `p` avec le texte `Protein 3g`. Utilise les classes nécessaires pour retirer la bordure du bas, et un `span` pour mettre `Protein` en gras. Enveloppe ensuite le texte `Protein 3g`, y compris le nouvel élément `span`, dans un nouvel élément `span`.

À la suite de cet élément, crée un grand diviseur.
...
Ton nouveau `div` devrait avoir l'attribut `class` défini sur `divider large`.
