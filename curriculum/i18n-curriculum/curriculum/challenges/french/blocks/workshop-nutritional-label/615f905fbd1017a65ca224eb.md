...
Ajoute un diviseur moyen après ton élément `.daily-value`. Sous ce nouveau diviseur, crée un élément `p` avec l'attribut `class` défini sur `note`.

Donne à l'élément `p` le texte suivant :
...
Tu devrais créer un nouveau `div` après ton élément `.daily-value`.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

Ton nouveau `div` devrait avoir la `class` définie sur `divider medium`.
...
Tu devrais créer un élément `p` après ton nouvel élément `div`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Ton nouvel élément `p` devrait avoir la `class` définie sur `note`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('note'));
```

Ton nouvel élément `p` devrait contenir le texte fourni.
