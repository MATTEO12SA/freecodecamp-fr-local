...
Sous ton élément `.double-indent`, ajoute un nouvel élément `p` avec le texte `Includes 10g Added Sugars 20%`. Ton nouvel élément `p` devrait être doublement indenté et ne pas avoir de bordure du bas. Utilise un `span` pour mettre `20%` en gras et aligné à droite.

Crée ensuite un autre diviseur après cet élément `p`.

# --hints--

Tu devrais créer un nouvel élément `p` et un nouvel élément `div` à la fin de ton élément `.daily-value`.
...
Ton nouvel élément `p` devrait avoir le texte `Includes 10g Added Sugars 20%`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Includes 10g Added Sugars[\s|\n]+20%/));
```

Ton nouvel élément `p` devrait avoir l'attribut `class` défini sur `double-indent no-divider`.
