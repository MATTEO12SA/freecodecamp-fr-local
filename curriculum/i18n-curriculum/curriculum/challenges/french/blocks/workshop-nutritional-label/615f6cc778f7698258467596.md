...
Sous ton élément avec le texte `Total Fat`, crée un nouvel élément `p` avec le texte `Saturated Fat 1g 5%`. Enveloppe le `5%` dans un `span` avec l'attribut `class` défini sur `bold`. Dans ce cas, ça suffit pour aligner le pourcentage à `5%`.

# --hints--

Tu devrais créer un nouvel élément `p` sous ton élément avec le texte `Total Fat`.
...
Ton nouvel élément `p` devrait avoir le texte `Saturated Fat 1g 5%`.
...
Ton élément `span` devrait entourer le texte `5%`.
