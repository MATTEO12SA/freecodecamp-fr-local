...
Crée un autre `.divider` après ton dernier élément `p`.

# --hints--

Tu devrais créer un nouvel élément `div` à la fin de ton élément `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'div');
```

Ton nouvel élément `div` devrait avoir l'attribut `class` défini sur `divider`.
