...
Après ton dernier `.divider`, crée un autre élément `p` avec le texte `Trans Fat 0g`. Mets le mot `Trans` en italique en l'enveloppant dans un élément `i`. Donne au nouvel élément `p` l'attribut `class` défini sur `indent no-divider`. Enveloppe `Trans Fat 0g` dans un élément `span` pour l'alignement.
...
Ton nouvel élément `p` devrait avoir le texte `Trans Fat 0g`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

Ton nouvel élément `p` devrait avoir l'attribut `class` défini sur `indent no-divider`.
...
Ton nouvel élément `p` devrait avoir un élément `i`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelector('i'));
```

Ton élément `i` devrait entourer le texte `Trans`.
...
Ton élément `span` devrait entourer le texte `Trans Fat 0g`.
