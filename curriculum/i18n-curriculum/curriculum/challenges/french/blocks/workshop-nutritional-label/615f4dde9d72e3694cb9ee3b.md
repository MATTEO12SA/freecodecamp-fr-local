...
Mets maintenant à jour ton sélecteur `h1` pour qu'il devienne `header h1` afin de cibler spécifiquement ton élément `h1` à l'intérieur de ton nouveau `header`.

# --hints--

Tu devrais avoir un sélecteur `header h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

Tu ne devrais pas avoir de sélecteur `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

Tu ne devrais pas changer les propriétés du sélecteur.
