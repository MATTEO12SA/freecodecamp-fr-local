...
Sous ton dernier élément `p`, crée un autre élément `p` avec le texte `Sodium 160mg 7%`. Place `Sodium` et `7%` chacun dans leur propre `span` avec une classe `bold` comme tu l'as fait avec les autres.

Ensuite, ajoute un élément `span` supplémentaire autour de `Sodium 160mg` pour l'alignement à nouveau.
...
Ton nouvel élément `p` devrait avoir le texte `Sodium 160mg 7%`.
...
Le `span` autour de `Sodium` devrait avoir l'attribut `class` défini sur `bold`.
...
Un élément `span` devrait entourer le texte `7%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '7%');
```

L'élément `span` autour de `7%` devrait avoir l'attribut `class` défini sur `bold`.
