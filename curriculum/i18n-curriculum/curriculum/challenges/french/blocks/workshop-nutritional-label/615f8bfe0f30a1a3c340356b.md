...
Crée un autre élément `p`, donne-lui le texte `Calcium 260mg 20%`. Aligne `20%` à droite. Sous ça, crée un élément `p` avec le texte `Iron 8mg 45%`, en alignant `45%` à droite.

# --hints--

Tu devrais créer deux nouveaux éléments `p` à la fin de ton élément `.daily-value`.
...
Ton premier nouvel élément `p` devrait avoir le texte `Calcium 260mg 20%`.
...
Ton premier nouvel élément `p` devrait avoir un élément `span`.
...
Ton premier élément `span` ne devrait pas être en gras.
...
Ton premier élément `span` devrait entourer le texte `20%`.
