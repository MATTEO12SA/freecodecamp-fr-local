...
Sous ton dernier élément `p`, ajoute un autre élément `p` avec le texte `Dietary Fiber 4g`. Donne à l'élément `p` la `class` nécessaire pour l'indenter et retirer la bordure de séparation. Crée ensuite un diviseur sous cet élément `p`.
...
Ton nouvel élément `p` devrait avoir le texte `Dietary Fiber 4g`.
