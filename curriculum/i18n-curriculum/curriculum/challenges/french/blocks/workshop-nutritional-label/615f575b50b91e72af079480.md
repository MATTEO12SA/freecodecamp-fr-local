...
Crée un nouveau sélecteur `.left-container p` en réglant le margin haut et bas sur `-5px`, et le margin gauche et droit sur `-2px`. Règle aussi la `font-size` sur `2em` et la `font-weight` sur `700`.

# --hints--

Tu devrais avoir un nouveau sélecteur `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Ton nouveau sélecteur `.left-container p` devrait avoir une propriété `margin` définie sur `-5px -2px`.
...
Ton nouveau sélecteur `.left-container p` devrait avoir une propriété `font-size` définie sur `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Ton nouveau sélecteur `.left-container p` devrait avoir une propriété `font-weight` définie sur `700`.
