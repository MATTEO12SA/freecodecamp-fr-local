...
Pour t'assurer que le texte saisi dans les éléments `input` et `textarea`, ainsi que dans leurs placeholders, soit visible, définis leur couleur.

Sélectionne `input, input::placeholder, textarea` et définis `color` sur `whitesmoke`.

# --hints--

Tu devrais cibler `input`, `input::placeholder`, `textarea`.
