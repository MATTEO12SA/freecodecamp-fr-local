...
Tu devrais donner une taille et une forme à tes boutons radio personnalisés.

En ciblant toujours `.contact-method-radio-btn`, définis `width` sur `20px`, `height` sur `20px`, `border-radius` sur `50%` et `border` sur `2px solid gray`.
...
Définis la propriété `width` sur `20px`.
...
Définis la propriété `height` sur `20px`.
...
Définis la propriété `border` sur `2px solid gray`.
