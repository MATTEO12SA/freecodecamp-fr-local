...
Tu dois positionner le cercle intérieur et le rendre invisible au départ.

En ciblant toujours `.contact-method-radio-btn::before`, définis `transform` sur `translate(3px, 3px) scale(0)`. La fonction `translate` déplace légèrement le cercle, et `scale(0)` le réduit à une taille nulle pour qu'il soit masqué par défaut.

Ajoute aussi une `transition` de `all 0.3s ease-in` pour créer une animation fluide quand il apparaît. Cela signifie que tout changement de style s'animera sur 0,3 seconde, en commençant lentement puis en accélérant.

# --hints--

Tu devrais continuer à cibler le pseudo-élément `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Définis la propriété `transform` sur `translate(3px, 3px) scale(0)`.
...
Définis la propriété `transition` sur `all 0.3s ease-in`.
