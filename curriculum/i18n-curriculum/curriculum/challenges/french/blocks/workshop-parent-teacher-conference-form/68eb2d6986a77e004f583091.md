...
Maintenant, il est temps de personnaliser les boutons radio. Par défaut, les boutons radio ont l'apparence native du navigateur. Tu veux retirer cela pour pouvoir créer un aspect personnalisé.

Sélectionne l'élément avec la classe `contact-method-radio-btn`. Définis `appearance` sur `none`.

# --hints--

Tu devrais cibler l'élément avec la classe `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Définis la propriété `appearance` sur `none`.
