...
Le texte de description doit être un peu plus grand. Sélectionne l'élément avec la classe `description` et définis son `font-size` sur `1.2rem`.

# --hints--

Tu devrais cibler l'élément avec la classe `description`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.description'));
```

Définis la propriété `font-size` sur `1.2rem`.
