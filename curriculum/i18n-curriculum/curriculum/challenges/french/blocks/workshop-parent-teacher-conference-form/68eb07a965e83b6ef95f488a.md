...
En continuant avec le conteneur principal, donne à la classe `container` une `width` de `80%` et une `max-width` de `600px`. Ajoute aussi un `border-radius` de `10px` et définis `margin` sur `20px auto` pour le centrer horizontalement.

# --hints--

Tu devrais cibler la classe `container`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.container'));
```

Définis la propriété `width` sur `80%`.
...
Définis la propriété `max-width` sur `600px`.
...
Définis la propriété `border-radius` sur `10px`.
...
Définis la propriété `margin` sur `20px auto`.
