...
Maintenant, tu devrais créer le cercle intérieur qui apparaîtra quand le bouton radio est coché. Tu utiliseras le pseudo-élément `::before` pour cela.

Un pseudo-élément comme `::before` te permet d'insérer du contenu supplémentaire avant l'élément réel. C'est souvent utilisé à des fins décoratives.

En ciblant le pseudo-élément `.contact-method-radio-btn::before`, définis un `display` de `block`, un `content` de `" "`, une `width` de `10px`, une `height` de `10px` et un `border-radius` de `50%`.

# --hints--

Tu devrais avoir un sélecteur `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Définis la propriété `display` sur `block`.
...
Définis la propriété `content` sur `" "`.
...
Définis la propriété `width` sur `10px`.
...
Définis la propriété `height` sur `10px`.
...
Définis la propriété `border-radius` sur `50%`.
