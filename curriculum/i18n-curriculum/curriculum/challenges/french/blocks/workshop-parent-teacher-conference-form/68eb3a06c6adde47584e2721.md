...
Il est temps de styliser le bouton d'envoi.

Sélectionne l'élément avec la classe `submit-btn`. Définis `cursor` sur `pointer` (pour indiquer qu'il est cliquable), `background-color` sur `royalblue`, `color` sur `whitesmoke`, `border` sur `none`, `border-radius` sur `6px` et `padding` sur `12px 20px`.

# --hints--

Tu devrais cibler l'élément avec la classe `submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Définis la propriété `cursor` sur `pointer`.
...
Définis la propriété `background-color` sur `royalblue`.
...
Définis la propriété `color` sur `whitesmoke`.
...
Définis la propriété `border` sur `none`.
...
Définis la propriété `border-radius` sur `6px`.
...
Définis la propriété `padding` sur `12px 20px`.
