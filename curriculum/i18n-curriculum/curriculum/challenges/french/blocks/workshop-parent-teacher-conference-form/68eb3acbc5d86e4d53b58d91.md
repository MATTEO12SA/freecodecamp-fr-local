...
Définis `font-size` sur `1.1rem`, `display` sur `block` et `margin` sur `auto` pour centrer le bouton.

# --hints--

Tu devrais continuer à cibler la classe `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Définis la propriété `font-size` sur `1.1rem`.
...
Définis la propriété `margin` sur `auto`.
