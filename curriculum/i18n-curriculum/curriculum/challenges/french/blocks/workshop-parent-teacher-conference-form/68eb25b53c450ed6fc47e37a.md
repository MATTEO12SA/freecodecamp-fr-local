...
Par défaut, les éléments `label` sont en ligne. Pour les empiler verticalement (sauf les labels de tes boutons radio), utilise la pseudo-classe `:not()`.

La pseudo-classe `:not()` inverse une sélection. Ici, elle sélectionne tous les éléments `label` qui n'ont pas la classe `contact-method`.

Sélectionne `label:not(.contact-method)`, puis définis `display` sur `block` et `margin` sur `10px 0`.

# --hints--

Tu devrais avoir un sélecteur `label:not(.contact-method)`.
...
Définis la propriété `margin` sur `10px 0`.
