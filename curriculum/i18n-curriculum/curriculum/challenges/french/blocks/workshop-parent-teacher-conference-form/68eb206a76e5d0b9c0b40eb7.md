...
Il est temps de styliser les éléments `fieldset` pour leur donner une bordure distincte.

Sélectionne l'élément `fieldset`. Définis son `border` sur `1px solid gray`, son `border-radius` sur `5px`, sa `margin` sur `20px 0` et son `padding` sur `20px`.

# --hints--

Tu devrais cibler l'élément `fieldset`.
...
Définis la propriété `margin` sur `20px 0`.
...
Définis la propriété `padding` sur `20px`.
