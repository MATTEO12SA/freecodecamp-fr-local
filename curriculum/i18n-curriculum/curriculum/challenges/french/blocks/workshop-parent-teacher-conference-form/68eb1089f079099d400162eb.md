...
Tu as ajouté une classe `center` à certains éléments dans le HTML. Maintenant, tu devrais définir cette classe pour centrer ces éléments.

Cible la classe `center` et centre les éléments qui l'ont avec une propriété `text-align` définie sur `center`.

# --hints--

Tu devrais cibler les éléments avec la classe `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Tu devrais définir la propriété `text-align` sur `center`.
