...
Passons au badge `Most popular`. Tu dois le placer dans le coin supérieur droit de la carte, donc tu peux utiliser certaines classes utilitaires de positionnement de Tailwind pour cela.

Voici comment utiliser le positionnement CSS dans Tailwind :
...
Assigne la classe `relative` au `div` enveloppeur et `absolute` au `div` avec le texte `Most popular`.

# --hints--

Ton élément `div` enveloppeur du plan premium doit avoir la classe `relative`.
...
Ton élément `div` avec le texte `Most popular` doit avoir la classe `absolute`.
