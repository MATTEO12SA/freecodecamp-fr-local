...
Les marqueurs des éléments de liste ont besoin d’un peu plus d’espace. Donc, donne à chaque `span` dans les éléments de liste les classes `text-green-700` et `mr-2`.

Ajoute aussi l’attribut `aria-hidden` à chacun des éléments `span`.

# --hints--

Tes éléments `span` doivent avoir la classe `text-green-700`.
