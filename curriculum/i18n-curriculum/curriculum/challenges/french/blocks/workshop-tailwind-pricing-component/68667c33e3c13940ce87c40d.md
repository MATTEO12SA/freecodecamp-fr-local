...
Le `span` qui contient la durée a aussi besoin de style, donc ajoute-lui les classes `text-base`, `font-medium` et `text-fuchsia-300`.
...
Ton élément `span` doit avoir la classe `text-fuchsia-300`.
