...
Maintenant, tu dois travailler sur l’espacement des éléments à l’intérieur de ce plan.

Ajoute la classe utilitaire `rounded-xl` pour adoucir les angles vifs de l’anneau, `p-8` pour augmenter le padding, et `gap-6` pour créer de l’espace entre le contenu et le bouton CTA.

# --hints--

Ton élément `div` doit avoir la classe `rounded-xl`.
...
Ton élément `div` doit avoir la classe `p-8`.
...
Ton élément `div` doit avoir la classe `gap-6`.
