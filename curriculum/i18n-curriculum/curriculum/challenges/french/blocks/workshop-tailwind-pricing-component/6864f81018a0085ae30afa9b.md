...
La coche dans chaque `span` au début d’un élément de liste est décorative. Ajoute `aria-hidden` à ces éléments `span` pour les masquer aux lecteurs d’écran.

Assigne aussi les classes `text-green-700` et `mr-2` à chacun des éléments `span`.

# --hints--

Tes deux éléments `span` doivent avoir l’attribut `aria-hidden`.
...
Tes deux éléments `span` doivent avoir la classe `text-green-700`.
...
Tes deux éléments `span` doivent avoir la classe `mr-2`.
