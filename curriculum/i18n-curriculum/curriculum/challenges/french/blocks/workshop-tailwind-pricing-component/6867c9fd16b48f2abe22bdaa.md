...
Le prochain élément à styliser est le texte du prix, y compris le `span` qu’il contient. Donne à l’élément `p` du prix les classes `text-4xl`, `font-bold` et `text-gray-900`.

# --hints--

Ton élément `p` doit avoir la classe `text-4xl`.
...
Ton élément `p` doit avoir la classe `font-bold`.
...
Ton élément `p` doit avoir la classe `text-gray-900`.
