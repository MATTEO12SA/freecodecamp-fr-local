...
Maintenant, tu dois travailler sur le placement des éléments à l’intérieur de la carte du plan.

Pour cela, configure le conteneur en display grid et précise une taille de ligne personnalisée avec des crochets dans ta classe Tailwind.

Voici comment définir une ligne de grille personnalisée avec des unités fractionnaires :
...
Donne au `div` extérieur du plan les classes utilitaires `grid` et `grid-rows-[1fr_auto]`.

# --hints--

Ton élément `div` doit avoir la classe `grid`.
...
Ton élément `div` doit avoir la classe `grid-rows-[1fr_auto]`.
