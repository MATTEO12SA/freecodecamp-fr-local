...
En passant au `h2` à l’intérieur du **plan Listener**, donne-lui la classe utilitaire `text-lg` pour l’agrandir, `font-semibold` pour le rendre plus gras, et `text-indigo-600` pour lui donner une belle couleur indigo.

# --hints--

Ton élément `h2` doit avoir la classe `text-lg`.
...
Ton élément `h2` doit avoir la classe `font-semibold`.
...
Ton élément `h2` doit avoir la classe `text-indigo-600`.
