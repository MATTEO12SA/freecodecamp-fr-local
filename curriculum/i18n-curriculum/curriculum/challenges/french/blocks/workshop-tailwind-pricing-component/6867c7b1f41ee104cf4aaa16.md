...
Maintenant, tu vas utiliser `grid` pour disposer les éléments à l’intérieur du `div` enveloppeur.

Ajoute les classes `grid`, `grid-rows-[1fr_auto]` et `gap-6` au `div` enveloppeur.

# --hints--

Ton élément `div` enveloppeur du plan family doit avoir la classe `grid`.
...
Ton élément `div` enveloppeur du plan family doit avoir la classe `grid-rows-[1fr_auto]`.
...
Ton élément `div` enveloppeur du plan family doit avoir la classe `gap-6`.
