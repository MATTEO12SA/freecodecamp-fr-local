...
La carte de tarification premium doit être celle qui se démarque le plus sur la page. C’est aussi la seule carte qui aura un badge `Most popular`. Pour faire ressortir cette carte et son badge, tu peux utiliser les classes utilitaires de positionnement relatif et d’échelle de Tailwind.

Voici comment agrandir un élément avec l’échelle dans Tailwind CSS :
...
Tu t’occuperas du positionnement dans les prochaines étapes. Pour l’instant, assigne la classe `scale-105` au `div` enveloppeur pour le rendre un peu plus grand que les autres cartes de tarification.

# --hints--

Ton élément `div` enveloppeur du plan premium doit avoir la classe `scale-105`.
