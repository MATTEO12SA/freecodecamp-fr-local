...
Pour le `span` à l’intérieur de l’élément `p` qui a le texte `/month`, donne-lui les classes `text-base`, `font-medium` et `text-gray-500`. Cela le distinguera du prix lui-même en rendant le texte plus clair.

# --hints--

Ton élément `span` doit avoir la classe `text-base`.
...
Ton élément `span` doit avoir la classe `font-medium`.
...
Ton élément `span` doit avoir la classe `text-gray-500`.
