...
Cela couvre un peu de style et d’espacement de base pour la carte, y compris le bouton CTA. Pour le reste du contenu intérieur, tu dois tout disposer en grille en ciblant le `div` intérieur.

Assigne les classes `grid`, `grid-rows-[auto_auto_auto_1fr]` et `gap-y-2` au `div` intérieur.

# --hints--

Ton élément `div` intérieur du plan family doit avoir la classe `grid`.
...
Ton élément `div` intérieur du plan family doit avoir la classe `grid-rows-[auto_auto_auto_1fr]`.
...
Ton élément `div` intérieur du plan family doit avoir la classe `gap-y-2`.
