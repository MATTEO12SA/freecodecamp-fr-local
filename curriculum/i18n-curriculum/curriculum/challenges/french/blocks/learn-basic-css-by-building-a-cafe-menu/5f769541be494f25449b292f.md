...
En utilisant votre nouveau `flavor` classe comme sélecteur, définissez la `text-align` valeur de la propriété à `left`.

# --hints--

Vous devriez avoir un `flavor` sélecteur de classe.
...
Vous devriez mettre le `text-align` propriété `left`.
...
Votre `flavor` sélecteur de classe doit définir le `text-align` propriété `left`.
