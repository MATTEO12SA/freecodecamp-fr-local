...
Dans les leçons précédentes, tu as appris à travailler avec la méthode `filter()`, qui sert à renvoyer un nouveau tableau de résultats filtrés.
...
Crée une fonction `getBooksByAuthor` avec deux paramètres : un tableau contenant des objets de livres et une chaîne contenant l'auteur.

La fonction doit renvoyer un tableau contenant les livres d'un auteur particulier.

# --hints--

Tu dois créer une fonction `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

Ta fonction `getBooksByAuthor` doit avoir deux paramètres.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

Ta fonction `getBooksByAuthor` doit renvoyer un tableau.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

Ta fonction `getBooksByAuthor` doit renvoyer le bon nombre de livres pour n'importe lequel des auteurs.
