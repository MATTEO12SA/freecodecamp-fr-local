...
Ce serait utile de tester ta fonction `getBooksByAuthor` avec un autre auteur.

Commence par afficher le message `"\nList of books by James Clear:\n"` dans la console.

Sous ce `console.log()`, ajoute un autre `console.log()`. Dans ce `console.log()`, appelle la fonction `getBooksByAuthor()` avec `library` et `"James Clear"` comme arguments.

Tu dois maintenant voir tous les livres de cet auteur précis dans la console.

# --hints--

Tu dois afficher le message `"\nList of books by James Clear:\n"` dans la console.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

Tu dois avoir un `console.log(getBooksByAuthor(library, "James Clear"))`.
