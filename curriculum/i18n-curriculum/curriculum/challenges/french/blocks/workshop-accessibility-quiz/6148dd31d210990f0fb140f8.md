...
Il est utile de voir la bordure par défaut autour des éléments `fieldset` pendant le développement. Cependant, ce n'est peut-être pas le style que tu souhaites.

Supprime la bordure et le padding inférieur sur les éléments `.question`.

# --hints--

Tu dois utiliser le sélecteur `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

La balise `.question` doit avoir une propriété `border` de `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

La balise `.question` doit avoir une propriété `padding-bottom` de `0`.
