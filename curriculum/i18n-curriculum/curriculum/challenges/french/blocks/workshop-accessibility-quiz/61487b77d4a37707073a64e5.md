...
Quand la largeur de l'écran est faible, le `h1` ne renvoie pas son contenu textuel à la ligne comme il le devrait. Aligne le texte de l'élément `h1` au centre.

Ensuite, donne au `main` un padding tel que l'en-tête de section `Student Info` puisse être entièrement visible.

# --hints--

La balise `h1` doit avoir une propriété `text-align` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Tu dois ajouter un sélecteur `main` pour cibler l'élément `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

La balise `main` doit avoir un `padding-top` d'au moins `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Tu dois seulement modifier la valeur de `padding-top`.
