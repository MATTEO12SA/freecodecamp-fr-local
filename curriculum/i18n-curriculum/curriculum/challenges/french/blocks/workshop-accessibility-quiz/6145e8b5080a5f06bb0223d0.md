...
Donne aux éléments `label` un texte tel que l'`input` vienne avant le texte. Ensuite, donne aux éléments `input` une `value` qui correspond au texte.

Le texte doit être soit `True`, soit `False`.

# --hints--

Le premier élément `label` doit avoir du contenu textuel.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.textContent?.trim());
```

Le deuxième élément `label` doit avoir du contenu textuel.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.textContent?.trim());
```

Le troisième élément `label` doit avoir du contenu textuel.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.textContent?.trim());
```

Le quatrième élément `label` doit avoir du contenu textuel.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.textContent?.trim());
```

Tu dois placer le contenu textuel du premier `label` après l'élément `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Tu dois placer le contenu textuel du deuxième `label` après l'élément `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Tu dois placer le contenu textuel du troisième `label` après l'élément `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Tu dois placer le contenu textuel du quatrième `label` après l'élément `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Le premier élément `label` doit avoir le texte `True` ou `False`.

```js
assert.include(['True', 'False'], document.querySelectorAll('ul.answers-list > li > label')?.[0]?.textContent?.trim());
```

Le deuxième élément `label` doit avoir le texte `True`.
...
Le deuxième élément `label` doit avoir le texte `False`.
...
Le troisième élément `label` doit avoir le texte `True` ou `False`.

```js
assert.include(['True', 'False'], document.querySelectorAll('ul.answers-list > li > label')?.[2]?.textContent?.trim());
```

Le quatrième élément `label` doit avoir le texte `True`.
...
Le quatrième élément `label` doit avoir le texte `False`.
...
Le premier élément `input` doit avoir une `value` correspondant au contenu textuel du `label`.
...
Le deuxième élément `input` doit avoir une `value` correspondant au contenu textuel du `label`.
...
Le troisième élément `input` doit avoir une `value` correspondant au contenu textuel du `label`.
...
Le quatrième élément `input` doit avoir une `value` correspondant au contenu textuel du `label`.
