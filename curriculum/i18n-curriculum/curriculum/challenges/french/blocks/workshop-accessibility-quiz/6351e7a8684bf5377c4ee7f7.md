...
Même si ce n'est pas obligatoire pour les éléments `label` avec un `input` imbriqué, c'est tout de même une bonne pratique de relier explicitement un `label` à son élément `input` correspondant.

Maintenant, ajoute un attribut `for` à chacun de tes quatre `label`s qui relie le `label` à son `input` radio correspondant.

# --hints--

Le premier élément `label` doit avoir un attribut `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Le premier élément `label` doit avoir un attribut `for` correspondant au `id` de son élément `input`.
...
Le deuxième élément `label` doit avoir un attribut `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Le deuxième élément `label` doit avoir un attribut `for` correspondant au `id` de son élément `input`.
...
Le troisième élément `label` doit avoir un attribut `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Le troisième élément `label` doit avoir un attribut `for` correspondant au `id` de son élément `input`.
...
Le quatrième élément `label` doit avoir un attribut `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Le quatrième élément `label` doit avoir un attribut `for` correspondant au `id` de son élément `input`.
