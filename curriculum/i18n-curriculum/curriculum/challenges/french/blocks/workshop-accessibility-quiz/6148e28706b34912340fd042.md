...
Centre horizontalement tout le texte dans l'élément `address`, et ajoute du padding.

# --hints--

Tu dois utiliser le sélecteur d'élément `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

`address` doit avoir une propriété `text-align` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

`address` doit avoir un `padding-top` d'au moins `1px`.
...
`address` doit avoir un `padding-right` d'au moins `1px`.
...
`address` doit avoir un `padding-bottom` d'au moins `1px`.
...
`address` doit avoir un `padding-left` d'au moins `1px`.
