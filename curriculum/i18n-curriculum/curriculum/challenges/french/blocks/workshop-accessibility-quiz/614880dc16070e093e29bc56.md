...
Remplace la marge supérieure des éléments `h2` par `60px` de padding supérieur.

# --hints--

La balise `h2` doit avoir une propriété `margin-top` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

La balise `h2` doit avoir une propriété `padding-top` de `60px`.
