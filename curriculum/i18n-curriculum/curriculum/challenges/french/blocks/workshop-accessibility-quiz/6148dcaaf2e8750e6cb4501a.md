...
Fais apparaître les éléments `h3` comme plus prioritaires, avec les propriétés CSS suivantes :
...
Tu dois utiliser le sélecteur d'élément `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

L'élément `h3` doit avoir une propriété `margin-top` de `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

L'élément `h3` doit avoir une propriété `padding-left` de `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

L'élément `h3` doit avoir une propriété `font-size` de `1.375ren`.
