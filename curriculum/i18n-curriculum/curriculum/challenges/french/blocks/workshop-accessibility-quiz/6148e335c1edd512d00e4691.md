...
Cliquer sur les liens de navigation devrait faire sauter le viewport vers la section pertinente. Cependant, ce saut peut désorienter certains utilisateurs.

Sélectionne tous les éléments, et définis le `scroll-behavior` sur `smooth`.

# --hints--

Tu dois utiliser le sélecteur `*`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('*'));
```

`*` doit avoir une propriété `scroll-behavior` de `smooth`.
