...
Cible tous les éléments `label` dans les éléments `.info`, définis leur `width` sur `10%`, et fais en sorte qu'ils ne puissent pas descendre sous `55px`.

# --hints--

Tu dois utiliser soit le sélecteur `.info label`, soit le sélecteur `.info > label`.
...
Les éléments `label` doivent avoir une propriété `width` de `10%`.
...
Les éléments `label` doivent avoir une propriété `min-width` de `55px`.
