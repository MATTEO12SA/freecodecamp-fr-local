...
Dans le deuxième élément `section`, ajoute deux éléments `div` avec une classe de `question-block`.

Ensuite, dans chaque élément `div.question-block`, ajoute un élément `h3` avec un texte composé de nombres croissants, en commençant à `1`, et un élément `fieldset` avec une classe de `question`.

# --hints--

Tu dois imbriquer deux éléments `div` dans le deuxième élément `section`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.length, 2);
```

Le premier nouvel élément `div` doit avoir une classe de `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

Le deuxième nouvel élément `div` doit avoir une classe de `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[1]?.className, 'question-block');
```

Tu dois imbriquer un élément `h3` dans l'élément `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')?.length, 2);
```

Le premier élément `h3` doit avoir le contenu textuel `1`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')[0]?.textContent.trim(), '1');
```

Le deuxième élément `h3` doit avoir le contenu textuel `2`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')[1]?.textContent.trim(), '2');
```

Tu dois imbriquer un élément `fieldset` dans l'élément `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.length, 2);
```

Tu dois placer le premier élément `fieldset` après le premier élément `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block > h3 + fieldset'));
```

Tu dois placer le deuxième élément `fieldset` après le deuxième élément `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block:nth-of-type(2) > h3 + fieldset'));
```

Le premier élément `fieldset` doit avoir une classe de `question`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.[0]?.className, 'question');
```

Le deuxième élément `fieldset` doit avoir une classe de `question`.
