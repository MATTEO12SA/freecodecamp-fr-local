...
Relie le premier élément `label` à l'élément `select`, et donne à l'élément `select` un attribut `name`.

# --hints--

L'élément `label` doit avoir un attribut `for`.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

L'élément `select` doit avoir un attribut `id`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

L'élément `select` doit avoir un `id` correspondant à l'attribut `for` de l'élément `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

L'élément `select` doit avoir un attribut `name`.
