...
Pour que la première section ait une apparence plus alignée, cible uniquement les éléments `input` dans les éléments `.info`, et définis leur `width` sur `50%`, puis aligne leur texte à gauche.

# --hints--

Tu dois utiliser soit le sélecteur `.info input`, soit le sélecteur `.info > input`.
...
Les éléments `input` doivent avoir une propriété `width` de `50%`.
...
Les éléments `input` doivent avoir une propriété `text-align` de `left`.
