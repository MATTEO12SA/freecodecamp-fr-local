...
Pour aligner les champs de saisie entre eux, crée une nouvelle règle qui cible tous les éléments `input` et `label` dans un élément `.info` et définis la propriété `display` sur `inline-block`.

Aligne aussi le texte de l'élément `label` à droite.

# --hints--

Tu dois utiliser soit le sélecteur `.info > label, .info > input`, soit le sélecteur `.info label, .info input`.
...
Les éléments `input` et `label` doivent avoir une propriété `display` de `inline-block`.
...
L'élément `label` doit avoir une propriété `text-align` de `right`.
