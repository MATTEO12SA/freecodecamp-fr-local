...
Dans le premier élément `span`, ajoute le texte `Question`.

Dans le deuxième élément `span`, ajoute le texte `Question`.

# --hints--

Le premier élément `span` doit avoir le contenu textuel `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent.trim(), 'Question');
```

Le deuxième élément `span` doit avoir le contenu textuel `Question`.
