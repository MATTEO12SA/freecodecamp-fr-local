...
Ajoute du padding en haut et à gauche des éléments `.info`, et définis les autres valeurs sur `0`.

# --hints--

Tu dois utiliser le sélecteur `.info`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

`.info` doit avoir un `padding-top` d'au moins `1px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

`.info` doit avoir une propriété `padding-right` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

`.info` doit avoir une propriété `padding-bottom` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

`.info` doit avoir un `padding-left` d'au moins `1px`.
