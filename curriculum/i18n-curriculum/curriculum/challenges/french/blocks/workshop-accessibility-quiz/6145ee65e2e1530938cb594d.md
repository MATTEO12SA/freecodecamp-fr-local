...
La dernière section de ce quiz contiendra une liste déroulante et une zone de texte.

Commence par imbriquer un `div` avec une `class` de `formrow`, puis imbrique quatre éléments `div` à l'intérieur, en alternant leurs attributs `class` avec `question-block` et `answer`.

# --hints--

Tu dois ajouter un élément `div` dans le dernier `section`.

```js
assert.exists(document.querySelector('section:nth-of-type(3) > div'));
```

Le premier élément `div` doit avoir une propriété `class` de `formrow`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div')?.className, 'formrow');
```

Tu dois placer `div.formrow` après l'élément `h2`.

```js
assert.exists(document.querySelector('section:nth-of-type(3) > h2 + div.formrow'));
```

Tu dois imbriquer quatre éléments `div` dans l'élément `div.formrow`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(3) > div.formrow > div')?.length, 4);
```

Le premier `div` imbriqué doit avoir une propriété `class` de `question-block`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(1)')?.className, 'question-block');
```

Le deuxième `div` imbriqué doit avoir une propriété `class` de `answer`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(2)')?.className, 'answer');
```

Le troisième `div` imbriqué doit avoir une propriété `class` de `question-block`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(3)')?.className, 'question-block');
```

Le quatrième `div` imbriqué doit avoir une propriété `class` de `answer`.
