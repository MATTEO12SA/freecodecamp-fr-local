...
L'élément `address` n'a pas besoin de contenir un emplacement géographique physique. Il peut être utilisé pour fournir un lien vers le sujet.

Enveloppe un lien autour du texte `freeCodeCamp`, et définis son emplacement sur `https://freecodecamp.org`.

# --hints--

Tu dois ajouter un élément `a`.

```js
assert.exists(document.querySelector('address > a'));
```

L'élément `a` doit avoir un attribut `href` avec la valeur `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

Tu dois envelopper l'élément `a` autour du texte existant `freeCodeCamp`.
