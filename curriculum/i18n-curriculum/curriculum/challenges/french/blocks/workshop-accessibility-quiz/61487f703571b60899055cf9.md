...
Définis la largeur des éléments `section` à `80%` de leur conteneur parent. Ensuite, utilise les marges pour centrer les éléments `section`, en ajoutant `10px` à la marge inférieure.

Assure-toi aussi que les éléments `section` ne peuvent pas dépasser `600px` de largeur.

# --hints--

Tu dois utiliser un sélecteur `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('section'));
```

La balise `section` doit avoir une propriété `width` de `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.width, '80%');
```

La balise `section` doit avoir une propriété `margin-top` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginTop, '0px');
```

La balise `section` doit avoir une propriété `margin-right` de `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginRight, 'auto');
```

La balise `section` doit avoir une propriété `margin-bottom` de `10px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginBottom, '10px');
```

La balise `section` doit avoir une propriété `margin-left` de `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginLeft, 'auto');
```

La balise `section` doit avoir une propriété `max-width` de `600px`.
