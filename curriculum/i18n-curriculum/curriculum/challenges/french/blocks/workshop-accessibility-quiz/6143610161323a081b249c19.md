...
Pour remplir le contenu du quiz, sous `#student-info`, ajoute trois éléments `div` avec une `class` de `info`.

Ensuite, dans chaque `div`, imbrique un élément `label` et un élément `input`.

# --hints--

Tu dois imbriquer trois éléments `div` sous l'élément `h2#student-info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Le premier élément `div` doit avoir une propriété `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Le deuxième élément `div` doit avoir une propriété `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Le troisième élément `div` doit avoir une propriété `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

Tu dois imbriquer un élément `label` dans le premier élément `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

Tu dois imbriquer un élément `input` dans le premier élément `div`, après le `label`.
...
Tu dois imbriquer un élément `label` dans le deuxième élément `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

Tu dois imbriquer un élément `input` dans le deuxième élément `div`, après le `label`.
...
Tu dois imbriquer un élément `label` dans le troisième élément `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

Tu dois imbriquer un élément `input` dans le troisième élément `div`, après le `label`.
