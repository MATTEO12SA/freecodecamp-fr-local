...
Définis la couleur d'arrière-plan du `footer` sur `#2a2a40`, et utilise _Flexbox_ pour centrer horizontalement le texte.

# --hints--

Tu dois utiliser le sélecteur d'élément `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

`footer` doit avoir une propriété `background-color` de `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

`footer` doit avoir une propriété `display` de `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

`footer` doit avoir une propriété `justify-content` de `center`.
