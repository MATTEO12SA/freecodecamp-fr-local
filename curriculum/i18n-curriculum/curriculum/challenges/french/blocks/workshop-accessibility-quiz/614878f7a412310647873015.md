...
Nettoie le `header` en utilisant _Flexbox_ pour mettre de l'espace entre les enfants et les centrer verticalement.

# --hints--

La balise `header` doit avoir une propriété `justify-content` de `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

La balise `header` doit avoir une propriété `align-items` de `center`.
