...
Pour fournir la fonctionnalité des questions vrai/faux, nous avons besoin d'un ensemble d'entrées qui n'autorisent pas la sélection simultanée des deux options.

Dans chaque élément de liste, imbrique un élément `label`, et dans chaque élément `label`, imbrique un élément `input` avec le `type` approprié.

# --hints--

Tu dois imbriquer un élément `label` dans le premier élément `li`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[0]?.querySelector('label'));
```

Tu dois imbriquer un élément `label` dans le deuxième élément `li`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[1]?.querySelector('label'));
```

Tu dois imbriquer un élément `label` dans le troisième élément `li`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[2]?.querySelector('label'));
```

Tu dois imbriquer un élément `label` dans le quatrième élément `li`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[3]?.querySelector('label'));
```

Tu dois imbriquer un élément `input` dans le premier élément `label`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[0]?.querySelector('label')?.querySelector('input'));
```

Tu dois imbriquer un élément `input` dans le deuxième élément `label`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[1]?.querySelector('label')?.querySelector('input'));
```

Tu dois imbriquer un élément `input` dans le troisième élément `label`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[2]?.querySelector('label')?.querySelector('input'));
```

Tu dois imbriquer un élément `input` dans le quatrième élément `label`.

```js
assert.exists(document.querySelectorAll('ul.answers-list > li')?.[3]?.querySelector('label')?.querySelector('input'));
```

Le premier élément `input` doit avoir une propriété `type` de `radio`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[0]?.type, 'radio');
```

Le deuxième élément `input` doit avoir une propriété `type` de `radio`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[1]?.type, 'radio');
```

Le troisième élément `input` doit avoir une propriété `type` de `radio`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[2]?.type, 'radio');
```

Le quatrième élément `input` doit avoir une propriété `type` de `radio`.
