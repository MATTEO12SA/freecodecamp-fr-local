...
Maintenant, nous ne pouvons pas lire le texte. Cible le `footer` et l'élément d'ancre à l'intérieur pour définir la couleur de police sur une couleur ayant un ratio de contraste adéquat.

# --hints--

Tu dois utiliser le sélecteur `footer, footer a`.
...
Tu dois définir le `color` sur une valeur avec un ratio de contraste d'au moins `7:1`. _Indice : je suggère `#dfdfe2`_.
