...
Ajoute un autre dégradé répétitif à cet immeuble ; fais-le identique à celui que tu viens d'ajouter, sauf que tu n'ajoutes pas la direction `90deg` et que tu utilises ta couleur de fenêtre à la place des deux couleurs `transparent`.

# --hints--

Tu devrais donner à `.fb6` un deuxième `repeating-linear-gradient` dans la propriété `background`.
...
Tu devrais donner au deuxième `repeating-linear-gradient` une première couleur de `--building-color3` de `0%` à `10%`.
...
Tu devrais donner au deuxième `repeating-linear-gradient` une deuxième couleur de `--window-color3` de `10%` à `30%`.
