...
Donne à l'élément `.fb3a` un `width` de `80%` et un `height` de `15%`. Ensuite, donne à l'élément `.fb3b` un `width` de `100%` et un `height` de `35%`.

# --hints--

Tu devrais donner à `.fb3a` un `width` de `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

Tu devrais donner à `.fb3a` un `height` de `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

Tu devrais donner à `.fb3b` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

Tu devrais donner à `.fb3b` un `height` de `35%`.
