...
Deplace la propriété et la valeur `background-color` de `.fb1` vers `.fb1b`.

# --hints--

Tu devrais supprimer `background-color` de `.fb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Tu devrais ajouter un `background-color` de `--building-color4` à `.fb1b`.
