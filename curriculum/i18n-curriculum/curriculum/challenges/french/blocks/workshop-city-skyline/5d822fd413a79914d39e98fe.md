...
Crée et ajoute les propriétés suivantes à `.bb2a` :
...
Apres les avoir ajoutees, tu verras comment une bordure epaisse sur un élément crée des angles là où deux cotes se rejoignent. Tu vas utiliser cette bordure du bas comme sommet de l'immeuble.

# --hints--

Tu devrais donner à `.bb2a` un `margin` de `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.margin, "auto");
```

Tu devrais donner à `.bb2a` un `width` de `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.width, "5vw");
```

Tu devrais donner à `.bb2a` un `height` de `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.height, "5vw");
```

Tu devrais donner à `.bb2a` un `border-top` de `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderTop, "1vw solid rgb(0, 0, 0)");
```

Tu devrais donner à `.bb2a` un `border-bottom` de `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderBottom, "1vw solid rgb(0, 0, 0)");
```

Tu devrais donner à `.bb2a` un `border-left` de `1vw solid #999`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderLeft, "1vw solid rgb(153, 153, 153)");
```

Tu devrais donner à `.bb2a` un `border-right` de `1vw solid #999`.
