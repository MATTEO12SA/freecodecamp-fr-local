...
Enfin, dans le sélecteur `:root` de la media query, redéfinis les quatre variables `--window-color` pour qu'elles utilisent `#777`. Quand tu as terminé, redimensionne la fenêtre et regarde la scène passer du jour à la nuit.

Les variables sont principalement utilisées avec les couleurs, et c'est ainsi que tu les as utilisées ici. Mais on peut leur donner n'importe quelle valeur et les utiliser sur n'importe quelle propriété. Ton projet est superbe !

# --hints--

Tu devrais ajouter `--window-color1` avec une valeur de `#777`.
...
Tu devrais ajouter `--window-color2` avec une valeur de `#777`.
...
Tu devrais ajouter `--window-color3` avec une valeur de `#777`.
...
Tu devrais ajouter `--window-color4` avec une valeur de `#777`.
