...
Utilise la même variable comme `background-color` des classes `.bb1b`, `.bb1c` et `.bb1d` pour remplir le reste de l'immeuble.

# --hints--

Le `background-color` de l élément `.bb1b` devrait être défini.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1b')?.backgroundColor)
```

Tu devrais utiliser `var(--building-color1)` pour définir le `background-color` de l élément `.bb1b`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1b')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Le `background-color` de l élément `.bb1c` devrait être défini.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1c')?.backgroundColor)
```

Tu devrais utiliser `var(--building-color1)` pour définir le `background-color` de l élément `.bb1c`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1c')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Le `background-color` de l élément `.bb1d` devrait être défini.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1d')?.backgroundColor)
```

Tu devrais utiliser `var(--building-color1)` pour définir le `background-color` de l élément `.bb1d`.
