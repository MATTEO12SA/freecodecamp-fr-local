...
Donne à `.fb2a` un `width` de `100%`, puis à `.fb2b` un `width` de `100%` et un `height` de `75%`.

# --hints--

Tu devrais donner à `.fb2a` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

Tu devrais donner à `.fb2b` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

Tu devrais donner à `.fb2b` un `height` de `75%`.
