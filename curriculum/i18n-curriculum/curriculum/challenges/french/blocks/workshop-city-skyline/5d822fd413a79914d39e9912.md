...
Donne à `.fb1b` un `width` de `60%` et un `height` de `10%`, puis à `.fb1c` un `width` de `100%` et un `height` de `80%`.

# --hints--

Tu devrais donner à `.fb1b` un `width` de `60%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.width, "60%");
```

Tu devrais donner à `.fb1b` un `height` de `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.height, "10%");
```

Tu devrais donner à `.fb1c` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1c")?.width, "100%");
```

Tu devrais donner à `.fb1c` un `height` de `80%`.
