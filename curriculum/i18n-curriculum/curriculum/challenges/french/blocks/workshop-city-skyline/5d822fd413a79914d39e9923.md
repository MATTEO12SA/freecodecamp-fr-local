...
Supprime la propriété et la valeur `background-color` de `.fb3`, puis ajoute-les à `.fb3a` et `.fb3b`.

# --hints--

Tu devrais supprimer le `background-color` de `.fb3`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

Tu devrais donner à `.fb3a` un `background-color` de `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Tu devrais donner à `.fb3b` un `background-color` de `--building-color1`.
