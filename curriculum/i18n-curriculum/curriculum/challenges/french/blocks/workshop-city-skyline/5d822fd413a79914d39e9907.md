...
Donne aux nouveaux éléments `div` ces valeurs de `width` et `height` : `3%` et `10%` pour `.bb4a`, `80%` et `5%` pour `.bb4b`, puis `100%` et `85%` pour `.bb4c`.

# --hints--

Tu devrais donner à `.bb4a` un `width` de `3%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.width, "3%");
```

Tu devrais donner à `.bb4a` un `height` de `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.height, "10%");
```

Tu devrais donner à `.bb4b` un `width` de `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.width, "80%");
```

Tu devrais donner à `.bb4b` un `height` de `5%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.height, "5%");
```

Tu devrais donner à `.bb4c` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4c")?.width, "100%");
```

Tu devrais donner à `.bb4c` un `height` de `85%`.
