...
Les transitions de dégradé passent souvent progressivement d'une couleur à une autre. Quand un changement plus net est nécessaire, la transition peut être faite avec un arrêt net comme ceci :
...
Ajoute à `.bb2b` un `linear-gradient` qui utilise `--building-color2` de `0%` à `6%` et `--window-color2` de `6%` à `9%`.

# --hints--

Tu devrais donner à `.bb2b` une propriété `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

Tu devrais utiliser un `linear-gradient` sur le `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

Tu devrais utiliser `--building-color2` de `0%` à `6%`.
...
Tu devrais utiliser `--window-color2` de `6%` à `9%`.
...
`.bb2b` devrait avoir un `linear-gradient` qui passe de `--building-color2` entre `0%` et `6%`, puis à `--window-color2` entre `6%` et `9%`.
