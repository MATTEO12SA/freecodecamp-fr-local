...
Ajoute un autre `repeating-linear-gradient` sous celui que tu viens d'ajouter. Donne-lui une direction de `90deg`, utilise ta couleur d'immeuble de `0%` à `12%` et la couleur de fenêtre de `12%` à `44%`. Cela créera beaucoup de fenêtres rectangulaires.

# --hints--

Tu devrais donner à `.fb5` un deuxième `repeating-linear-gradient` dans la propriété `background`.
...
Tu devrais donner au deuxième `repeating-linear-gradient` une direction de `90deg`.
...
Tu devrais donner au deuxième `repeating-linear-gradient` une première couleur de `--building-color2` de `0%` à `12%`.
...
Tu devrais donner au deuxième `repeating-linear-gradient` une deuxième couleur de `--window-color2` de `12%` à `44%`.
