...
Le skyline prend forme. Remplis la propriété `background-color` des immeubles de premier plan. Utilise ta variable `--building-color1` pour remplir `.fb3` et `.fb4`, `--building-color2` pour `.fb5`, `--building-color3` pour `.fb2` et `.fb6`, et `--building-color4` pour `.fb1`.

# --hints--

Tu devrais donner à `.fb1` un `background-color` en utilisant `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb1')?.getPropVal('background-color', true), 'var(--building-color4)');
```

Tu devrais donner à `.fb2` un `background-color` en utilisant `--building-color3`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb2')?.getPropVal('background-color', true), 'var(--building-color3)');
```

Tu devrais donner à `.fb3` un `background-color` en utilisant `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb3')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Tu devrais donner à `.fb4` un `background-color` en utilisant `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Tu devrais donner à `.fb5` un `background-color` en utilisant `--building-color2`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.getPropVal('background-color', true), 'var(--building-color2)');
```

Tu devrais donner à `.fb6` un `background-color` en utilisant `--building-color3`.
