...
Passons à l'immeuble suivant ! Imbrique deux éléments `div` dans `.fb2` et donne-leur les classes `fb2a` et `fb2b`, dans cet ordre.

# --hints--

Tu devrais ajouter deux éléments `div` dans `.fb2`.

```js
assert.lengthOf(document.querySelectorAll("div.fb2 > div"), 2);
```

Tu devrais donner au premier nouveau `div` une classe `fb2a`.
...
Tu devrais donner au deuxième nouveau `div` une classe `fb2b`.
