...
Imbrique six éléments `div` dans `.foreground-buildings` et donne-leur les classes `fb1` à `fb6`, dans cet ordre. "fb" signifie "foreground building". Ce seront six immeubles supplémentaires pour le premier plan.

# --hints--

Tu devrais créer un nouveau `div` avec une classe `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Tu devrais créer un nouveau `div` avec une classe `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Tu devrais créer un nouveau `div` avec une classe `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Tu devrais créer un nouveau `div` avec une classe `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Tu devrais créer un nouveau `div` avec une classe `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Tu devrais créer un nouveau `div` avec une classe `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Tu devrais placer ces nouveaux éléments `div` dans l élément `.foreground-buildings`.
