...
Enfin ! Tu es arrivé au dernier immeuble ! Ajoute-lui un dégradé répétitif avec une direction de `90deg`. Utilise la couleur d'immeuble de `0%` à `10%` et `transparent` de `10%` à `30%`.

# --hints--

Tu devrais ajouter un `repeating-linear-gradient` à `.fb6` dans la propriété `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Tu devrais donner au `repeating-linear-gradient` une direction de `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Tu devrais donner au `repeating-linear-gradient` une première couleur de `--building-color3` from `0%` to `10%`.
...
Tu devrais donner au `repeating-linear-gradient` une deuxième couleur de `transparent` de `10%` à `30%`.
