...
Deplace les propriétés et valeurs `display`, `flex-direction` et `align-items` de `.bb1` vers la nouvelle classe `building-wrap`.

# --hints--

Tu devrais supprimer `display` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.display);
```

Tu devrais déplacer `display` avec une valeur de `flex` vers `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.display, "flex");
```

Tu devrais supprimer `flex-direction` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.flexDirection);
```

Tu devrais déplacer `flex-direction` avec une valeur de `column` vers `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.flexDirection, "column");
```

Tu devrais supprimer `align-items` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.alignItems);
```

Tu devrais déplacer `align-items` avec une valeur de `center` vers `.building-wrap`.
