...
Tu veux ajouter le même dégradé aux deux sections suivantes. Au lieu de le refaire, crée un nouveau sélecteur de classe appele `bb1-window`, puis déplace les propriétés et valeurs `height` et `background` de `.bb1a` vers ce nouveau sélecteur de classe.

# --hints--

Tu devrais créer un nouveau sélecteur de classe appele `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Tu devrais déplacer la propriété et la valeur `height` de `.bb1a` vers `.bb1-window`.
...
Tu devrais déplacer la propriété et la valeur `background` de `.bb1a` vers `.bb1-window`.
...
Tu ne devrais pas déplacer la propriété `background-color` de `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Tu ne devrais pas déplacer la propriété `width` de `.bb1a`.
