...
Donne à la classe `sky` un `radial-gradient`. Utilise `#ffcf33` de `0%` à `20%`, `#ffff66` à `21%`, et `#bbeeff` à `100%`. Cela ajoutera à l'arrière-plan un dégradé circulaire qui deviendra ton soleil.

# --hints--

Tu devrais donner à `.sky` une propriété `radial-gradient` in the `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

Tu devrais donner au `radial-gradient` une première couleur de `#ffcf33`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

Tu devrais donner au `radial-gradient` une deuxième couleur de `#ffff66` à `21%`.
...
Tu devrais donner au `radial-gradient` une troisième couleur de `#bbeeff` à `100%`.
