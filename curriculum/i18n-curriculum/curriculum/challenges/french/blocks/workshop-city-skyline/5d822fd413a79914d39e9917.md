...
Tu peux ajouter plusieurs dégradés à un élément en les séparant par une virgule (`,`), comme ceci :
...
Ajoute à `.fb1c`, sous celui qui est déjà là, un `repeating-linear-gradient` ; utilise ta variable `--building-color4` de `0%` à `10%` et `--window-color4` de `10%` à `90%`. Cela remplira l'arrière du dégradé que tu as ajouté en dernier.

# --hints--

Tu ne devrais pas modifier le premier `repeating-linear-gradient`.
...
Tu devrais avoir une virgule (`,`) après le premier `repeating-linear-gradient`.
...
Tu devrais ajouter un `repeating-linear-gradient` avec une première couleur de `--building-color4` de `0%` à `10%`.
...
Tu devrais utiliser une deuxième couleur de `--window-color4` de `10%` à `90%`.
