...
Passons à l'immeuble suivant ! C'est le vert au premier plan. Donne-lui un `repeating-linear-gradient` avec ta couleur d'immeuble de `0%` à `5%`, puis `transparent` de `5%` à `10%`.

# --hints--

Tu devrais donner à `.fb5` une propriété `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Tu devrais donner au `background` un `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Tu devrais donner au `repeating-linear-gradient` une première couleur de `--building-color2` from `0%` to `5%`.
...
Tu devrais donner au `repeating-linear-gradient` une deuxième couleur de `transparent` de `5%` à `10%`.
