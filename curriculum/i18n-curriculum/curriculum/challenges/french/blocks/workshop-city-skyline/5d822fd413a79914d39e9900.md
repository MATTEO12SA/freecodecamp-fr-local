...
Ensuite, change les deux `#999` de `.bb2a` en `transparent`. Les bordures gauche et droite deviendront invisibles.

# --hints--

Tu devrais changer le `border-left` pour utiliser `transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Tu devrais changer le `border-right` pour utiliser `transparent`.
