...
Sous l'élément de citation en bloc, ajoute un élément paragraphe avec le texte `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.

`&mdash;` est une entité HTML qui représente un tiret cadratin `—`.

# --hints--

Tu devrais avoir un élément `p` sous l'élément `blockquote`.

```js
assert.exists(document.querySelector('section:first-of-type > blockquote + p'));
```

Ton élément `p` devrait avoir le texte `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.
