...
Testons ta nouvelle fonction curryfiée. Déclare une variable `servingsResult`, et assigne-lui le résultat de l'ajustement d'une quantité de base de 4 à une portion de 6.

Affiche cette variable dans la console.

# --hints--

Tu dois déclarer une variable `servingsResult`.

```js
assert.isDefined(servingsResult);
```

Ta variable `servingsResult` doit contenir le résultat de l'appel `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

Tu dois afficher la valeur de `servingsResult`.
