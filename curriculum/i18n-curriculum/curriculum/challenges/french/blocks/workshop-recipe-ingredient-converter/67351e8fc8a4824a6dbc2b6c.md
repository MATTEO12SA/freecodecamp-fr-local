...
Donne à ton `conversionTable` une propriété `cup`, et assigne-lui un objet avec une propriété `gram` définie à `240`, une propriété `ounce` définie à `8.0`, et une propriété `teaspoon` définie à `48`.

# --hints--

Ton `conversionTable` doit avoir une propriété `cup`.

```js
assert.property(conversionTable, "cup");
```

Ta propriété `cup` doit être un objet.

```js
assert.isObject(conversionTable.cup);
```

Ton objet `cup` doit avoir une propriété `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

Ta propriété `gram` doit être définie à `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

Ton objet `cup` doit avoir une propriété `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

Ta propriété `ounce` doit être définie à `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

Ton objet `cup` doit avoir une propriété `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

Ta propriété `teaspoon` doit être définie à `48`.
