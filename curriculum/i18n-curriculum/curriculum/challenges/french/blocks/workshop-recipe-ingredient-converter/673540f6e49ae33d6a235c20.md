...
Tu vas devoir conserver une liste des unités supportées par ton appli.

Déclare un tableau `units`, et donne-lui les valeurs de chaîne `"cup"`, `"gram"`, `"ounce"` et `"teaspoon"`.

# --hints--

Tu dois déclarer une variable `units`.

```js
assert.isDefined(units);
```

Ta variable `units` doit être un tableau.

```js
assert.isArray(units);
```

Ton tableau `units` doit avoir quatre éléments.

```js
assert.lengthOf(units, 4);
```

Ton tableau `units` doit inclure la chaîne `"cup"`.

```js
assert.include(units, "cup");
```

Ton tableau `units` doit inclure la chaîne `"gram"`.

```js
assert.include(units, "gram");
```

Ton tableau `units` doit inclure la chaîne `"ounce"`.

```js
assert.include(units, "ounce");
```

Ton tableau `units` doit inclure la chaîne `"teaspoon"`.
