...
Dans cette étape, attache un écouteur d'événement `submit` à `recipeForm` pour que, lorsque le bouton `Convert` est cliqué, la fonction `updateResultsList()` soit appelée.

Attache un écouteur d'événement `submit` à `recipeForm`. Lorsque le formulaire est soumis, appelle `event.preventDefault()` pour arrêter le comportement par défaut, puis appelle `updateResultsList()`.

Cela te permettra de déclencher la fonction via l'interface en remplissant le formulaire et en appuyant sur `Convert`, utile pour le débogage en ajoutant la logique de conversion à l'étape suivante.

# --hints--

Fais en sorte que cliquer sur le bouton `Convert` exécute `updateResultsList()` en ajoutant un écouteur d'événement `"submit"` à `recipeForm` qui appelle `event.preventDefault()` pour arrêter le comportement par défaut du formulaire.
