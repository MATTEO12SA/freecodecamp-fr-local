...
Tu vas devoir ajuster selon le nombre de portions calculées. C'est une excellente occasion d'utiliser une autre fonction curryfiée.

Déclare une fonction `adjustForServings`, qui accepte un argument `baseQuantity`. Elle doit renvoyer une seconde fonction qui accepte un argument `newServings`.

La fonction interne doit renvoyer le résultat de la multiplication de `baseQuantity` et `newServings`.

# --hints--

Tu dois déclarer une fonction `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

Ta fonction `adjustForServings` doit accepter un argument `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

Ta fonction `adjustForServings` doit renvoyer une fonction.

```js
assert.isFunction(adjustForServings?.());
```

Ta fonction interne doit avoir un paramètre `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

Ta fonction interne doit renvoyer le résultat de la multiplication de `baseQuantity` et `newServings`.
