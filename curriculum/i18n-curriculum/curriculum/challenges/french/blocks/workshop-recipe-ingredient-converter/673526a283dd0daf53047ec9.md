...
Maintenant tu vas créer ta première fonction curryfiée. Souviens-toi qu'une fonction curryfiée est une fonction qui renvoie une autre fonction, comme ceci :
...
Commence par déclarer une fonction `convertQuantity`. Elle doit avoir un seul paramètre `fromUnit`, et doit être vide.

# --hints--

Tu dois déclarer une fonction `convertQuantity`.

```js
assert.isFunction(convertQuantity)
```

Ta fonction `convertQuantity` doit avoir un paramètre `fromUnit`.

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

Ta fonction `convertQuantity` doit être vide.
