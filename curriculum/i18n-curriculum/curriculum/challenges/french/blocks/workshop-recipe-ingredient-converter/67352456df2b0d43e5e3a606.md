...
Donne à ton `conversionTable` une propriété `gram`, et assigne-lui un objet avec une propriété `cup` définie à `1/240`, une propriété `ounce` définie à `0.0353`, et une propriété `teaspoon` définie à `0.2`.

# --hints--

Ton `conversionTable` doit avoir une propriété `gram`.

```js
assert.property(conversionTable, "gram");
```

Ta propriété `gram` doit être un objet.

```js
assert.isObject(conversionTable.gram);
```

Ton objet `gram` doit avoir une propriété `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

Ta propriété `cup` doit être définie à `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Ton objet `gram` doit avoir une propriété `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

Ta propriété `ounce` doit être définie à `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Ton objet `gram` doit avoir une propriété `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

Ta propriété `teaspoon` doit être définie à `0.2`.
