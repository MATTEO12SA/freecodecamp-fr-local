...
Dans cette étape, tu vas terminer la fonction `updateResultsList`.

Maintenant parcours le tableau `units`. Pour chaque valeur, vérifie si elle n'est pas égale à l'unité actuelle (`unitToConvert.value`).

Si la condition est vraie, convertis l'unité existante vers l'unité actuelle et ajoute un nouvel élément de liste à `resultList` au format :

`ingredient: quantity unit`

Par exemple, si l'entrée est `flour`, `5` et `gram`, le résultat devrait être :
...
Lorsque tu cliques sur le bouton, ta fonction `updateResultsList` doit correctement mettre à jour le DOM.
