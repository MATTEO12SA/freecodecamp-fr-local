...
Tu vas avoir besoin d'une fonction qui met réellement à jour le DOM. Déclare une fonction `updateResultsList`.

Pour commencer, la fonction doit vider le HTML de l'élément `resultList`.

# --hints--

Tu dois définir une fonction `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

Ta fonction `updateResultsList` doit définir `resultList.innerHTML` à une chaîne vide.
