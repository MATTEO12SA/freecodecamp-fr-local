...
Enfin, mets à jour ta fonction interne pour qu'elle renvoie une troisième fonction. Cette troisième fonction doit aussi accepter un seul argument nommé `quantity`, et avoir un corps vide.

# --hints--

Ta seconde fonction doit renvoyer encore une autre fonction.

```js
assert.isFunction(convertQuantity()());
```

Ta fonction la plus interne doit avoir un paramètre `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

Ta fonction la plus interne doit être vide.
