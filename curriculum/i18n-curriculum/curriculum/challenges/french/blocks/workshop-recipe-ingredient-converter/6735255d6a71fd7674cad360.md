...
Enfin, donne à ton `conversionTable` un objet `teaspoon` avec une propriété `cup` définie à `1/48`, une propriété `gram` définie à `5`, et une propriété `ounce` définie à `0.167`.

# --hints--

Ton `conversionTable` doit avoir une propriété `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

Ta propriété `teaspoon` doit être un objet.

```js
assert.isObject(conversionTable.teaspoon);
```

Ton objet `teaspoon` doit avoir une propriété `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

Ta propriété `cup` doit être définie à `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Ton objet `teaspoon` doit avoir une propriété `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Ta propriété `gram` doit être définie à `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Ton objet `teaspoon` doit avoir une propriété `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

Ta propriété `ounce` doit être définie à `0.167`.
