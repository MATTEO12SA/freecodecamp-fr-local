...
Il est temps d'utiliser tes fonctions. Déclare une nouvelle fonction `processIngredient` qui accepte quatre arguments : `baseQuantity`, `baseUnit`, `newUnit` et `newServings`.

Ta fonction doit ajuster les portions pour la quantité de base selon les nouvelles portions, convertir cette quantité ajustée de l'unité de base vers la nouvelle unité, et renvoyer la quantité convertie sous forme de chaîne fixée à deux décimales.

Si tu es bloqué, reporte-toi aux appels de fonction d'exemple que tu as affichés. Tu peux changer les valeurs dans ces deux exemples pour explorer.

# --hints--

Tu dois déclarer une fonction `processIngredient`.

```js
assert.isFunction(processIngredient);
```

Ta fonction `processIngredient` doit avoir `baseQuantity` comme premier paramètre.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

Ta fonction `processIngredient` doit avoir `baseUnit` comme deuxième paramètre.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

Ta fonction `processIngredient` doit avoir `newUnit` comme troisième paramètre.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

Ta fonction `processIngredient` doit avoir `newServings` comme quatrième paramètre.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

Ta fonction `processIngredient` doit renvoyer une chaîne.

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

Ta fonction `processIngredient` doit renvoyer la bonne valeur.
