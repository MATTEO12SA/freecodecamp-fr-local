...
À l'intérieur de ton `div`, ajoute deux éléments `p`. Définis le texte du second élément `p` à la valeur de la variable `price`.

# --hints--

Tu dois ajouter deux éléments `p` à l'intérieur de ton élément `div`.
...
Ton second élément `p` doit avoir le texte de la variable `price`.
