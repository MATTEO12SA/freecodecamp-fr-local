...
Après ton élément `h2`, crée deux éléments `p`. Donne au premier une `class` de `dessert-price`, et définis le texte à un signe dollar `"$"` suivi de la valeur de la variable `price`. Donne au second une `class` de `product-category`, et le texte `"Category: "` suivi de la valeur de la variable `category`.

# --hints--

Tu dois créer deux éléments `p`.
...
Tes éléments `p` doivent venir après ton élément `h2`.
...
Ton premier élément `p` doit avoir une `class` de `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Ton premier élément `p` doit avoir le signe dollar `"$"` suivi de la valeur de la variable `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Ton second élément `p` doit avoir une `class` de `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Ton second élément `p` doit avoir le texte `"Category: "` suivi de la valeur de la variable `category`.
