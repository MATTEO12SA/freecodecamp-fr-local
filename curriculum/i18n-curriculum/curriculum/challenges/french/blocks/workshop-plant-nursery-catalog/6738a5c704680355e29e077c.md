...
Comme tu l'as déjà appris, la méthode `delete` sert à retirer une entrée d'une map. Les Sets ont aussi une méthode `delete` que tu peux utiliser pour en retirer un élément

Parfois, tu peux avoir besoin de retirer tous les éléments d'une map ou d'un set. La méthode `clear` fait exactement ce travail.

Appelle la méthode `clear` sur `plantsSet` pour en retirer tous les éléments.

# --hints--

Tu dois appeler la méthode `clear` sur `plantsSet`.
