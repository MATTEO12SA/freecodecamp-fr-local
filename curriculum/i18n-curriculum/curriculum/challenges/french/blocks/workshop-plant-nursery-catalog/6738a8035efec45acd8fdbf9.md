...
Comme tu peux le voir, ton set est maintenant complètement vide. Retire les appels à `clear` et `has` de ton code.

# --hints--

Tu ne dois pas avoir `plantsSet.clear()` dans ton code.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

Tu ne dois pas avoir `plantsSet.has(ballerina)` dans ton code.
