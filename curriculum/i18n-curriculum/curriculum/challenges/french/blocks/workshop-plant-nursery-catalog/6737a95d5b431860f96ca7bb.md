...
Essaie maintenant d'ajouter l'objet `ballerina` à ton `catalogSet` une deuxième fois à l'intérieur de ta fonction. Tu verras que rien ne se passe, car l'objet est déjà stocké dans ton set et les éléments en double ne sont pas autorisés dans les sets.

Si tu veux, tu peux vérifier le nombre d'éléments dans ton set avec la propriété `size`, comme tu l'as fait avant pour ta map.

# --hints--

Tu dois `add` l'objet `ballerina` une deuxième fois à ton set dans la fonction `displayPlantsSet`.
