...
Les Maps et les Sets partagent quelques méthodes qui ont le même nom et le même rôle. Par exemple, tu peux utiliser la méthode `has` sur un set pour vérifier s'il contient un élément spécifique.

Utilise la méthode `has` pour vérifier si l'objet `ballerina` est contenu dans ton `plantsSet` et affiche le résultat dans la console.

# --hints--

Tu dois afficher `plantsSet.has(ballerina)` dans la console.
