...
Le constructeur `Set` accepte aussi un itérable comme argument et crée un set à partir de celui-ci :
...
Pour terminer ce workshop, modifie ta fonction `displayPlantsSet` pour qu'elle renvoie un set des noms communs uniques des plantes de ton catalogue.

Pour y parvenir, crée un tableau contenant le `commonName` de chaque objet plante stocké comme clé de la map `catalog`. Passe le tableau au constructeur `Set` et renvoie le set depuis la fonction.

# --hints--

Ta fonction `displayPlantsSet` doit renvoyer un set contenant le `commonName` de chaque plante de ton catalogue.
