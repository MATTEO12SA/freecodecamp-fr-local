...
Avant de continuer, retire les appels `console.log` de ton code.

# --hints--

Tu ne dois pas avoir `console.log(removePlant(ballerina))` dans ton code.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

Tu ne dois pas avoir `console.log(sellPlants(ballerina, "small", 10));` dans ton code.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Tu ne dois pas avoir `console.log(catalog)` dans ton code.
