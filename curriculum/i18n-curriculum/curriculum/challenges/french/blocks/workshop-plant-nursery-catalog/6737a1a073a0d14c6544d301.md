...
Déclare une variable nommée `plantsSet` et assigne-lui un appel à la fonction `displayPlantsSet`.

Ensuite, affiche `plantsSet` dans la console. Tu devras ouvrir la console du navigateur pour voir ton set.


# --hints--

Tu dois avoir une variable nommée `plantsSet`.

```js
assert.exists(plantsSet);
```

Tu dois appeler `displayPlantsSet` et l'assigner à `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Tu dois afficher `plantsSet` dans la console.
