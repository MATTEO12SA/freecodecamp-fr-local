...
title: Construire une app de quiz flashcards
challengeType: 25
...
Dans ce lab, tu vas créer une app qui affiche et stocke des données de flashcards pouvant
être récupérées plus tard.

**Objectif :** Remplis les user stories ci-dessous et fais passer tous les tests pour terminer le lab.

**User Stories :**

1) Tu dois avoir un élément HTML avec un id `flashcard`.
2) Tu dois avoir une interface appelée `FlashCard`.
3) L’interface `FlashCard` doit contenir les propriétés suivantes `questionText` et `questionAnswer`, toutes deux de type `string`.
4) Tu dois avoir une collection d’éléments `FlashCard` appelée `currentCards`.
5) Quand l’élément `#flashcard` est cliqué, la carte doit avoir la classe `flipped`.
6) Tu dois avoir un élément avec un id `delete-btn`.
7) Quand l’élément `#delete-btn` est cliqué, il doit supprimer la carte courante et afficher les données de la carte précédente.
8) Tu dois créer un formulaire de saisie avec un id `entry-form` pour pouvoir ajouter plus de flashcards à la collection `currentCards` à la soumission.
9) Les deux éléments à l’intérieur du formulaire doivent avoir respectivement les ids `front-text` et `back-text`.
10) Tu dois créer et lever une `InvalidUserInputError` quand le texte de la question ou la réponse de la question est vide dans le formulaire de saisie.

# --hints--

Tu dois avoir un élément HTML avec un id `flashcard`.
...
Tu dois avoir une interface `FlashCard`.
...
L’interface `FlashCard` doit avoir une propriété `questionText` de type `string`.
...
L’interface `FlashCard` doit avoir une propriété `questionAnswer` de type `string`.
...
Tu dois avoir une collection d’éléments `FlashCard` appelée `currentCards` avec le type `FlashCard[]`.
...
Quand l’élément `#flashcard` est cliqué pour la première fois, l’élément doit recevoir la classe `flipped`.
...
Tu dois avoir un élément avec un id `delete-btn`.
...
Quand le `delete-btn` est cliqué, un élément flashcard doit être retiré de la collection `currentCards`.
...
Quand le `delete-btn` est cliqué, les données de la flashcard précédente doivent être affichées.
...
Tu dois créer un formulaire de saisie avec un id `entry-form`.
...
Tu dois avoir deux éléments `textarea` avec les ids `front-text` et `back-text` respectivement à l’intérieur du formulaire.
...
Une `InvalidUserInputError` doit être levée quand le texte de la question ou la réponse de la question est vide dans le formulaire de saisie.
