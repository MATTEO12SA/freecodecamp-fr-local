...
L'attribut `alt` d'une image devrait décrire le contenu de l'image. Les lecteurs d'écran annoncent le texte alternatif à la place des images. Si l'image ne peut pas être chargée, ce texte est affiché à la place de l'image.

Pour finir le projet, ajoute un attribut `alt` à tes neuf images de chats pour les décrire. Utilise une valeur d'au moins cinq caractères pour chacune.


# --hints--

Tes neuf éléments `img` devraient tous avoir un attribut `alt` avec un texte décrivant chaque image, chaque description utilisant au moins `5` caractères.
