...
Retire la marge de ton élément body, définis le `font-family` à `sans-serif`, et donne-lui un `background-color` de `#f5f6f7` comme valeur.
...
Ton sélecteur `body` devrait avoir une propriété `margin` définie à 0 comme valeur.
...
Ton sélecteur `body` devrait avoir une propriété `font-family` définie à `sans-serif` comme valeur.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'sans-serif');
```

Ton sélecteur `body` devrait avoir une propriété `background-color` définie à `#f5f6f7` comme valeur.
