...
Donne à ton sélecteur `.gallery` une propriété `padding` définie à `20px 10px` pour créer un peu d'espace autour du conteneur.

Ensuite, donne-lui un `max-width` de `1400px` et ajoute un `margin` de `0 auto` pour le centrer.

# --hints--

Ton sélecteur `.gallery` devrait avoir une propriété `padding` définie à `20px 10px` comme valeur.
...
Ton sélecteur `.gallery` devrait avoir une propriété `max-width` définie à `1400px` comme valeur.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth, '1400px');
```

Ton élément `.gallery` devrait être centré à l'aide d'un `margin` avec `0 auto` comme valeur.
