...
Les formulaires utilisent souvent des **cases à cocher** pour des questions qui peuvent avoir plusieurs réponses. Un `input` avec `type="checkbox"` crée une case à cocher.

Sous le `legend` que tu viens d'ajouter, ajoute un `input` avec `type="checkbox"` suivi du texte :
...
Tu ne dois ajouter qu'**un seul** `input` (supprime les éventuelles balises en trop).

```js
assert.lengthOf(document.querySelectorAll('fieldset > input'), 1);
```

Le nouvel `input` doit être **sous** le `legend` `What's your cat's personality?`. L'ordre n'est pas bon.
...
L'attribut `type` doit valoir `checkbox`.
...
Le texte ` Loving` doit être **juste à droite** de la case à cocher (avec un espace entre les deux).
