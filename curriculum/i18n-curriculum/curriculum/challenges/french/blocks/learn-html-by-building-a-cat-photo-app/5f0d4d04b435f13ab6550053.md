...
Ajoute un `legend` avec le texte `What's your cat's personality?` à l'intérieur du **second** `fieldset`.

# --hints--

Le second `fieldset` doit toujours être présent (avec balise ouvrante et fermante).
...
Ton `legend` doit avoir une balise fermante.

```js
assert(code.match(/<\/legend\>/g).length === 2);
```

Le texte du `legend` doit être `What's your cat's personality?`.
