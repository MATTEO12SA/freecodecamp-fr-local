...
Il existe une **autre façon** d'associer un texte à un `input` : tu peux mettre le texte dans un élément `label` séparé, puis ajouter un attribut `for` au `label` avec la **même valeur** que l'`id` de l'`input`.

Exemple — pour un `input` comme celui-ci :

```html
<input id="breakfast" type="radio" name="meal" value="breakfast">
```

Voici un `label` associé :

```html
<label for="breakfast">Breakfast</label>
```

Associe le texte `Loving` à ta case à cocher en mettant **uniquement** le texte `Loving` dans un `label`, avec l'attribut `for` qui convient.

# --hints--

La case à cocher doit toujours être présente, inchangée par rapport au code de départ.
...
La case à cocher doit toujours avoir `id="loving"`.
...
Tu dois ajouter un nouveau `label` après la case à cocher (avec balises ouvrante et fermante).
...
Ton `label` doit être placé **après** la case à cocher.
...
Le texte du `label` doit être `Loving`.
...
Le texte `Loving` ne doit apparaître **qu'à l'intérieur** du nouveau `label` (et pas en dehors).
...
Ton `label` doit avoir un attribut `for`.
...
L'attribut `for` doit valoir `loving`.
...
Il doit y avoir un espace entre la case à cocher et le nouveau `label`.
