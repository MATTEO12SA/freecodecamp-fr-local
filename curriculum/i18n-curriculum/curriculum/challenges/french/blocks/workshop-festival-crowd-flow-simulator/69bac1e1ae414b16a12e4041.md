...
Enfin, tu peux exécuter tes simulations.

Commence par le créneau du matin. Appelle `simulateFestival()` avec `morningGates` et `"Morning"`.

# --hints--

Tu dois appeler `simulateFestival()` avec `morningGates` et `"Morning"`.
