...
Avant de démarrer la simulation, tu as besoin d'un objet pour suivre combien de participants chaque porte traite sur l'ensemble des ticks. Appelle `initializeThroughput()` avec `gates` et affecte le résultat à une variable nommée `throughputSummary`.

# --hints--

Tu dois appeler `initializeThroughput()` avec `gates` et affecter le résultat à une variable nommée `throughputSummary`.
...
Ta fonction `simulateFestival` doit appeler la fonction `initializeThroughput()` avec `gates`.
