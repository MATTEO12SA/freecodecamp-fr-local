...
Maintenant, tu peux construire une fonction pour simuler le festival. Crée une fonction vide nommée `simulateFestival` avec les paramètres `gates` et `timeBlock`.

# --hints--

Tu dois créer une fonction nommée `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

Ta fonction `simulateFestival` doit avoir les paramètres `gates` et `timeBlock`.
