...
Il sera utile d'avoir une fonction pour afficher un résumé du débit pendant une simulation. Crée une fonction vide nommée `printSummary` avec un paramètre `summary`.

# --hints--

Tu dois créer une fonction nommée `printSummary`.

```js
assert.isFunction(printSummary);
```

Ta fonction `printSummary` doit avoir un paramètre `summary`.
