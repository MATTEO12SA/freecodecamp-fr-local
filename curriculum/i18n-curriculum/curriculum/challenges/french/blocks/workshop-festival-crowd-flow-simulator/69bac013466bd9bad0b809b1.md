...
D'abord, tu vas trouver la position d'une porte précise au sein du tableau `gates`.

Utilise la méthode `indexOf()` sur `gates` pour localiser l'élément `currentGate` et affecte le résultat à une variable nommée `currentIndex`.

Voici un rappel de comment cela peut se faire :
...
Tu dois utiliser `indexOf()` sur le tableau `gates` pour accéder à l'index de `currentGate` et l'affecter à une variable nommée `currentIndex`. Cela doit se faire en une ligne.
