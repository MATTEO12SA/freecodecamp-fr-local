...
Pour compléter ta fonction `simulateFestival`, affiche un résumé des résultats de la simulation. Appelle `printSummary()` avec ton objet local `throughputSummary`.

# --hints--

Tu dois appeler `printSummary()` avec `throughputSummary`.
