...
Maintenant, il est temps de faire passer les participants par la porte. À l'intérieur de la boucle `while` :

- Décrémente `currentTickQueue` de 1 pour indiquer qu'un participant est passé. Utilise l'opérateur de décrémentation (`--`).
- Incrémente `processed` de 1 pour suivre combien de participants ont été traités. Utilise l'opérateur d'incrémentation (`++`).

# --hints--

À l'intérieur de ta boucle `while`, tu dois décrémenter `currentTickQueue` en utilisant l'opérateur de décrémentation `--` et incrémenter `processed` en utilisant l'opérateur d'incrémentation `++`.
