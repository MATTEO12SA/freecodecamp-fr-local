...
Maintenant que tu sais vers quelle porte rediriger, tu peux déplacer les participants en débordement.

Prends l'`overflowAmount` et ajoute-le à la file de la porte suivante au même `tickIndex`.

Plus précisément, tu dois :

- Accéder à la porte suivante en utilisant `nextGateIndex`.
- Accéder à sa `queue`.
- Ajouter l'`overflowAmount` à la valeur à la position `tickIndex`.

# --hints--

Ta fonction `rerouteOverflow` doit ajouter `overflowAmount` à la valeur située à `tickIndex` dans la `queue` de la porte suivante.
