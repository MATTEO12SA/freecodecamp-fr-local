...
Simule le créneau de la nuit en appelant `simulateFestival()` avec `nightGates` et `"Night"`.

Avec ça, tes simulations sont terminées !

# --hints--

Tu dois appeler `simulateFestival()` avec `nightGates` et `"Night"`.
