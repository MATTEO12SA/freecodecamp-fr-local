...
La dernière tâche de ta fonction `handleGateAtTick` est de gérer tout débordement pour le tick courant.

S'il existe un débordement (`result.overflow > 0`), tu dois :

- Afficher le message suivant `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
- Rediriger le débordement en appelant ta fonction `rerouteOverflow()` avec `gates`, `gate`, `tickIndex` et `result.overflow`.

# --hints--

S'il existe un débordement (`result.overflow > 0`), tu dois d'abord afficher le message `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
...
S'il existe un débordement (`result.overflow > 0`), tu dois rediriger le débordement en appelant `rerouteOverflow()` avec `gates`, `gate`, `tickIndex` et `result.overflow`.
