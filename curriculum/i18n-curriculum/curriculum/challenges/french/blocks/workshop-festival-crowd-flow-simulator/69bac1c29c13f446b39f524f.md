...
Tu dois maintenant implémenter la boucle principale de simulation. Cette boucle garantira que chaque porte est traitée à chaque tick de la simulation jusqu'à ce que tous les ticks soient terminés.

Crée une boucle `while` vide qui continue tant que `tickIndex` est inférieur à `maxTicks`.

# --hints--

Tu dois créer une boucle `while` vide avec la condition `(tickIndex < maxTicks)`.
