...
Ensuite, définis un objet nommé `bugMap` avec le type `Record<string, Bug<string>>` et assigne-lui un objet vide pour l’instant.

# --hints--

Tu dois avoir une constante nommée `bugMap`.
...
Ton `bugMap` doit avoir une annotation de type `Record<string, Bug<string>>`.
...
Ton `bugMap` doit être initialisé à un objet vide.
