...
Après le constructeur, crée une méthode abstraite `render` qui a un type de retour `void`. Tu l’utiliseras dans les sous-classes pour mettre à jour l’emoji affiché.

# --hints--

Ta classe `Bug` doit avoir une méthode abstraite `render`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Ta méthode `render` doit avoir un type de retour `void`.
