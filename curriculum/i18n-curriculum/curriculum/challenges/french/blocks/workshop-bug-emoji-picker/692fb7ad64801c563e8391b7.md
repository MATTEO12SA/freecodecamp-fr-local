...
Crée une sous-classe `Spider` qui étend la classe `Bug` avec le paramètre de type générique défini sur `string`.

# --hints--

Tu dois avoir une classe nommée `Spider`.
...
Ta classe `Spider` doit étendre `Bug`.
...
Ta classe `Spider` doit étendre `Bug` avec un argument de type `string`.
