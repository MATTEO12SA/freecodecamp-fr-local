...
Ensuite, crée une sous-classe nommée `Bee` qui étend la classe `Bug` avec le paramètre de type générique défini sur `string`.

# --hints--

Tu dois avoir une classe nommée `Bee`.
...
Ta classe `Bee` doit étendre `Bug`.
...
Ta classe `Bee` doit étendre `Bug` avec un argument de type `string`.
