...
À l’intérieur de la classe `Spider`, crée un constructeur qui prend un seul paramètre `emojiElement` de type `HTMLParagraphElement`.

À l’intérieur du constructeur, appelle la fonction `super` avec le paramètre `emojiElement` pour invoquer le constructeur de la classe parente `Bug`. Puis, définis la propriété `emoji` sur la valeur chaîne `"🕷️"`.

# --hints--

Ta classe `Spider` doit avoir un constructeur.
...
Ton constructeur `Spider` doit avoir un paramètre nommé `emojiElement` de type `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ton constructeur `Spider` doit appeler `super` avec le paramètre `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Ton constructeur `Spider` doit définir `this.emoji` sur `"🕷️"`.
