...
Maintenant, remplis `bugMap` avec deux entrées. Ajoute `"bee"` comme clé associée à une nouvelle instance de `Bee`, et `"spider"` comme clé associée à une nouvelle instance de `Spider`. Passe `bugEmojiElement` comme argument des deux.

# --hints--

Ton `bugMap` doit avoir une entrée `"bee"` définie sur `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Ton `bugMap` doit avoir une entrée `"spider"` définie sur `new Spider(bugEmojiElement)`.
