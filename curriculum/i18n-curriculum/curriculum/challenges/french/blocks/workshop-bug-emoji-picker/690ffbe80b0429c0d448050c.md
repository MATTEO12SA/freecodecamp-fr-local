...
Maintenant, tu dois définir les propriétés que tous les insectes auront.

À l’intérieur de la classe `Bug`, déclare une propriété nommée `emoji` avec le type `T` en utilisant l’opérateur d’assertion d’assignation définie (`!`). Cela indique à TypeScript que la propriété sera assignée plus tard.

# --hints--

Ta classe `Bug` doit avoir une propriété nommée `emoji`.
...
Ta propriété `emoji` doit avoir le type `T`.
...
Ta propriété `emoji` doit utiliser l’opérateur d’assertion d’assignation définie (`!`).
