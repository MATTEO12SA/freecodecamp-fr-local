...
Jusqu’à présent, rien ne change quand tu sélectionnes un insecte différent.

Ajoute un écouteur d’événement `change` à `selectElement`. Dans le callback, utilise `isSelect` pour vérifier si `e.target` est un `HTMLSelectElement`. Tu n’as pas encore besoin de faire quoi que ce soit à l’intérieur du bloc `if`.

# --hints--

Ton `selectElement` doit avoir un écouteur d’événement `change` avec un callback qui prend un paramètre événement.
...
Ton callback d’écouteur d’événement doit avoir une instruction `if` avec la condition `isSelect(e.target)`.
