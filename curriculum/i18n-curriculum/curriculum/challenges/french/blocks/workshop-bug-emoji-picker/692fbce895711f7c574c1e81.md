...
Maintenant, à l’intérieur de la fonction `isSelect`, renvoie un booléen indiquant si `element` est une instance de `HTMLSelectElement`. Utilise l’opérateur `instanceof` pour effectuer cette vérification.

# --hints--

Ta fonction `isSelect` doit renvoyer `element instanceof HTMLSelectElement`.
