...
Récupère l’élément `select` avec l’ID `species` depuis le DOM et stocke-le dans une constante nommée `selectElement`. Utilise l’opérateur d’assertion non-null `!` pour affirmer que l’élément n’est pas `null`

# --hints--

Tu dois avoir une constante nommée `selectElement`.
...
Ton `selectElement` doit interroger l’élément avec l’ID `species` en utilisant `document.querySelector` avec un argument de type `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

Ton `selectElement` doit utiliser l’opérateur d’assertion non-null (`!`).
