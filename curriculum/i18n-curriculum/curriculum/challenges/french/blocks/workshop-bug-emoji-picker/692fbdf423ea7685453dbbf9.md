...
Tu vas maintenant commencer à référencer les éléments DOM nécessaires.

Récupère l’élément paragraphe avec l’ID `bug-emoji` et stocke-le dans une constante nommée `bugEmojiElement`. Utilise l’opérateur d’assertion non-null `!` pour affirmer que l’élément n’est pas `null`.

# --hints--

Tu dois avoir une constante nommée `bugEmojiElement`.
...
Ton `bugEmojiElement` doit interroger l’élément avec l’ID `bug-emoji` en utilisant `document.querySelector` avec un argument de type `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Ton `bugEmojiElement` doit utiliser l’opérateur d’assertion non-null (`!`).
