...
Ensuite, crée un constructeur pour la classe `Bug` qui prend un seul paramètre nommé `emojiElement` de type `HTMLParagraphElement`. À l’intérieur du constructeur, assigne le paramètre `emojiElement` à la propriété `emojiElement` de la classe.

# --hints--

Ta classe `Bug` doit avoir un constructeur.
...
Ton constructeur `Bug` doit avoir un paramètre nommé `emojiElement` de type `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ton constructeur `Bug` doit assigner `emojiElement` à `this.emojiElement`.
