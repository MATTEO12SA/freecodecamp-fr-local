...
Maintenant que la vérification `if` est en place, utilise `e.target.value` pour chercher l’insecte correspondant dans `bugMap` et appelle sa méthode `render`. Ensuite, tu devrais pouvoir sélectionner les insectes et voir le résultat tout de suite.

Avec ça, ton atelier Bug Emoji Picker est terminé !

# --hints--

Tu dois appeler la méthode `render` sur l’insecte correspondant depuis `bugMap`.
