...
À l’intérieur de la classe `Bee`, implémente la méthode `render`, préfixée par le mot-clé `override`, pour mettre à jour la propriété `innerText` de `emojiElement` avec la valeur de la propriété `emoji`.

# --hints--

Ta classe `Bee` doit avoir une méthode `render` utilisant le mot-clé `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Tu dois définir `this.emojiElement.innerText` sur `this.emoji` à l’intérieur de la méthode `render` de `Bee`.
