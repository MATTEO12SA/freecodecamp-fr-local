...
Crée une fonction nommée `isSelect` qui prend un seul paramètre `element` de type `EventTarget | null`. Cette fonction vérifiera si l’élément sélectionné est un élément HTML `<select>`.

# --hints--

Tu dois avoir une fonction nommée `isSelect`.
...
Ta fonction `isSelect` doit avoir un paramètre nommé `element` de type `EventTarget | null`.
...
Ta fonction `isSelect` doit avoir un type de retour `element is HTMLSelectElement`.
