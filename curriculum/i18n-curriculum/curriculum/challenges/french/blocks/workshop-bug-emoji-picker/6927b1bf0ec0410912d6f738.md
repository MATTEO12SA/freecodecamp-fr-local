...
Toujours à l’intérieur de la classe `Bug`, déclare une propriété nommée `emojiElement` avec le type `HTMLParagraphElement`. Cette propriété contiendra une référence à l’élément paragraphe HTML où l’emoji de l’insecte sera affiché.

# --hints--

Ta classe `Bug` doit avoir une propriété nommée `emojiElement`.
...
Ta propriété `emojiElement` doit avoir le type `HTMLParagraphElement`.
...
Ta propriété `emojiElement` doit utiliser l’opérateur d’assertion d’assignation définie (`!`).
